	...
	switch($method){
		...
		*/
		...
		case 'POST':
				...
				if($result['status']){
        			$response['status'] = 202;
        			$response['data']=array('success'=>'Informacion registrada','prefijo'=>$result['prefijo'],'numero_factura'=>$result['numero_factura'],'numero_factura_completo'=>$result['numero_factura_completo']);
				}else{
					$response['status'] = 400;
