<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory){
            ...
            $nameFileUpload = $random1.''.$random2; // ID UNICO

            if ($this->file->save($uploadDirectory.$nameFileUpload.'.'.$ext)){

                return array('success'=>true,'nameFileUpload' =>  $nameFileUpload.'.'.$ext);
    ...
    //CONDICION PARA QUE FUNCIONE EN LOCAL Y EN DESARROLLO
    $url = $_SERVER['DOCUMENT_ROOT'].'/ARCHIVOS_PROPIOS/documentos_crear_empresa/';
    if(!file_exists($url)){ mkdir ($url); }

    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($ruta1);
