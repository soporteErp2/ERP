...
<?php
    ...
    if(isset($idConsecutivoCarga)){
        ...
        $id_cliente_cotizacion      = mysql_result($queryConsulOrden,0,'id_proveedor');
        $cod_cliente_cotizacion     = mysql_result($queryConsulOrden,0,'cod_proveedor');
        $cliente_cotizacion         = mysql_result($queryConsulOrden,0,'proveedor');
        ...
        $sqlInsertFactura   = "INSERT INTO $tablaPrincipal(id_empresa,random,nit,id_proveedor,cod_proveedor,proveedor,fecha_inicio,id_sucursal,id_bodega,documento_vendedor,nombre_vendedor)
                                VALUES('$empresa','$random_factura','$nit','$id_cliente_cotizacion','$cod_cliente_cotizacion','$cliente_cotizacion','$fecha','$filtro_sucursal','$filtro_bodega','$documento_vendedor','$nombre_vendedor')";
        ...
        $id_entrada_almacen   = mysql_result(mysql_query($sqlSelectIdFactura,$link),0,'id');
        ...
        while ($rowCotizacion=mysql_fetch_array($queryConsultaInventario)) {
            ...
            $cadenaInsert.="('$id_entrada_almacen',
       ...
       //CARGA DATOS EN LA INTERFACE
        ...
        $acumScript .=  'id_cliente_'.$opcGrillaContable.'                                    = "'.$id_cliente_cotizacion.'";
                        document.getElementById("codCliente'.$opcGrillaContable.'").value     = "'.$cod_cliente_cotizacion.'";
        ...
        $bodyArticle = cargaArticulosSave($id_entrada_almacen,$observacion,0,$opcGrillaContable,$tablaInventario,$idTablaPrincipal,$link);  
    }

    //============================================ SI NO SE CARGA DOCUMENTO DE REFERENCIA PREVIO ===================================================
    else if(!isset($id_entrada_almacen)){
        ...
        $id_entrada_almacen  = mysql_result(mysql_query($sqlSelectId,$link),0,'id');
        
    }

    //======================================================= CARGA SI LA FACTURA EXISTE =================================================================
    else{
        
        include("../bd/functions_body_article.php");

        $sql   = "SELECT id_proveedor,cod_proveedor,nit,proveedor,date_format(fecha_inicio,'%Y-%m-%d') AS fecha, observacion, estado,nombre_vendedor 
                FROM $tablaPrincipal  WHERE id='$id_entrada_almacen' AND activo = 1";
        ...
        if ($opcGrillaContable=='EntradaAlmacen' && $estado=='1') { echo "ESTA ENTRADA DE ALMACEN ESTA CERRADA "; exit; }
     
        $arrayReplaceString = array("\n", "\r","<br>");
        $observacion = str_replace($arrayReplaceString, "\\n", mysql_result($query,0,'observacion'));

   
        $acumScript .=  'id_cliente_'.$opcGrillaContable.'                                      = "'.$id_proveedor.'";
                        document.getElementById("codCliente'.$opcGrillaContable.'").value       = "'.$cod_proveedor.'";                 
                        document.getElementById("nitCliente'.$opcGrillaContable.'").value       = "'.$nit.'";
                        ...
                        codigoCliente'.$opcGrillaContable.' = "'.$cod_proveedor.'";
                        nitCliente'.$opcGrillaContable.'    = "'.$nit.'";
                        nombreCliente'.$opcGrillaContable.' = "'.$proveedor.'";';
         
        $bodyArticle = cargaArticulosSave($id_entrada_almacen,$observacion,$estado,$opcGrillaContable,$tablaInventario,$idTablaPrincipal,$link);
    }
    ...
    //======================== DOCUMENTOS AGREGADOS A LA PRESENTE ENTRADA ======================//
    $acumDocReferencia  = '';
    $margin_left        = '';
    $sqlDocReferencia   = "SELECT DISTINCT id_consecutivo_referencia AS id_referencia, consecutivo_referencia AS cod_referencia, LEFT(nombre_consecutivo_referencia,1) AS doc_referencia
                            ...
                            WHERE id_consecutivo_referencia>0 AND $idTablaPrincipal='$id_entrada_almacen' AND activo=1 AND id_empresa='$empresa'";
    ...
    <div class="bodyTop">
        <div class="contInfoFact">
            ...
            <div class="contTopFila">
                ...
                <div class="renglonTop">
                    <div class="labelTop">Orden de Compra</div>
                ...
                <div class="renglonTop">
                    <div class="labelTop">N. Doc. Proveedor</div>
                    <div class="campoTop"><input type="text" id="documentoProveedor<?php echo $opcGrillaContable; ?>" ></div>
                </div>
                <div class="renglonTop">
                ...
                <div class="renglonTop" title="Si los articulos son obsequios, entonces no son facturables">
                    <div class="labelTop">Opciones</div>
                    <div class="campoTop" align="center"><input type="checkbox" id="checkboxFacturable<?php echo $opcGrillaContable; ?>" onchange="cambiaFacturable<?php echo $opcGrillaContable; ?>(event,this);" /> No facturable</div>
                </div>
...
<script>
    ...
    //===================== FUNCION PARA ESTABLECER LA ENTRADA COMO FACTURABLE O NO ============================//
    function cambiaFacturable<?php echo $opcGrillaContable; ?>(){
        check = document.getElementById('checkboxFacturable<?php echo $opcGrillaContable; ?>');

        if (check.checked) { 

            //CON ESTE AJAX ACTUALIZAMOS LA ENTRADA COMO NO FACTURABLE
            Ext.get('renderOpcionEntrada').load({
                url     : 'bd/bd.php',
                scripts : true,
                nocache : true,
                params  :
                {
                    opc               : 'actualizaEntradaFacturable',
                    opcGrillaContable : '<?php echo $opcGrillaContable; ?>',
                    id                : '<?php echo $id_entrada_almacen; ?>',
                    accion : 'false'
                }
            });
        }
        else{
            //CON ESTE AJAX ACTUALIZAMOS LA NOTA COMO FACTURABLE
            Ext.get('renderOpcionEntrada').load({
                url     : 'bd/bd.php',
                scripts : true,
                nocache : true,
    ...
    function agregarDocumento<?php echo $opcGrillaContable; ?>(){
        ...
        Ext.get("renderCargaCotizacionPedido<?php echo $opcGrillaContable; ?>").load({
            ...
            params  : 
            {
                ...
                id_factura        : '<?php echo $id_entrada_almacen ?>',
                opcGrillaContable : "<?php echo $opcGrillaContable; ?>",
                filtro_bodega     : document.getElementById("filtro_ubicacion_<?php echo $opcGrillaContable; ?>").value                      
            }
        });
    ...
    function ventanaBuscarCotizacionPedido<?php echo $opcGrillaContable; ?>(){
        ...
        nombreGrillaCotizacionPedido="grillaOrdenCompra";
    ...
    function guardarNewArticulo<?php echo $opcGrillaContable; ?>(cont){
        ...
        else if(cantArticulo<?php echo $opcGrillaContable; ?> < 1 || cantArticulo<?php echo $opcGrillaContable; ?> == ''){document.getElementById('cantArticulo<?php echo $opcGrillaContable; ?>_'+cont).blur(); alert('El campo Cantidad es obligatorio'); document.getElementById('cantArticulo<?php echo $opcGrillaContable; ?>_'+cont).focus();  return; }
        else if(costoArticulo<?php echo $opcGrillaContable; ?> < 1 || costoArticulo<?php echo $opcGrillaContable; ?> == ''){ document.getElementById('costoArticulo<?php echo $opcGrillaContable; ?>_'+cont).blur();alert('El campo costo es obligatorio'); document.getElementById('costoArticulo<?php echo $opcGrillaContable; ?>_'+cont).focus();  return; }
    ...
    function guardar<?php echo $opcGrillaContable; ?>(){ 
        ...
        var validacion = validarArticulos<?php echo $opcGrillaContable; ?>();
        if (validacion==0) { alert("No se puede generar una factura sin articulos relacionados!"); return; }
    ...
    function validarNumberArticulo<?php echo $opcGrillaContable; ?>(event,input,typeValidate,cont){
        ...
        if(tecla == 13){
            if(nombreInput == 'cantArticulo<?php echo $opcGrillaContable; ?>'){ 
                ...
                if(arrayRemisionesSaldos[id_insert] < numero){
                    input.blur();
                    alert('la cantidad no puede ser mayor a la ya registrada en la remision');
                    input.value = arrayRemisionesSaldos[id_insert];
    ...
    function imprimir<?php echo $opcGrillaContable; ?> (){
        window.open("bd/imprimirGrillaContable.php?id=<?php echo $id_entrada_almacen; ?>&opcGrillaContable=<?php echo $opcGrillaContable; ?>&tablaPrincipal=<?php echo $tablaPrincipal; ?>&idTablaPrincipal=<?php echo $idTablaPrincipal; ?>&tablaInventario=<?php echo $tablaInventario; ?>&tablaRetenciones=<?php echo $tablaRetenciones; ?>");                                                                                                                                                          
    }

    //================================== IMPRIMIR EN EXCEL =================================================================//
    function imprimir<?php echo $opcGrillaContable; ?>Excel (){
       window.open("bd/exportarExcelGrillaContable.php?id=<?php echo $id_entrada_almacen; ?>&opcGrillaContable=<?php echo $opcGrillaContable; ?>&tablaPrincipal=<?php echo $tablaPrincipal; ?>&idTablaPrincipal=<?php echo $idTablaPrincipal; ?>&tablaInventario=<?php echo $tablaInventario; ?>&tablaRetenciones=<?php echo $tablaRetenciones; ?>");                                                                                                                                                          
    }
