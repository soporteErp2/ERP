<?php
	function cargaArticulosSave($id,$observacion,$estado,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$link){
		...
		//VARIABLE GUARDA DATOS DE REMISION CUANDO SE FACTURA
    	$scriptRemision          	= '';
		...
		// echo "<script>console.log($tablaInventario);</script>";
		  ...
		  $sql = "SELECT
			...
			$sqlConfirmSaldo = "SELECT
													...
													FROM
														...
														ventas_pedidos_inventario AS TS
													WHERE
														TI.$idTablaPrincipal='$id'
													AND
														TI.activo = 1
													AND
														TI.nombre_consecutivo_referencia='Pedido'
													AND
														TS.id=TI.id_tabla_inventario_referencia
													GROUP BY TI.id";
			$queryConfirmSaldo = mysql_query($sqlConfirmSaldo,$link);

			while($rowConfirmSaldo = mysql_fetch_array($queryConfirmSaldo)){
				$scriptRemision .= 'objDocumentosCruce'.$opcGrillaContable.'["'.$rowConfirmSaldo['id'].'"] ={typeDoc:"Pedido",saldo_cantidad:"'.$rowConfirmSaldo['saldo_cantidad'].'"};';
			}

			if($opcGrillaContable=='FacturaVenta'){
				$sqlConfirmSaldo = "SELECT
															TI.id,TS.saldo_cantidad
														FROM
															$tablaInventario AS TI,
															ventas_remisiones_inventario AS TS
														WHERE
															TI.$idTablaPrincipal='$id'
														AND
															TI.activo = 1
														AND
															TI.nombre_consecutivo_referencia='Remision'
														AND
															TS.id=TI.id_tabla_inventario_referencia
														GROUP BY TI.id";
				$queryConfirmSaldo = mysql_query($sqlConfirmSaldo,$link);
				while($rowConfirmSaldo = mysql_fetch_array($queryConfirmSaldo)){
					$scriptRemision .= 'objDocumentosCruce'.$opcGrillaContable.'["'.$rowConfirmSaldo['id'].'"] ={typeDoc:"Remision",saldo_cantidad: "'.$rowConfirmSaldo['saldo_cantidad'].'"};';
				}
			}
		...
		$body = '<script>
						...
						<div class="contenedorHeadArticulos">
								<div class="headArticulos" id="head'.$opcGrillaContable.'">
									...
									<div class="label" style="width:21px">A.F.</div>
		...
		if ($opcGrillaContable=="FacturaVenta" || $opcGrillaContable=="RemisionesVenta" || $opcGrillaContable=='PedidoVenta') {
			...
			if ($estado < 1) {
				...
				while($row = mysql_fetch_array($query)){
					...
					$cont++;
					$body .='<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
								'.cargaDivsUnidadesBloqueadas($cont,$opcGrillaContable, $row['id'], $row['id_inventario'], $row['codigo'], $row['nombre'], $row['cantidad'], $row['costo_unitario'], $row['tipo_descuento'], $row['descuento'],$row['id_impuesto'],$row['impuesto'], $row['valor_impuesto'],$estado,$row['nombre_unidad_medida'],$row['cantidad_unidad_medida'],$row['check_opcion_contable']).'
							</div>';
				}
				$mostarBoton='disable()';
				$deshabilita='readonly';
		...
		$body .='	</div>
				...
				<div class="contenedor_totales" id="contenedor_totales_'.$opcGrillaContable.'">
					<div class="contenedorObservacionGeneral">
						...
						<textarea id="observacion'.$opcGrillaContable.'" '.$eventoObservacionGrilla.' '.$deshabilita.'>'.$observacion.'</textarea>
					</div>
	...
	function cargaDivsUnidadesSave($cont,$opcGrillaContable,$color='',$id = 0,$id_inventario = '',$codigo = '',$nombre = '',$cantidad = 0,$costo_unitario = 0,$tipoDescuento='',$descuento = 0, $id_impuesto=0,$impuesto='',$valor_impuesto=0, $estado='',$nombre_unidad='',$cantidad_unidad='',$check_opcion_contable,$NCR='',$ICR='',$CR=''){
		...
		if($opcGrillaContable=='EntradaAlmacen'){//PARA QUE GUARDE AL DARLE CANTIDAD CUANDO NO ESTAN VISIBLES LOS OTROS CAMPOS
      		if(user_permisos(180,'false') == 'false'){
				$displayCampos='display:none';
		...
		$body ='<div class="campo" style="width:40px !important;">
					...
					<div class="'.$clase.'"><input type="text" id="unidades'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly '.$deshabiltar.'value="'.$mostrarUnidad.'" /></div>
					<div class="'.$clase.'"><input type="text" id="cantArticulo'.$opcGrillaContable.'_'.$cont.'" value="'.($cantidad * 1).'" '.$deshabiltar.' onKeyup="'.$eventoCantidad.'"/></div>
					...
					<div class="campo" style="'.$displayCampos.'"><input type="text" id="costoArticulo'.$opcGrillaContable.'_'.$cont.'" '.$deshabiltar.' '.$readonly_precio.'  '.$readonly_precio_remision_adjunta.' value="'.$costo_unitario.'" onKeyup="guardarAuto'.$opcGrillaContable.'(event,this,'.$cont.');" value="0"/></div>

					<div class="campo" style="'.$displayCampos.'"><input type="text" id="costoTotalArticulo'.$opcGrillaContable.'_'.$cont.'" '.$deshabiltar.'   readonly/></div>

					<div id="div_check_entrada_activo_fijo_'.$cont.'" class="campoOptionCheck" style="width:21px">
						<input type="checkbox" id="check_entrada_activo_fijo_'.$cont.'" class="optionCheckContable_'.$cont.'" onchange="changeCheckOptionContable('.$cont.', this);"/>
					</div>
              	...
              	<div style="width:7px; height:2px; background-color:#fff; margin:7px 4px;"></div>
						...
						<script>
							var codigoNulo = document.getElementById("eanArticulo'.$opcGrillaContable.'_'.$cont.'").value
							var checkbox = document.getElementById("check_entrada_activo_fijo_'.$cont.'").value;
							if("'.$check_opcion_contable.'" == "activo_fijo"){
								document.getElementById("check_entrada_activo_fijo_'.$cont.'").checked = true;
							}
							if(codigoNulo == ""){
								document.getElementById("div_check_entrada_activo_fijo_'.$cont.'").innerHTML = "";
								document.getElementById("cantArticulo'.$opcGrillaContable.'_'.$cont.'").value = "";
							}
	...
	function cargaDivsUnidadesBloqueadas($cont,$opcGrillaContable,$id = 0,$id_inventario = '',$codigo = '',$nombre = '',$cantidad = 0,$costo_unitario = 0,$tipoDescuento='',$descuento = 0, $id_impuesto=0,$impuesto='',$valor_impuesto=0,$estado='',$nombre_unidad='',$cantidad_unidad='', $check_opcion_contable){
		...
		$body ='<div class="campo" style="width:40px !important;">
				...
				<div class="'.$clase.'"><input type="text" id="unidades'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly value="'.$nombre_unidad.' x '.$cantidad_unidad.'" /></div>
				<div class="'.$clase.'"><input type="text" id="cantArticulo'.$opcGrillaContable.'_'.$cont.'" value="'.($cantidad * 1).'"  readonly /></div>
				...
				<div class="campo" style="'.$displayCampos.'"><input type="text" id="costoArticulo'.$opcGrillaContable.'_'.$cont.'"  value="'.$costo_unitario.'" readonly  value="0"/></div>

				<div class="campo" style="'.$displayCampos.'"><input type="text" id="costoTotalArticulo'.$opcGrillaContable.'_'.$cont.'"    readonly/></div>

				<div class="campoOptionCheck" style="width:21px">
					<input type="checkbox" disabled="true" id="check_entrada_activo_fijo_'.$cont.'" class="optionCheckContable_'.$cont.'" onchange="changeCheckOptionContable('.$cont.', this);"/>
				</div>
