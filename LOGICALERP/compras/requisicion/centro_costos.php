<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//NOMBRE DE LA GRILLA
			$grilla->GrillaName	 		= 'centroCostos_'.$opcGrillaContable;	//NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
		//QUERY
			...
			$grilla->MySqlLimit			= '0,1000';			//LIMITE DE LA CONSULTA
		//TAMANO DE LA GRILLA
...
if(!isset($opcion)){  ?>
	<script>
	function Agregar_centroCostos_<?php echo $opcGrillaContable; ?>(){ }
	function Editar_centroCostos_<?php echo $opcGrillaContable; ?>(id){
		var codigoCcos = ''
		,	contError  = 0
		,	cCos       = ''

		//VALIDAR QUE EL CENTRO DE COSTOS SEA HIJO Y NO PADRE
		codigoCcos = document.getElementById('div_centroCostos_<?php echo $opcGrillaContable; ?>_codigo_'+id).innerHTML
		cCos       = $(".my_grilla_celdas");
			Object.keys(cCos).forEach(
				function(codigo, key, array) {
					if (codigoCcos.length <= cCos[key]) {return}
					if (typeof(cCos[key]) == 'undefined') {return}
					if (codigoCcos == cCos[key].innerHTML){ return }
					if (codigoCcos == cCos[key].innerHTML.substring(0, codigoCcos.length)){ contError++; /*console.log(`Codigo: ${codigoCcos} hijos ${cCos[key].innerHTML} `);*/ }
					// VERIFICAR CUANDO SE TERMINEN DE RECORRER TODOS LOS ELEMENTOS (CALLBACK MANUAL) Y LLAMAR LA FUNCION
					if(key == cCos.length - 1 && contError==0) { <?php echo $impressFunctionScript; ?> }
					if(key == cCos.length - 1 && contError>0){ alert("Aviso\nNo se pueden seleccionar centro de costos padre"); }
				}
			)
