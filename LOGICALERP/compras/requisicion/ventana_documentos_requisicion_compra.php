<?php
    ...
    //CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //NOMBRE DE LA GRILLA
            $grilla->GrillaName         = 'requisicionCompraDocumentos';     //NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
        //QUERY
            $grilla->TableName          = 'compras_requisicion_documentos';       //NOMBRE DE LA TABLA EN LA BASE DE DATOS
            $grilla->MyWhere            = "activo=1 AND id_requisicion_compra='$id_requisicion_compra'";     //WHERE DE LA CONSULTA A LA TABLA "$TableName"
        ...
        //CONFIGURACION DE CAMPOS EN LA GRILLA
            ...
            $grilla->AddRowImage('','<center><div id="renderDeleteDocumentoRequisicion_[id]" style="float:left; max-width:20px; overflow:hidden;"></div>
                                    <img src="img/delete.png" style="cursor:pointer" style="width:16px; height:16px;" onclick="deleteDocumentoRequisicion([id],\'[nombre]\',\'[ext]\');" /></center>','20');
            $grilla->AddRowImage('Ver','<center><img src="img/ver.png" style="cursor:pointer" style="width:16px; height:16px;" onclick="viewDocumentoRequisicion([id],\'[nombre]\',\'[ext]\');" /></center>','25');
...
if(!isset($opcion)){ ?>
    ...
    <script>
        ...
        function Editar_requisicionCompraDocumentos(){}

        function deleteDocumentoRequisicion(id_documento,nombre,ext){
            if(!confirm("Aviso,\nEsta seguro de eliminar un documento cargado?")) return;

            Ext.get('renderDeleteDocumentoRequisicion_'+id_documento).load({
                url     : 'requisicion/bd/bd.php',
                scripts : true,
                nocache : true,
                params  :
                {
                    opc         : 'deleteDocumentoRequisicion',
		...
		function createUploader(){
	        ...
	        var uploader = new qq.FileUploader({
	            ...
	            action  : 'requisicion/upload_file/upload_file.php',
	            debug   : false,
	            ...
	            onComplete        : function(id, fileName, responseJSON){
                                        ...
                                        if(responseJSON.idInsert > 0){ Inserta_Div_requisicionCompraDocumentos(responseJSON.idInsert); }
	                                },
        ...
        function viewDocumentoRequisicion(id,nameFile,ext){

            if(!validaImagen(ext)){
                window.open("requisicion/bd/bd.php?opc=downloadFile&nameFile="+nameFile+"&ext="+ext+"&id="+id);
        ...
        function ventanaViewDocumento(id,nameFile,ext,width,height){
            ...
            var titulo = (document.getElementById('div_requisicionCompraDocumentos_nombre_'+id).innerHTML)+'.'+(document.getElementById('div_requisicionCompraDocumentos_ext_'+id).innerHTML);
