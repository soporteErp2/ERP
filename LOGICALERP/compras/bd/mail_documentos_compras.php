<?php
    ...
    */
	...
	if($opcGrillaContable == 'OrdenCompra'){
		...
		$dir_adjunto  = 'ordenes_compra';

		$sql              = "SELECT id_proveedor,proveedor FROM $tabla WHERE id = '$id'";
		$con_proveedor    = mysql_query($sql,$link);
		$id_proveedor     = mysql_result($con_proveedor,0,"id_proveedor");
		$nombre_proveedor = mysql_result($con_cliente,0,"proveedor");
	}
	else if($opcGrillaContable == 'ComprobanteEgreso'){
		$tabla       = 'comprobante_egreso';
		$dir_adjunto = 'comprobante_egreso';

		$sql              = "SELECT id_tercero,tercero FROM $tabla WHERE id = '$id'";
		$con_proveedor    = mysql_query($sql,$link);
		$id_proveedor     = mysql_result($con_proveedor,0,"id_tercero");
		$nombre_proveedor = mysql_result($con_cliente,0,"tercero");

	}/*
	else if($opcGrillaContable == 'RemisionesVenta'){
		$tabla       = 'ventas_remisiones';
		$dir_adjunto = 'remisiones';
	}*/



	//$file = file_exists("../bd/js.php");
/*
	if($CUAL == 4){
		$formato = "cotizacion_mail";
	}
	if($CUAL == 5){
		$formato = "contrato_mail";
	}
	if($CUAL == 6){
		$formato = "pedido_mail";
	}
	...
	if($idioma == 'espanol'){
		$cual_columna = 'encabezado';
	}
	if($idioma == 'ingles'){
		$cual_columna = 'encabezado_ingles';
	}
*/
			...
			<fieldset  style="width:290px; height:236px; padding:0 0 0 0; margin:10px 0 0 0;  overflow:hidden;  border:1px solid #006699">
				...
				<div style="float:left; width:280px; height:236px; margin:0 0 0 10px; overflow:auto; overflow-x:hidden">
					<?php
						...
						$SqlEmail="	SELECT 	email FROM terceros
						...
						$SqlCheck="	SELECT 	c.nombre AS nombre_contacto,
									...
									WHERE 	c.id_tercero=$id_proveedor
						...
						$varInputsCheck.='		    <div style="float:left; margin:2px 0 0 20px; width:100%">
														<div style="float:left"><input name="env_email_cartera" type="checkbox" id="env_email_cartera" value="'.$email_cartera.'" checked disabled /></div>
