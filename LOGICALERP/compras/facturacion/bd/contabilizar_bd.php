<?php
  ...
  function contabilizarConPlantilla($fechaInicioFactura,$consecutivoFactura,$idBodega,$idSucursal,$idEmpresa,$idPlantilla,$idFactura,$idProveedor,$link){
		...
		while($rowplantilla = mysql_fetch_array($queryPlantilla)){
			...
			if($rowplantilla['descripcion'] == 'IMPUESTO'){
				if($rowplantilla['porcentaje'] == ''){ echo'<script>alert("Aviso.\nLa plantilla Colgaap ingresada tiene iva sin porcentaje.")</script>'; exit; }
		...
		if($contCuentasPantillas == 0){ echo'<script>alert("Aviso.\nLa plantilla Colgaap ingresada no tiene configuracion contable.")</script>'; exit; }
		else if($contCuentasPantillas == 1){ echo'<script>alert("Aviso.\nLa plantilla Colgaap ingresada no cumple doble partida.")</script>'; exit; }

		//COMPRAS FACTURA INVENTARIO
		...
		while($rowArticulo = mysql_fetch_array($queryArticulos)){
			...
			if($rowArticulo['check_opcion_contable'] == 'activo_fijo'){
		...
		if($contCuentasPantillas > 1){
			foreach ($arrayCuentas AS $cuentaPuc => $valArrayCuenta){
				...
				if($valArrayCuenta['descripcion'] == 'CONTRAPARTIDA PRECIO'){ $saldo = $cuentaContraPartida; }		//CUENTA DE PAGO
				...
				if($configCuenta2 == 22 || $configCuenta4 == 2335 || $configCuenta4 == 2340 || $configCuenta4 == 2345){ $saldoGlobalFacturaSinAbono += $saldo; } 	//ACUMULADOR VARIABLE GLOBAL TOTAL FACTURA SIN ABONO
			...
			if($globalDebito != $globalCredito){echo'<script>alert("Aviso.\nLa plantilla Colgaap ingresada no cumple doble partida, Confirme su configuracion en el modulo panel de control.")</script>'; exit; }
  ...
  function contabilizarSinPlantilla($arrayAnticipo,$arrayCuentaPago,$fechaInicioFactura,$consecutivoFactura,$consecutivoDocReferencia,$idBodega,$idSucursal,$idEmpresa,$idFactura,$idProveedor,$link){
		...
		$arrayCuentasTransito['id_cuenta_colgaap_debito']  = mysql_result($query,0,'id_cuenta_colgaap_debito');
		$arrayCuentasTransito['cuenta_colgaap_debito']     = mysql_result($query,0,'cuenta_colgaap_debito');
		$arrayCuentasTransito['id_cuenta_colgaap_credito'] = mysql_result($query,0,'id_cuenta_colgaap_credito');
		$arrayCuentasTransito['cuenta_colgaap_credito']    = mysql_result($query,0,'cuenta_colgaap_credito');
		...
		foreach($arrayInventarioFactura AS $valArrayInventario){
			...
			//**********************************************************************************************************//
				...
				if($cuentaPrecio == ''){
					echo'<script>
							alert("Aviso.\nEl item Codigo '.$valArrayInventario['codigo'].' no tiene asignado la cuenta de Activo Fijo en configuracion items");
							document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
						</script>';
					exit;
				}
		...
		if(round($saldoAnticipo,$_SESSION['DECIMALESMONEDA']) > 0 && round($saldoAnticipo,$_SESSION['DECIMALESMONEDA']) > round($saldoClientes,$_SESSION['DECIMALESMONEDA'])){
			echo'<script>
					alert("Aviso.\nLos anticipos no pueden ser mayores a la factura de compra! '.$saldoAnticipo.' > 0 && '.$saldoAnticipo.' > '.$saldoClientes.'");
					document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
				</script>';
			exit;
		}
		...
		//CUENTAS NIIF
		$sqlInsertCuentasColgaap = "INSERT INTO
												asientos_colgaap(
													id_documento,
													consecutivo_documento,
													tipo_documento,
		...
		contabilizacionSimultanea($idFactura,'FC',$idSucursal,$idEmpresa,$link);
  }

  function contabilizarSinPlantillaManual($arrayCuentaPago,$fechaInicioFactura,$consecutivoFactura,$consecutivoDocReferencia,$idBodega,$idSucursal,$idEmpresa,$idFactura,$idProveedor,$link){
  	...
  	global $saldoGlobalFactura, $saldoGlobalFacturaSinAbono;
		...
		$arrayAsiento[$cuentaPago]['type']        = 'cuentaPago';
		...
		$saldoGlobalFactura = $total_manual;
