<?php
	...
	//=============================// ANTICIPOS //=============================//
    //*************************************************************************//
    $sqlAnticipos   = "SELECT SUM(valor) AS valorAnticipos FROM anticipos WHERE id_documento='$idFactura' AND activo=1 AND tipo_documento='FC' AND id_empresa='$id_empresa'";
    $queryAnticipos = mysql_query($sqlAnticipos,$link);
    $totalAnticipo  = mysql_result($queryAnticipos, 0, 'valorAnticipos');
    $totalAnticipo *= 1;

	//====================// CONSULTA CUENTAS ANTICIPO EN COMPRAS //====================//
	//**********************************************************************************//
	$whereCuenta = "";
	$sqlCuenta   = "SELECT cuenta FROM puc WHERE tipo='Anticipo a proveedor' AND activo=1 AND id_empresa='$id_empresa'";
	...
	$sqlAnticipo = "SELECT E.consecutivo,
					...
					FROM comprobante_egreso AS E,
						comprobante_egreso_cuentas AS C LEFT JOIN anticipos AS A ON(
							A.activo=1
							AND A.id_cuenta_anticipo=C.id
							AND A.id_documento='$idFactura'
							AND A.tipo_documento='FC'
							AND A.tipo_documento_anticipo='CE'
							AND A.valor > 0
	...
	while ($rowCuentas = mysql_fetch_array($queryAnticipo)) {
		...
		$filaGrilla .= '<div class="fila_'.$opcGrilla.'" id="fila_'.$opcGrilla.'_'.$contFila.'" ondblclick="ventanaValorAnticipo(\''.$contFila.'\',\''.$rowCuentas['id_anticipo'].'\',\''.$rowCuentas['id_cuenta_anticipo'].'\');">
							...
							<div class="campo3" style="width:30px; text-align:left;">CE</div>
...
<script>
	...
	function ventanaValorAnticipo(contFila,id_anticipo,id_cuenta_anticipo){
		var	valor_Factura  = totalFacturaCompra
        ...
        valor_Factura=valor_Factura.toFixed(<?php echo $_SESSION['DECIMALESMONEDA'] ?>);
	...
	function guardar_valor_<?php echo $opcGrilla; ?>(contFila,id_anticipo,id_cuenta_anticipo){
		var saldo_anticipo = document.getElementById("saldo_anticipo").value
		...
		if(valor_anticipo > saldo_anticipo){ alert('Aviso\nEl valor del anticipo no puede ser mayor al saldo registrado!'); return; }
