<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory, $link, $id_factura_compra,$id_empresa,$id_usuario,$id_tercero,$tipo_documento_cruce,$prefijo_documento_cruce,$numero_documento_cruce){
            ...
            $sqlInsert = "INSERT INTO compras_facturas_archivos_adjuntos (id_factura_compra,nombre_archivo,ext,fecha_creacion,id_empresa,id_tercero,tipo_documento,prefijo_documento,numero_documento,id_usuario)
                            VALUES ($id_factura_compra,'$filename','$ext',now(),'$id_empresa',$id_tercero,'$tipo_documento_cruce','$prefijo_documento_cruce','$numero_documento_cruce','$id_usuario')";
            if(mysql_query($sqlInsert,$link)){ $idRow = mysql_insert_id($link);
                ...
                if ($this->file->save($uploadDirectory.$filename.'.'.$ext)){
        			...
        			return array('success'=>true, 'idRow'=>$idRow,'filename'=> $filename,'ext'=> $ext,'fecha'=>date('Y-m-d'),'usuario'=>$_SESSION['NOMBREUSUARIO'] );

        			// unlink($uploadDirectory . $filename . '.' . $ext);
                }
                else{ return array('error'=> 'No se puedo guardar el Documento en el servidor','sql'=>$sqlInsert); }
            }
            else{ return array('error'=> 'No se pudo guardar el Documento en la base de datos','sql'=>$sqlInsert); }
        }
    }
    ...
    $ruta2 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/compras/facturas_cuentas';
    $url   = $ruta2.'/';
    if(!file_exists($ruta2)){ mkdir ($ruta2); }

    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($url, $link, $id_factura_compra,$_SESSION['EMPRESA'],$_SESSION['IDUSUARIO'],$id_tercero,$tipo_documento_cruce,$prefijo_documento_cruce,$numero_documento_cruce);
