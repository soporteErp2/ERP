<?php
	...
	if($tipo_documento_cruce == 'CE'){
	 	...
	 	$sql = "SELECT
				...
				ON
					CEC.id_comprobante_egreso = CE.id
				LEFT JOIN
					puc AS P
				ON
					P.id = CEC.id_puc
				...
				AND
					CE.estado = 1
				AND
					CE.id_empresa = $filtro_empresa
				AND
					CE.id_sucursal = $filtro_sucursal
				AND
					CEC.saldo_pendiente > 0
				AND
					CEC.id_documento_cruce = 0
				AND
					CEC.tipo_documento_cruce = ''
				AND
					P.tipo = 'Anticipo a proveedor'";
	 	$query = mysql_query($sql,$link);
	 	while($row = mysql_fetch_array($query)){
			$var .= ($var == '')?' id = '.$row['id'] : ' OR id = '.$row['id'];
	 		$id_cuentas .= ($id_cuentas == '')?' id_puc = '.$row['id_puc'] : ' OR id_puc = '.$row['id_puc'];
	 	}

	 	if($var != ''){
			$where = ' AND ('.$var.') ';
	 		$whereCuentas = ' AND ('.$id_cuentas.') ';
	 	}
	 }
			...
			Win_Ventana_cuentas_documento_cruce = new Ext.Window({
			    ...
			    autoLoad    :
			    {
			        ...
			        params  :
			        {
						...
						whereCuentas		 : '<?php echo $whereCuentas; ?>'
			        }
			    },
			    tbar        :
			    [
