<?php
    ...
    function contabilizar($arrayCuentaPago,$fechaInicioFactura,$consecutivoFactura,$idBodega,$idSucursal,$idEmpresa,$idFactura,$idProveedor,$link){
		...
		foreach ($arrayInventarioFactura AS $valArrayInventario) {
			...
			//**********************************************************************************************************//
				...
				if($arrayItemEstado['debito'] != $arrayItemEstado['credito']){
					echo '<script>
							alert("Aviso.\nEl item Codigo '.$valArrayInventario['codigo'].' No establece doble partida por favor revise la configuracion contable");
							document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
						</script>';
						exit;
				}
		...
		$saldoGlobalFactura = $arrayAsiento[0][$acumCuentaClientes][$acumEstadoCuentaClientes];						//VARIABLE GLOBAL TOTAL FACTURA
		...
		if($totalDebito != $totalCredito){ echo '<script>
		...
		$sqlInsertCuentas = "INSERT INTO asientos_colgaap(
								id_documento,
