<?php
	...
	if ($tipo=='baja_activo_fijo') {
		$tipo2 = 'Baja de Activos Fijos';
	}
	else{
...
if(!isset($opcion)){ ?>
	...
	<script>
		...
		function eliminarArticulo(id){
			var mensaje='';
			if ('<?php echo $tipo; ?>'=='baja_activo_fijo') {
				mensaje ="Aviso!\nSi elimina el activo, este estara nuevamente disponible en el modulo de activos fijos\nContinuar?";
			}
			else{
				mensaje="Aviso!\nRealmente desea eliminar este articulo\nLos articulos ";
				mensaje += ('<?php echo $tipo; ?>'=='entrada')? 'saldran' :'regresaran' ;
				mensaje += " del inventario y se movera el kardex";
			}
	            		...
	            		result.responseText= (result.responseText).replace(/[^a-z]/g,'');
	            	            ...
	            	            if(result.responseText != 'true'){
	            	                alert('No hay conexion con el servidor,\nPor favor intentelo de nuevo si el problema persiste comuniquese con el administrador del sistema');
	            	            		...
	            	            		Elimina_Div_<?php echo $opcGrillaContable; ?>(id);
	            	            	}
