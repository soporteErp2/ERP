<?php
	...
	$sql="SELECT id_tabla_referencia FROM comprobante_egreso_cuentas WHERE activo=1 AND id_comprobante_egreso=$id_comprobante_egreso AND tipo_documento_cruce = '$tipo_documento_cruce'";
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//QUERY
			...
			$grilla->MyWhere			= "activo = 1 AND id_planilla=$id_planilla AND total_sin_abono>0  AND tipo_planilla='$tipo_documento_cruce' AND id_empresa=$id_empresa $whereId";
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			...
			$grilla->CamposBusqueda		= 'cuenta_colgaap,cuenta_niif,tercero,empleado_cruce,debito,credito,total_sin_abono,documento_tercero,documento_empleado_cruce';

			$grilla->AddRow('Cuenta Colgaap','cuenta_colgaap',90);
			$grilla->AddRow('Descripcion','descripcion_cuenta_colgaap',120);
			$grilla->AddRow('Cuenta Niif','cuenta_niif',80);
			$grilla->AddRowImage('Tercero','<label id="tercero_[id]" title="[tercero]">[tercero]</label><input type="hidden" id="id_cuenta_[id]" value="[id_cuenta_colgaap]"><input type="hidden" id="id_tercero_cuenta[id]" value="[id_tercero]">',500);
	...
	<script>
		...
		function Editar_nomina_planillas_empleados_contabilizacion(id){
  	    ...
  	    document.getElementById("divImageSave<?php echo $opcGrillaContable; ?>_"+cont).style.display = 'inline';
			...
			id_tercero = document.getElementById('id_tercero_cuenta'+id).value;

			document.getElementById('tercero<?php echo $opcGrillaContable; ?>_'+cont).value   = document.getElementById('tercero_'+id).innerHTML;
			document.getElementById('tercero<?php echo $opcGrillaContable; ?>_'+cont).title   = document.getElementById('tercero_'+id).innerHTML;
