<?php
	...
	$sql        = "SELECT estado,id_tercero FROM comprobante_egreso WHERE activo = 1 AND id_empresa = $id_empresa AND id = $id_comprobante_egreso";
	...
	$sql = "SELECT id,nombre_archivo,ext,fecha_creacion,usuario
			FROM comprobante_egreso_archivos_adjuntos
			WHERE activo = 1
	...
	while($row = $mysql->fetch_array($query)){
		$img = ($estado == 0)? '<img src="img/delete_file.png" title="Eliminar Archivo" onclick="eliminarArchivoAdjuntoComprobanteEgreso(\''.$row['id'].'\',\''.$row['nombre_archivo'].'.'.$row['ext'].'\')">' : '&nbsp;' ;
		$bodyTable .=  '<tr '.$style.' id="archivo_adjunto_'.$row['id'].'">
							<td>'.$row['nombre_archivo'].'.'.$row['ext'].'</td>
							<td>'.$row['fecha_creacion'].'</td>
							<td>'.$row['usuario'].'</td>
							<td><img src="img/view.png" title="Ver Archivo" onclick="ver_documento_comprobante_egreso(\''.$row['id'].'\',\''.$row['nombre_archivo'].'\',\''.$row['ext'].'\')"></td>
							<td>'.$img.'</td>
						</tr>';
...
<div class="content" style="overflow:hidden;">
	<div class="separator">DOCUMENTOS ADJUNTOS
		<div class="close" onclick="Win_ventanaDocumentosCruceComprobanteEgreso.close();" title="Cerrar"></div>
		...
		<?php
			...
			if($estado == 0){
				...
				<div class="buttom-content">
					<button class="button" data-value="new" onclick="CargarImagenDocumentoTerceroComprobanteEgreso()">Nuevo</button>
				</div>
				<?php
			}
		?>
...
<script>
	function ver_documento_comprobante_egreso(id,nombre,ext){
		if(ext != 'bmp' && ext != 'BMP' && ext != 'jpg' && ext != 'JPG' && ext != 'png' && ext != 'PNG' && ext != 'gif' && ext != 'GIF' && ext != 'pdf' && ext != 'PDF'){
			window.location.href = '../../../../ARCHIVOS_PROPIOS/empresa_<?php echo $_SESSION["ID_HOST"]; ?>/compras/comprobante_egreso/' + nombre + '.' + ext;
		}
		else{
			if(ext == 'pdf'){
				viewDocumentosAdjuntosComprobanteEgreso(id,nombre,ext,Ext.getBody().getWidth()-50,Ext.getBody().getHeight()-50);
				return;
			}
			else{
				Ext.Ajax.request({
					...
					success	: function(response){
								...
								viewDocumentosAdjuntosComprobanteEgreso(id,nombre,ext,ancho,alto);
							  },
					params	: {
	...
	function viewDocumentosAdjuntosComprobanteEgreso(id,nombre,ext,ancho,alto){
	...
	function eliminarArchivoAdjuntoComprobanteEgreso(id,nombre){
	...
	function CargarImagenDocumentoTerceroComprobanteEgreso(id){
		Win_Ventana_Agregar_archivos_adjuntos = new Ext.Window({
			...
			autoLoad	:
			{
				url		: 'comprobante_egreso/upload_documento.php',
				scripts	: true,
				nocache	: true,
				params	:
				{
