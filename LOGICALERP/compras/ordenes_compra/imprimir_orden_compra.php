<?php
	...
	if(file_exists("../../../ARCHIVOS_PROPIOS/empresa_".$_SESSION['ID_HOST']."/panel_de_control/formato_documentos/formato_orden_compra.php")){
		include("../../../ARCHIVOS_PROPIOS/empresa_".$_SESSION['ID_HOST']."/panel_de_control/formato_documentos/formato_orden_compra.php");
	}
	else{
		...
		$consul_moneda    = mysql_query($SQLMoneda,$link);
		...
		mysql_query($sql_orden_compra,$link);
		...
		$nombre_empresa   = mysql_result(mysql_query($sqlNombreEmpresa,$link),0,'nombre');
		...
		$SQLComprasOrdenes   = "SELECT O.*,
								...
								FROM $tablaPrincipal AS O LEFT JOIN terceros AS T ON(
										O.id_proveedor = T.id
									)
								WHERE O.id='$id'
									AND O.activo=1
								...
								LIMIT 0,1";
		$queryComprasOrdenes = mysql_query($SQLComprasOrdenes,$link);
		...
		// SI ES PLATAFORMA, MOSTRAR EN LAS OBSERVACIONES LAS REQUISICIONES CRUZADAS
		if($id_empresa == 1 ||  $id_empresa == 47){
			$sql="SELECT consecutivo_referencia FROM compras_ordenes_inventario
					WHERE activo=1 AND nombre_consecutivo_referencia='Requisicion' AND id_orden_compra=$id GROUP BY consecutivo_referencia";
			$query=$mysql->query($sql,$mysql->link);
			while ($row=$mysql->fetch_array($query)) {
				$acumRQ .= "R $row[consecutivo_referencia]. ";
			}
			$observaciones .= ($acumRQ<>'')? "Requisiciones: $acumRQ " : "";
			// $observaciones .= "Requisiciones: $sql ";
		}
		...
		while ($array = mysql_fetch_array($queryArticulos)) {
			...
			if($opcGrillaContable=='RequisicionCompra' || ($opcGrillaContable=='EntradaAlmacen' && user_permisos(180,'false') == 'false')){
				$filasItems = '<td valign="top" width="137" style="">'.$array["codigo"].'</td>
							   <td valign="top" width="339" align="left">'.$array["nombre"].'</td>
							   <td valign="top" width="137" align="left">'.$array["nombre_unidad_medida"].' x '.$array["cantidad_unidad_medida"].'</td>
							   <td valign="top" width="127" align="right" style="padding-right:5px;">'.($array["cantidad_total"] * 1).'</td>';
			}
		...
		if($opcGrillaContable=='RequisicionCompra'){
			$contenido = '';
		}
	  ...
	  $numeros_en_letras = utf8_decode(num2letras(round($totalOC_tc,$decimales)));
		...
		if($opcGrillaContable=='RequisicionCompra' || ($opcGrillaContable=='EntradaAlmacen' && user_permisos(180,'false') == 'false')){
			$cabeceraItems = '<td style="width:137px;">CODIGO</td>
							  <td style="width:339px;">ARTICULO</td>
							  <td style="width:137px;">UNIDAD</td>
							  <td style="width:127px;">CANTIDAD</td>';
		}
