<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if ($opcGrillaContable=='FacturaVenta') {
			$tablaValidar='ventas_facturas_inventario';
			$idTablaValidar='id_factura_venta';
		}
		else if ($opcGrillaContable=='RemisionesVenta') {
			$tablaValidar='ventas_remisiones_inventario';
			$idTablaValidar='id_remision_venta';
		}
		else if ($opcGrillaContable=='PedidoVenta') {
			$tablaValidar='ventas_pedidos_inventario';
			$idTablaValidar='id_pedido_venta';
		}
		...
		while ($row=mysql_fetch_array($query)) {
			// $whereId.=($whereId=='')? 'id<>'.$row['id_consecutivo_referencia'] : ' AND id<>'.$row['id_consecutivo_referencia'] ;
			if ($row['nombre_consecutivo_referencia']=='Cotizacion') {
				$whereIdCotizacion.=($whereIdCotizacion=='')? ' AND id<>'.$row['id_consecutivo_referencia'] : ' AND id<>'.$row['id_consecutivo_referencia'] ;
			}
			else if ($row['nombre_consecutivo_referencia']=='Pedido') {
				$whereIdPedido.=($whereIdPedido=='')? ' AND id<>'.$row['id_consecutivo_referencia'] : ' AND id<>'.$row['id_consecutivo_referencia'] ;
			}
			else if ($row['nombre_consecutivo_referencia']=='Remision') {
				$whereIdRemision.=($whereIdRemision=='')? ' AND id<>'.$row['id_consecutivo_referencia'] : ' AND id<>'.$row['id_consecutivo_referencia'] ;
			}
		}
		...
		if ($tablaCotizacionPedido=='ventas_cotizaciones') {
		...
		else if($tablaCotizacionPedido=='ventas_remisiones'){
		...
		//TOOLBAR Y CAMPO DE BUSQUEDA Y FILTROS
			...
			$grilla->CamposBusqueda		= 'fecha_inicio,consecutivo,nit,cliente,bodega';		//VARIABLE QUE DEFINE LOS CAMPOS DE LA BD DONDE SE BUSCARA
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			...
			if($tablaCotizacionPedido=='ventas_pedidos'){ $grilla->AddRow('Unidades Pendientes','unidades_pendientes',120); }
			else if($tablaCotizacionPedido=='ventas_remisiones'){
...
if(!isset($opcion)){  ?>
	<script>
		...
		function Editar_<?php echo $nameGrillaLoad; ?>(id){
			...
			ajaxBuscarCotizacionPedido<?php echo $opcGrillaContable; ?>(consecutivo,'false');
