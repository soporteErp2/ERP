	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//CAMPOS OCULTOS
 		...
 		//CONFIGURACION DE LOS FORMULARIOS DE CAPTURA Y EDICION
			...
			$grilla->AddSeparator('Informacion Fiscal');
			...
			$grilla->AddTextField('Numero Identificacion','numero_identificacion',150,'true','false','false','true');
            ...
            $grilla->AddTextField('NumRegIdTrub','NumRegIdTrub',200,'false','false');
            $grilla->AddTextField('Residencia Fiscal','residencial_fiscal',200,'false','false');

		///DIRECCION PRINCIPAL
			$grilla->AddSeparator('Direccion Principal');
			$grilla->AddTextField('Direccion','direccion',200,'false','false');
			$grilla->AddComboBox ('Pais','id_pais',200,'false','true','ubicacion_pais,id,pais,true','activo = 1 ORDER BY pais ASC');
			$grilla->AddComboBox ("$labelDepto",'id_departamento',200,'false','true','ubicacion_departamento,id,departamento,true','activo = 1 AND id_pais=49 ORDER BY departamento ASC');
			$grilla->AddComboBox ("$labelCiudad",'id_ciudad',200,'false','true','ubicacion_ciudad,id,ciudad,true','activo = 1 ORDER BY ciudad ASC');
			$grilla->AddComboBox ("$labelMunicipio",'id_comuna',200,'false','true','ubicacion_comuna,id,comuna,true','activo = 1 ORDER BY ciudad ASC');
			$grilla->AddTextField('Codigo Postal','codigo_postal',200,'false','false');
			$grilla->AddSeparator('Contactos');
		...
		///DATOS TRIBUTARIOS
			...
			$grilla->AddTextField('Primer Nombre','nombre1',200,'false','false');
			$grilla->AddTextField('Segundo Nombre','nombre2',200,'false','false');
			$grilla->AddTextField('Primer Apellido','apellido1',200,'false','false');
			...
			$grilla->AddComboBox ('Tipo Regimen','id_tercero_tributario',200,'false','true','terceros_tributario,id,nombre,true',"activo = 1 AND id_pais='$id_pais'");
			$grilla->AddComboBox ('Tipo Persona','id_tipo_persona_dian',200,'false','false','1:PERSONA JURIDICA,2:PERSONA NATURAL');
			$grilla->AddComboBox ('Exento de iva (venta)','exento_iva',200,'false','false','Si:Si,No:No');
			$grilla->AddComboBox ('Forma de pago(por defecto)','id_forma_pago',200,'false','true','configuracion_formas_pago,id,nombre,true',"activo = 2 AND id_empresa='$id_empresa'");
			$grilla->AddComboBox ('Metodo de pago(por defecto)','id_metodo_pago',200,'false','true','configuracion_metodos_pago,id,nombre,true',"activo = 2 AND id_empresa='$id_empresa'");
...
if($opcion == 'Vupdate' || $opcion == 'Vagregar'){ ?>
	...
	<script>
		...
		var ComboRegion = Ext.get('Terceros_id_departamento');

		ComboRegion.addListener(
			'change',
			function(event,element,options){
		...
		var ComboProvincia = Ext.get('Terceros_id_ciudad');

		ComboProvincia.addListener(
			'change',
			function(event,element,options){
				id_provincia = document.getElementById('Terceros_id_ciudad').value;
				ActualizaComunaTerceros(id_provincia);
			},
			this
		);
		...
		function ActualizaDepartamento(id_pais){
			...
			Ext.get(MyParentDepartamento).load({
				url		: '../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/bd/bd.php',
				timeout : 180000,
				scripts	: true,
				nocache	: true,
				params	:
		...
		?>
		// CONSULTAR LA PROVINCIA DE LA REGION
		...
		// CONSULTAR LA COMUNA DE LA PROVINCIA
		function ActualizaComunaTerceros(id_ciudad){
			var MyParentCiudadTerceros = document.getElementById('Terceros_id_comuna').parentNode;
			id_ciudad = document.getElementById('Terceros_id_ciudad').value;
	...
	<script>
		...
		function VentanaSubirExcelCliente(){
			...
			Win_Ventana_subir_excel_terceros = new Ext.Window({
			    ...
			    autoLoad    :
			    {
			        url     : '../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/subir_terceros.php',
			        scripts : true,
			        nocache : true,
			        params  : { }
			    },
		...
		function exportarTerceros() {
			window.open("../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/exportarTerceros.php");
		}
			...
			if(cual == 'false'){
				...
				Win_Agregar_Terceros = new Ext.Window({
					...
					autoLoad	:
					{
	                    ...
	                    url     : '../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/contenedor_terceros.php',
						scripts	: true,
						nocache	: true,
						params	:
						{
		...
		function ventanaFichaTecnica(id){
		    ...
		    Win_Formulario_FichaProveedor = new Ext.Window({
		        ...
		        autoLoad    :
		        {
		            url     : '../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/ficha_tecnica_proveedor.php',
		            scripts : true,
		            nocache : true,
		            params  : { id_tercero : id }
		        },
		...
		function VentanaCambiaFuncionario2(id){
				Win_Cambia_Funcionario = new Ext.Window({
					...
					autoLoad	:
					{
						url		: '../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/CambiaFuncionarioAsignado.php',
						scripts	: true,
						nocache	: true,
						params	:
						{
		...
		function VentanaEnviarEmail(id){
            ...
            ventana_email = new Ext.Window({
                ...
                autoLoad    :
                {
                    url     : "../erp_paises_global/<?= $_SESSION['PAIS'] ?>/terceros/terceros/envio_mail/mail_terceros.php",
                    scripts : true,
                    nocache : true,
                    params  :
                    {
