...
<?php
    ...
    if(empty($arrayResoluciones)){
      ...
      $sql = "SELECT id,prefijo,consecutivo_resolucion,fecha_resolucion,numero_inicial_resolucion,numero_final_resolucion
              FROM ventas_facturas_configuracion WHERE activo = 1 AND id_empresa = $id_empresa AND consecutivo_factura <= numero_final_resolucion AND($whereResoluciones)";
      $query = $mysql->query($sql,$mysql->link);
      while($row = $mysql->fetch_array($query)){
