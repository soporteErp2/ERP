<?php
	...
	switch ($opc){
		...
		//============================ GRUPOS DE ITEMS ===========================//
		case 'saveGroup':
			saveGroup($codigo,$nombre,$cantidad,$observaciones,$id_bodega,$id_documento,$id_impuesto,$nombre_impuesto,$valor_impuesto,$codigo_dian,$opcGrillaContable,$id_sucursal,$id_empresa,$mysql);
			break;
		case 'updateGroup':
			updateGroup($codigo,$nombre,$cantidad,$observaciones,$id_bodega,$id_documento,$opcGrillaContable,$id_row,$id_sucursal,$id_empresa,$mysql);
			break;
		case 'deleteGrupo':
	...
	function terminarGenerar($id,$id_empresa,$id_sucursal,$idBodega,$observacion,$tablaPrincipal,$idTablaPrincipal,$tablaInventario,$tablaRetenciones,$opcGrillaContable,$id_empresa,$idPlantilla,$fechaFactura,$link){
		...
		if($arrayAnticipo['total'] > 0){
			...
			if(!$queryValidaAnticipos || $conFailAnticipo>0){ echo '<script>alert("Aviso,\nHay anticipos que superan el valor registrado, por favor realice las correcciones respectivas.")</script>'; exit; }
	...
	function agregarDocumento($typeDoc,$codDocAgregar,$id_factura,$filtro_bodega,$id_sucursal,$id_empresa,$opcGrillaContable,$tablaInventario,$idTablaPrincipal,$link){
		...
		$idClienteDocAgregar    = '';
		$estadoDocAgregar       = '';
		$sqlValidateDocumento   = "SELECT id_cliente,estado,id,observacion $campoSelect FROM $tablaCarga WHERE consecutivo='$codDocAgregar' AND id_bodega='$filtro_bodega' AND id_empresa='$id_empresa' AND id_sucursal='$id_sucursal' AND activo = 1";
	...
	function guardarAnticipo($contFila,$idFactura,$valor_anticipo,$id_cuenta_anticipo,$id_anticipo,$opcGrilla,$id_empresa,$link){
		...
		//DATOS DE FACTURA
		$sqlfactura   = "SELECT id_cliente,cliente,nit FROM ventas_facturas WHERE id='$idFactura' LIMIT 0,1";
		$queryFactura = mysql_query($sqlfactura,$link);
	...
	function saveGroup($codigo,$nombre,$cantidad,$observaciones,$id_bodega,$id_documento,$id_impuesto,$nombre_impuesto,$valor_impuesto,$codigo_dian,$opcGrillaContable,$id_sucursal,$id_empresa,$mysql){
		$sql="INSERT INTO
	...
	function updateGroup($codigo,$nombre,$cantidad,$observaciones,$id_bodega,$id_documento,$opcGrillaContable,$id_row,$id_sucursal,$id_empresa,$mysql){
		$sql="UPDATE ventas_facturas_grupos SET codigo='$codigo',nombre='$nombre',cantidad='$cantidad',observaciones='$observaciones'
