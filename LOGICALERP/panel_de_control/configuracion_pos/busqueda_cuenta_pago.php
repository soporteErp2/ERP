<?php
	...
	$tablaAsientos = ($typeCuenta == 'niif')? 'asientos_niif': 'asientos_colgaap';
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		//NOMBRE DE LA GRILLA
			$grilla->GrillaName	 		= 'configuracion_cuentas_pago';  	//NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
		//QUERY
			$grilla->TableName			= 'configuracion_cuentas_pago';			//NOMBRE DE LA TABLA EN LA BASE DE DATOS
			$grilla->MyWhere			= "activo = 1 AND id_empresa='$id_empresa' AND tipo='Pos' ";		//WHERE DE LA CONSULTA A LA TABLA "$TableName"
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			...
			$grilla->AddRow('Descripcion','nombre',220);
			// $grilla->AddRow('Tipo','tipo',120);
			$grilla->AddRow('Cuenta','cuenta',100);
			$grilla->AddRow('Cuenta Niif','cuenta_niif',100);
			$grilla->AddRow('Estado','estado',90);
...
if(!isset($opcion)){  ?>
	<script>
		function Editar_configuracion_cuentas_pago(id){
			var descripcion = document.getElementById(`div_configuracion_cuentas_pago_nombre_${id}`).innerHTML
			,	cuenta      = document.getElementById(`div_configuracion_cuentas_pago_cuenta_${id}`).innerHTML
			,	cuenta_niif = document.getElementById(`div_configuracion_cuentas_pago_cuenta_niif_${id}`).innerHTML

			document.getElementById('configuracion_pos_id_configuracion_cuenta_cobro').value = id;
			document.getElementById('configuracion_pos_cuenta_por_cobrar_colgaap').value     = cuenta;
			document.getElementById('configuracion_pos_cuenta_por_cobrar_niif').value        = cuenta_niif;
			document.getElementById('configuracion_pos_descripcion_cuenta_cobro').value      = descripcion;
			Win_VentanaBuscarCuentasPago.close();
