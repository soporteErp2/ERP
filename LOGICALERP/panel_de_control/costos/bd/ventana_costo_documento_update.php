<?php
		...
		$SQL     = "SELECT *, if(id_centro_costos>0,CONCAT(codigo_centro_costo,' - ',centro_costos),'') AS acum_centro_costos FROM costo_documento_porcentaje WHERE activo=1 AND id_empresa = '$id_empresa' AND id_costo_documento = '$id'";
		$consul  = mysql_query($SQL,$link);

		$SQL2    = "SELECT nombre FROM costo_documento WHERE activo=1 AND id_empresa = '$id_empresa' AND id = '$id'";
		$consul2 = mysql_query($SQL2,$link);
		$nombre  = mysql_result($consul2 ,0,'nombre');
...
?>
		...
		<div id="contenedor_costos_documento" style="width:20px; height:20px; position:fixed; margin-left:10px;"></div>
 		<div style="float:left;margin: 20px 0 0 10px">
			    ...
			    <div id="recibidor_orden_compra_<?php echo $opcGrilla; ?>" style="float:left; height:23px;">
				    <input id="nombre_costo_documento" class="myfield" style="width:200px;height:19px;vertical-align:middle" value="<?php echo $nombre; ?>"/>
...
?>
			<div style="float:left;margin: 5px 0 0 10px;width:90%">
			    <div style="width:100px; padding:3px 0 0 0;font-size:12;font-weight:bold"><?php echo $row['costo_tipo']; ?></div>
			</div>

            <div style="float:left;margin: 7px 0 0 10px;width:90%">
			    <div style="float:left; width:100px; padding:3px 0 0 0">Valor:</div>
			    <div id="recibidor_orden_compra_<?php echo $opcGrilla; ?>" style="float:left; width:160px;height:23px;">
				    <input id="valor_<?php echo $row['id']; ?>" class="myfield" style="width:200px;height:19px;vertical-align:middle" value="<?php echo $row['valor']; ?>"/>


				</div>
		    </div>

		    <input type="hidden" id="<?php echo $row['id']; ?>_id_centro_costos" style="width:150px;height:23px;vertical-align:middle" value="<?php echo $row['id_centro_costos']; ?>"/>
		    <input type="hidden" id="<?php echo $row['id']; ?>_id_cuenta_colgaap" style="width:150px;height:23px;vertical-align:middle" value="<?php echo $row['id_cuenta_colgaap']; ?>"/>
		    <input type="hidden" id="<?php echo $row['id']; ?>_id_cuenta_niif" style="width:150px;height:23px;vertical-align:middle" value="<?php echo $row['id_cuenta_niif']; ?>"/>
		    ...
		    <div style="float:left;margin: 7px 0 0 10px;width:90%" id="DIV_centro_costo_<?php echo $row['id']; ?>">
			    ...
			    <div id="recibidor_orden_compra_<?php echo $opcGrilla; ?>" style="float:left;height:23px;">
				    <input id="<?php echo $row['id']; ?>_centro_costos" class="myfield" style="width:178px;height:19px;vertical-align:middle;" value="<?php echo $row['acum_centro_costos']; ?>"/>
			...
			<div style="float:left;margin: 7px 0 0px 10px;width:90%" id="DIV_cuenta_colgaap<?php echo $row['id']; ?>">
			    ...
			    <div id="recibidor_orden_compra_<?php echo $opcGrilla; ?>" style="float:left; height:23px;">
				    <input id="cuenta_colgaap<?php echo $row['id']; ?>" class="myfield" style="width:120px;height:23px;vertical-align:middle" value="<?php echo $row['cuenta_colgaap']; ?>"/>
			...
			<div style="float:left;margin: 7px 0 15px 10px;width:90%" id="DIV_cuenta_niif<?php echo $row['id']; ?>">
			    ...
			    <div id="recibidor_orden_compra_<?php echo $opcGrilla; ?>" style="float:left; height:23px;">
				    <input id="cuenta_niif<?php echo $row['id']; ?>" class="myfield" style="width:120px;height:23px;vertical-align:middle" value="<?php echo $row['cuenta_niif']; ?>"/>
