<?php
	...
	$sqlCuentas   = "UPDATE items_cuentas_niif SET cuenta='' WHERE activo=1 AND id_empresa='$id_empresa' AND id_puc='' AND puc>0 AND id_items='$idItems'";
	...
	while ($rowCuentasItems = mysql_fetch_array($queryCuentasItems)) {
		...
		$acumArrayJavaScript .= 'arrayIdItemsCuentaNiif["'.$texto.'"]="'.$rowCuentasItems['id'].'";';
	}

	$sqlItems   = "SELECT inventariable, id_impuesto,opcion_gasto,opcion_activo_fijo,opcion_costo,estado_compra,estado_venta FROM items WHERE id='$idItems' AND activo=1 LIMIT 0,1";
	...
	// $opc_compra      = mysql_result($queryItems,0,'opcional_compra');
	...
	for($i=1; $i<=2; $i++){
		...
		if($estado == 'compra' && $estadoCompra != 'true'){ continue; }
		else if($estado == 'venta' && $estadoVenta != 'true'){ continue; }
		...
		$divItemsCuenta .= 		'<div class="filaCuentasItems" id="contenedor_'.$estado.'_precio">
									...
									<div style="float:left; width:90px; margin-left:5px;">
										<input type="text" class="myfield" id="'.$estado.'_precio_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_precio']['puc'].'"/>
									</div>
									...
									<div class="cuentaPuc" id="cuenta_'.$estado.'_precio_niif">'.$arraySaveCampos[$estado.'_precio']['cuenta'].'</div>
								</div>';
		...
		if ($id_impuesto > 0 && 1==2){
			$divItemsCuenta .=	'<div class="filaCuentasItems" id="contenedor_'.$estado.'_impuesto">
									...
									<div style="float:left; width:90px; margin-left:5px;">
										<input type="text" class="myfield" id="'.$estado.'_impuesto_niif" readonly value="'.$arraySaveCampos[$estado.'_impuesto']['puc'].'" style="cursor:default;"/>
									</div>
									<div style="float:left; width:18px; overflow:hidden; margin-left:-18px;" id="render_'.$estado.'_impuesto"></div>
									'.$arraySaveCampos[$estado.'_impuesto']['btns'].'
									<div class="cuentaPuc" id="cuenta_'.$estado.'_impuesto_niif">'.$arraySaveCampos[$estado.'_impuesto']['cuenta'].'</div>
								</div>';
		}
		...
		// 							<div style="float:left; width:90px; margin-left:5px;">
		// 								<input type="text" class="myfield" id="'.$estado.'_contraPartida_precio_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_contraPartida_precio']['puc'].'"/>
		// 							</div>
		...
		// 							<div class="cuentaPuc" id="cuenta_'.$estado.'_contraPartida_precio_niif">'.$arraySaveCampos[$estado.'_contraPartida_precio']['cuenta'].'</div>
		// 						</div>';
		...
		if($estado == 'compra' ){
			if ($opcion_activo_fijo == 'true') {
				...
				$divItemsCuenta .= 	'<div class="filaCuentasItems" id="contenedor_'.$estado.'_activo_fijo">
										...
										<div style="float:left; width:90px; margin-left:5px;">
											<input type="text" class="myfield" id="'.$estado.'_activo_fijo_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_activo_fijo']['puc'].'"/>
										</div>
										...
										<div class="cuentaPuc" id="cuenta_'.$estado.'_activo_fijo_niif">'.$arraySaveCampos[$estado.'_activo_fijo']['cuenta'].'</div>
									</div>';
			}
			if ($opcion_gasto == 'true') {
				...
				$divItemsCuenta .= 	'<div class="filaCuentasItems" id="contenedor_'.$estado.'_gasto">
										<div style="float:left; width:145px;">'.$texto.'</div>
										<div style="float:left; width:90px; margin-left:5px;">
											<input type="text" class="myfield" id="'.$estado.'_gasto_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_gasto']['puc'].'"/>
										</div>
										...
										<div class="cuentaPuc" id="cuenta_'.$estado.'_gasto_niif">'.$arraySaveCampos[$estado.'_gasto']['cuenta'].'</div>
									</div>';
			}
			if($opcion_costo=='true'){
				...
				$divItemsCuenta .= 	'<div class="filaCuentasItems" id="contenedor_'.$estado.'_costo">
										<div style="float:left; width:145px;">'.$texto.'</div>
										<div style="float:left; width:90px; margin-left:5px;">
											<input type="text" class="myfield" id="'.$estado.'_costo_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_costo']['puc'].'"/>
										</div>
										...
										<div class="cuentaPuc" id="cuenta_'.$estado.'_costo_niif">'.$arraySaveCampos[$estado.'_costo']['cuenta'].'</div>
									</div>';
			}
		}
		...
		// 							<div style="float:left; width:90px; margin-left:5px;">
		// 								<input type="text" class="myfield" id="'.$estado.'_prestamo_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_prestamo']['puc'].'"/>
		// 							</div>
		...
		if ($inventariable=='true' && $estado == 'venta'){
			$divItemsCuenta .= 	'<div class="filaCuentasItems" id="contenedor_'.$estado.'_costo">
								...
								<div class="filaCuentasItems" id="contenedor_'.$estado.'_contraPartida_costo">
									...
									<div style="float:left; width:90px; margin-left:5px;">
										<input type="text" class="myfield" id="'.$estado.'_contraPartida_costo_niif" onclick="buscarPucNiif(this)" readonly value="'.$arraySaveCampos[$estado.'_contraPartida_costo']['puc'].'"/>
									</div>
									...
									<div class="cuentaPuc" id="cuenta_'.$estado.'_contraPartida_costo_niif">'.$arraySaveCampos[$estado.'_contraPartida_costo']['cuenta'].'</div>
								</div>';
		}
...
<script>
	...
	var arrayIdItemsCuentaNiif = new Array();
	<?php echo $acumArrayJavaScript; ?>

	function buscarPucNiif(inputCuenta,idItemsCuenta){
		var idInput  = (inputCuenta.id).replace(/_niif/g, '')
		,	type_puc = 'niif';
		...
		Win_VentanaBuscarPucItems = new Ext.Window({
            ...
            autoLoad    :
            {
                ...
                scripts : true,
                nocache : true,
                params  : { ejecutaFuncion : 'responseVentanaPucItemsNiif(id,"'+idInput+'","'+idItemsCuenta+'")', type_puc : type_puc }
            },
            tbar        :
            [
                {
	...
	function responseVentanaPucItemsNiif(idPuc,idInput,idItemsCuenta){
		...
		Ext.get('render_'+idInput).load({
		    ...
		    params	:
		    {
				op           : 'saveCuenta',
				typeResponse : 'niif',
				// idItemCuenta : arrayIdItemsCuentaNiif[idInput],
				idItemCuenta : idPuc,
				puc          : puc,
				nombre       : nombre,
				estado       : estado,
