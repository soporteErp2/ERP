<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory,$mysql){
            ...
            if ($this->file->save($uploadDirectory.$filename.'.'.$ext)){
                ...
                if (!empty($arrayError)) {
                    $msjError =($debug=="items")? "El archivo de Excel contiene errores de validacion\nCorrijalos e intentelo de nuevo" : "Se genero un error en el sistema, si el problema continua comuniquese con el administrador de sistema" ;
                    return array('error'=> $msjError, 'debug'=> $debug, 'detalle'=>$arrayError);
                }

                return array('success'=>true, 'debug'=>$sql);
            }
    ...
    // $url  = $serv.'ARCHIVOS_PROPIOS/documentos_contabilidad/nota/';
    // if(!file_exists($url)){ mkdir ($url); }

    // $url = $url.'empresa_'.$id_host.'/';
    // if(!file_exists($url)){ mkdir ($url); }
    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($url,$mysql);
