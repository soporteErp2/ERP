<?php
    ...
    header("Content-Disposition: attachment; filename=puc_niif_".date("Y-m-d").".xls");
    header("Pragma: no-cache");
    header("Expires: 0");

    // CONSULTAR LAS CUENTAS CONTABLES
	$sql="SELECT cuenta,descripcion,departamento,ciudad,centro_costo,tipo FROM puc_niif WHERE activo=1 AND id_empresa=$_SESSION[EMPRESA] ORDER BY CAST(cuenta AS CHAR) ASC ";
