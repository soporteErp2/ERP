<?php
	...
	// $opcGrillaContable = 'configuracionInformeNiif';
	...
	$sql   = "SELECT id_puc_niif,cuenta_niif,descripcion_cuenta_niif,clasificacion FROM configuracion_informe_estado_resultado_niif WHERE activo=1 AND id_empresa=$id_empresa  ";
	$query = mysql_query($sql,$link);
	while ($row=mysql_fetch_array($query)) {
		...
		if ($row['clasificacion']=='ingresos') { $filasingresos.=$filas_configuracion; }
		else if ($row['clasificacion']=='costos') { $filascostos.=$filas_configuracion; }
		else if ($row['clasificacion']=='impuestos') { $filasimpuestos.=$filas_configuracion; }
		else if ($row['clasificacion']=='gastos') { $filasgastos.=$filas_configuracion; }
	}
...
<style>
	...
	#configEstadoResultado .campo2{
		width        : calc(100% - 120px);
		border-right : none;
	}

	#configEstadoResultado .over {
...
<div class="contenedorPrincipal" id="configEstadoResultado">
	...
	<div id="contenedor_grilla_manual" style="float:left; height: calc(100% - 140px); width:47%;">
		...
		<div id="contenedor_tabla_boletas" style="height: calc(100% - 36px);">
			...
			<div id="bodyTablaBoletas" style="height: calc(100% - 55px);" ondrop="drop_grilla_manual<?php echo $opcGrillaContable; ?>(event)" ondragover="allowDrop<?php echo $opcGrillaContable; ?>(event)"><?php echo $filaInsertBoleta; ?></div>
	...
	<div class="contenedor_configuracion_informe" id="divPadreConfiguracion">
		<div class="titulos_contenedor_configuracion">INGRESOS</div>
		<div  id="ingresos" ondrop="drop<?php echo $opcGrillaContable; ?>(event,this); handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)" ondragover="allowDrop<?php echo $opcGrillaContable; ?>(event); handleDragOver_<?php echo $opcGrillaContable; ?>(event,this)" ondragleave="handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)">
			<?php echo $filasingresos; ?>
		</div>
		<div class="titulos_contenedor_configuracion">COSTOS</div>
		<div  id="costos" ondrop="drop<?php echo $opcGrillaContable; ?>(event,this); handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)" ondragover="allowDrop<?php echo $opcGrillaContable; ?>(event); handleDragOver_<?php echo $opcGrillaContable; ?>(event,this)" ondragleave="handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)">
			<?php echo $filascostos; ?>
		</div>
		<div class="titulos_contenedor_configuracion">IMPUESTOS</div>
		<div  id="impuestos" ondrop="drop<?php echo $opcGrillaContable; ?>(event,this); handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)" ondragover="allowDrop<?php echo $opcGrillaContable; ?>(event); handleDragOver_<?php echo $opcGrillaContable; ?>(event,this)" ondragleave="handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)">
			<?php echo $filasimpuestos; ?>
		</div>
		<div class="titulos_contenedor_configuracion">GASTOS</div>
		<div id="gastos" ondrop="drop<?php echo $opcGrillaContable; ?>(event,this); handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)" ondragover="allowDrop<?php echo $opcGrillaContable; ?>(event); handleDragOver_<?php echo $opcGrillaContable; ?>(event,this)" ondragleave="handleDragLeave_<?php echo $opcGrillaContable; ?>(event,this)">
			<?php echo $filasgastos; ?>
		</div>
	</div>
</div>
...
<script>
	...
	function drop<?php echo $opcGrillaContable; ?>(ev,div){
		...
		else if(ev.target.id=='ingresos' || ev.target.id=='costos' || ev.target.id=='gastos' || ev.target.id=='impuestos'){
	...
	function drop_grilla_manual<?php echo $opcGrillaContable; ?>(ev) {
	    ...
	    if (fromElement=='bodyTablaBoletas') { return; }

	    if(campo=='fila'){
	    	ev.target.parentNode.appendChild(document.getElementById(data));
	    }
	    else if (ev.target.id!='bodyTablaBoletas') {
	    	ev.target.parentNode.parentNode.appendChild(document.getElementById(data));
	    }
	    else {
		...
		ajaxConfiguracion<?php echo $opcGrillaContable; ?>(fromElement,'bodyTablaBoletas',id_puc_niif);
		//LIMPIAR EL ELEMENTO DE INICIO
		fromElement='';
	}
	...
	function ajaxConfiguracion<?php echo $opcGrillaContable; ?>(elementInicio,elementFin,id_puc_niif){
		...
		if (elementInicio=='bodyTablaBoletas') { opc_registro='insert'; }
		else if (elementFin=='bodyTablaBoletas') { opc_registro='delete'; }
		else {opc_registro='update'; }

		// var opc_registro=(elementInicio=='bodyTablaBoletas')? 'insert': 'update' ;
