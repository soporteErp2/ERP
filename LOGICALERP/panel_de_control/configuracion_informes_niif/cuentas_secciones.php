<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//NOMBRE DE LA GRILLA
			$grilla->GrillaName	 		= 'cuentas_seccion';  	//NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
		//QUERY
			$grilla->TableName			= 'informes_niif_formatos_secciones_cuentas';		//NOMBRE DE LA TABLA EN LA BASE DE DATOS
			$grilla->MyWhere			= "activo = 1 AND id_empresa='$id_empresa' AND id_formato=$id_formato AND id_seccion=$codigo_seccion ";		//WHERE DE LA CONSULTA A LA TABLA "$TableName"
			$grilla->OrderBy			= '';
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			...
			$grilla->AddRow('Cuenta Inicial','descripcion_cuenta_inicial',170);
			$grilla->AddRow('Cuenta Final','cuenta_final',80);
			$grilla->AddRow('Cuenta Inicial','descripcion_cuenta_final',170);
		...
		//CONFIGURACION DE LA VENTANA DE INSERT, UPDATE Y DELETE
			...
			$grilla->TituloVentana		= 'Ventana cuentas_seccion formato'; //NOMBRE DE LA VENTANA DE INSER, UPDATE Y DELETE
			...
			$grilla->AddBotton('Regresar','regresar','Win_Ventana_cuentas_seccion.close();');
		...
		//CONFIGURACION DE LOS FORMULARIOS DE CAPTURA Y EDICION
			...
			$grilla->AddComboBox('Calcular por','forma_calculo',150,'true','false','suma_debitos:Suma Debitos,suma_creditos:Suma Creditos,debito_menos_credito:Debito - Credito,saldo_actual:Saldo Actual,saldo_inicial:Saldo Inicial');
			$grilla->AddTextField('','id_empresa',200,'true','true',$id_empresa);
			$grilla->AddTextField('','id_formato',200,'true','true',$id_formato);
			$grilla->AddTextField('','id_seccion',200,'true','true',$codigo_seccion);
...
?>
	<script>
		// CUENTA INICIAL
		var inputCuentaInicial      = document.getElementById('cuentas_seccion_cuenta_inicial');
		var inputDescCuentaInicial      = document.getElementById('cuentas_seccion_descripcion_cuenta_inicial');
		...
		divBtnCuentaInicial.setAttribute("onclick","ventanaBuscarPucCuentasPago('colgaap','cuentas_seccion_id_cuenta_inicial','cuentas_seccion_cuenta_inicial','cuentas_seccion_descripcion_cuenta_inicial')");
		divBtnCuentaInicial.setAttribute('title','Buscar Cuenta Colgaap');
		divBtnCuentaInicial.innerHTML = '<img src="img/buscar20.png" />';
		document.getElementById("DIV_cuentas_seccion_cuenta_inicial").appendChild(divBtnCuentaInicial);

		// CUENTA FINAL
		var inputCuentaFinal      = document.getElementById('cuentas_seccion_cuenta_final');
		var inputDescCuentaFinal      = document.getElementById('cuentas_seccion_descripcion_cuenta_final');
		...
		var divBtnCuentaInicial = document.createElement("div");
		divBtnCuentaInicial.setAttribute("class","divBtnBuscarPuc");
		divBtnCuentaInicial.setAttribute("onclick","ventanaBuscarPucCuentasPago('colgaap','cuentas_seccion_id_cuenta_final','cuentas_seccion_cuenta_final','cuentas_seccion_descripcion_cuenta_final')");
		divBtnCuentaInicial.setAttribute('title','Buscar Cuenta Colgaap');
		divBtnCuentaInicial.innerHTML = '<img src="img/buscar20.png" />';
		document.getElementById("DIV_cuentas_seccion_cuenta_final").appendChild(divBtnCuentaInicial);
