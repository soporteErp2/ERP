<?php
	...
	if(isset($responseWs['identificacion'])){ $responseWs = array(0 => $responseWs); }
	else if(!isset($responseWs[0]['identificacion'])){ response_error(array("estado"=>"error", "msj"=>"No hay terceros por sincronizar para el dia ".fecha_larga($fecha_metodo)),true); return; }

	foreach ($responseWs as $i => $arrayWsTercero) {

		list($nit,$dv) = explode('-', $arrayWsTercero['identificacion']);
		$nit = str_replace('.', '', $nit);

		$arrayWs['tercero']['nombre']                       = $arrayWsTercero['nombre'];
		$arrayWs['tercero']['nombre_comercial']             = $arrayWsTercero['nombre'];
		$arrayWs['tercero']['codigo_tipo_identificacion']   = $arrayWsTercero['tipo_id'];
		$arrayWs['tercero']['numero_identificacion']        = $nit;
		$arrayWs['tercero']['dv']                           = $dv;
		$arrayWs['tercero']['codigo_ciudad_identificacion'] = $arrayWsTercero['ciudad'];
		$arrayWs['tercero']['codigo_ciudad']                = $arrayWsTercero['ciudad'];
		$arrayWs['tercero']['codigo_regimen']               = $arrayWsTercero['regimen'];
		$arrayWs['tercero']['direccion']                    = $arrayWsTercero['direccion'];
		$arrayWs['tercero']['cliente']                      = 'si';
		$arrayWs['tercero']['proveedor']                    = 'si';
		$arrayWs['tercero']['telefono1']                    = $arrayWsTercero['telefono'];
		$arrayWs['tercero']['email']                        = $arrayWsTercero['correo'];

		$response = insertUpdateTerceroEmpresa($arrayWs,$id_empresa,$link);
	}


	//FUNCION POR CADA UNA DE LAS EMPRESAS
	function insertUpdateTerceroEmpresa($arrayWs,$idEmpresaErp,$link){
		...
		//================================// UBICACION //================================//
		//*******************************************************************************//
		$arrayUbicacion = ubicacion($infoTercero['codigo_ciudad']);
		if($arrayUbicacion["estado"] == "true"){
			$infoTercero['id_pais']         = $arrayUbicacion['id_pais'];
			$infoTercero['id_departamento'] = $arrayUbicacion['id_departamento'];
			$infoTercero['id_ciudad']       = $arrayUbicacion['id_ciudad'];
		}
		else{ response_error(array("estado"=>"error", "msj"=>"El Codigo #".$infoTercero['codigo_ciudad']." de ubicacion no valido Tercero ".$infoTercero['numero_identificacion']." ".$infoTercero['nombre']),false); return; }

		//===========================// UBICACION DOCUMENTO //===========================//
		//*******************************************************************************//
		$arrayUbicacion = ubicacion($infoTercero['codigo_ciudad_identificacion']);
		$arrayUbicacion["estado"];
		if($arrayUbicacion["estado"] == "true"){ $infoTercero['ciudad_identificacion'] = $arrayUbicacion['ciudad']; }
		else{ response_error(array("estado"=>"error", "msj"=>"Codigo de ubicacion ciudad documento no valido Tercero ".$infoTercero['numero_identificacion'].' '.$infoTercero['nombre']),false); return; }

		//================================// TIPO NIT //================================//
		//******************************************************************************//
		$arrayTipoDocumento = tipo_documento($infoTercero['codigo_tipo_identificacion'], $idEmpresaErp);
		if($arrayTipoDocumento["estado"] == "true"){ $idTipoNitTercero = $arrayTipoDocumento['id']; }
		else{ response_error(array("estado"=>"error", "msj"=>"Codigo tipo de identificacion no valido Tercero ".$infoTercero['numero_identificacion'].' '.$infoTercero['nombre']),false); return; }

		//================================// REGIMEN //================================//
		//*****************************************************************************//
		$arrayRegimen = tipo_regimen($arrayWs['id_pais'],$infoTercero['codigo_tipo_identificacion']);
		if($arrayRegimen["estado"] == "true"){ $idRegimenErp = $arrayRegimen['id']; }
		else{ response_error(array("estado"=>"error", "msj"=>"Codigo tipo de Regimen no valido Tercero ".$infoTercero['numero_identificacion'].' '.$infoTercero['nombre']),false); return; }
		...
		if($contTercero == 0){
			...
			$queryInsertTercero = mysql_query($sqlInsertTercero,$link);

			$sqlIdTerceroErp    = "SELECT LAST_INSERT_ID()";
			$idTerceroErp       = mysql_result(mysql_query($sqlIdTerceroErp,$link),0,0);
		}
		//SI EXISTE UPDATE
		else{
			$sqlUpdateTercero = "UPDATE terceros
								SET id_tipo_identificacion = '$idTipoNitTercero',
									...
									email                 = '$infoTercero[email]'
								WHERE numero_identificacion='$infoTercero[numero_identificacion]' AND activo=1";
			$queryUpdateTercero = mysql_query($sqlUpdateTercero,$link);
		}
		//===================================================// SINC CONTACTOS //===================================================//
		$arrayEmail = array();
		if(isset($arrayWs['arrayContactos'])){
			...
			$queryTratamiento = mysql_query($sqlTratamiento,$link);
			...
			$queryContactos = mysql_query($sqlContactos,$link);
			...
			if(COUNT($arrayContactoInsert) > 0){
				$estadoContacto = insertUpdateTerceroContacto($arrayContactoInsert,"INSERT",$link);
				if(@$estadoContacto['estado']=='error') return $estadoContacto;
			}

			if(COUNT($arrayContactoUpdate) > 0){
				$estadoContacto = insertUpdateTerceroContacto($arrayContactoUpdate,"UPDATE",$link);
				if(@$estadoContacto['estado']=='error') return $estadoContacto;
			}
			...
			$queryIdErpContactos = mysql_query($sqlIdErpContactos,$link);
			...
			if(is_array($arrayEmail)){
				...
				if(COUNT($arrayEmailInsert) > 0){
					$estadoEmail = insertUpdateTerceroEmail($arrayEmailInsert,"INSERT",$link);
					if(@$estadoEmail['estado'] == 'error'){	return $estadoEmail; }
				}
				// if(COUNT($arrayEmailUpdate) > 0){ insertUpdateTerceroEmail($arrayEmailUpdate,"UPDATE",$link); }	//NO HAY FORMA DE VALIDAR CUANDO UN EMAIL SE HA UPDATE
			}
		}
		...
		if(isset($arrayWs['arraySucursales'])){
			$whereIn = "";
			...
			if(COUNT($arrayDireccionInsert) > 0){
				$estadoSucursal = insertUpdateTerceroSucursales($infoTercero,$arrayDireccionInsert,"INSERT",$link);
				if(@$estadoSucursal['estado'] == 'error'){ return $estadoSucursal; }
			}

			if(COUNT($arrayDireccionUpdate) > 0){
				$estadoSucursal = insertUpdateTerceroSucursales($infoTercero,$arrayDireccionUpdate,"UPDATE",$link);
				if(@$estadoSucursal['estado'] == 'error'){ return $estadoSucursal; }
			}
		}

		response_error(array('estado' => 'true', 'msj'=>'- '.$infoTercero['numero_identificacion'].' '.$infoTercero['nombre'].' Ok...'),false); return;
	}

	function insertUpdateTerceroContacto($arrayContacto,$action,$link){
		...
		if($action == "INSERT"){
			...
			if($valueInsert != ""){
				...
				$sqlInsertContactos = "INSERT INTO terceros_contactos (
											...
											id_empresa)
										VALUES $valueInsert";
				$queryInsertContactos = mysql_query($sqlInsertContactos,$link);
			...
			foreach ($arrayContacto as $nitContacto => $contacto) {
				...
				$sqlUpdateContacto = "UPDATE terceros_contactos
									...
									WHERE identificacion = '$nitContacto'
										AND activo=1";
				$queryUpdateContacto = mysql_query($sqlUpdateContacto,$link);
			}
		}
	}


	function insertUpdateTerceroEmail($arrayEmails,$action,$link){
		...
		if($action == "INSERT"){
			...
			$queryInsertEmail = mysql_query($sqlInsertEmail,$link);
		}
			...
			foreach ($arrayEmail as $idContacto => $infoEmail) {
				...
				$queryUpdateEmail = mysql_query($sqlUpdateEmail,$link);
			}
		}
	}

	function insertUpdateTerceroSucursales($arrayTercero,$arraySucursal,$action,$link){
		...
		if($action == "INSERT"){
			...
			foreach ($arraySucursal as $infoSucursal) {
				...
				if($infoSucursal['pais'] != ''){
					$queryUbicacion = validatePais($infoSucursal['pais'],$link);
				...
				if($infoSucursal['departamento'] != ''){
					$queryUbicacion = validateDepartamento($infoSucursal['id_pais'],$infoSucursal['departamento'],$link);
					if($queryUbicacion['estado'] != 'true'){ return $queryUbicacion; }
				...
				if($infoSucursal['ciudad'] != ''){
					$queryUbicacion = validateCiudad($infoSucursal['id_pais'],$infoSucursal['id_departamento'],$infoSucursal['ciudad'],$link);
					if($queryUbicacion['estado'] != 'true'){ return $queryUbicacion; }
			...
			if($valueInsert != ""){
				...
				$sqlInsertSucursal = "INSERT INTO terceros_direcciones (
									...
									VALUES $valueInsert";
				$queryInsertSucursal = mysql_query($sqlInsertSucursal,$link);
			...
			foreach ($arraySucursal as $infoSucursal) {
				...
				$queryUpdateSucursal = mysql_query($sqlUpdateSucursal,$link);
