<?php
	...
	foreach ($responseWs as $indice => $arrayCuentas) {
		// VALIDAR SI ES UNA NOTA DE AJUSTE
		if ($arrayCuentas['tipo_doc']=='I') { continue; }
	...
	foreach ($arrayFacturas as $numeroFactura => $arrayFactura) {
		...
		foreach ($cuentasFactura as $i => $arrayCuenta) {
			...
			$absSaldo    = ABS($saldo);
/*
			if($arrayCuenta['valor_iva'] > 0 && $arrayCuenta['procentage_iva'] > 0){
				// $ivaCalculado = ROUND($arrayCuenta['valor'] * $arrayCuenta['procentage_iva'] / 100, $_SESSION['DECIMALESMODENA']);
				// $totalCalculado = $ivaCalculado + $arrayCuenta['valor'];

				// if($ivaCalculado != $arrayCuenta['valor_iva']){ response_error(array('factura' => $numeroFactura,'estado' => 'error','msj'=>$ivaCalculado.' != '.$arrayCuenta['valor_iva'].'El iva calculado no concide con el Facturado #'.$numeroFactura),false); continue 2; }
				// else if($totalCalculado != $arrayCuenta['valor_total']){ response_error(array('factura' => $numeroFactura,'estado' => 'error','msj'=>'El total de la cuenta no concide con el Facturado #'.$numeroFactura),false); continue 2; }

				$absSaldo = $arrayCuenta['valor'];			// VALOR SIN IVA

				//CONTABILIDAD IVA
				$debito  = ($saldo > 0)? $arrayCuenta['valor_iva']: 0;
				$credito = ($saldo < 0)? $arrayCuenta['valor_iva']: 0;

				$validaDebito  += $debito;
				$validaCredito += $credito;

				$j++;
				$arrayWs['documento']['cuentas'][$j] = array('cuenta_niif'=> $arrayCuenta['cuenta_iva_niif'],
																'cuenta_colgaap' => $arrayCuenta['cuenta_iva_colgaap'],
...
*/
			...
			$arrayWs['documento']['cuentas'][$j] = array('cuenta_niif'=> $arrayCuenta['cuenta_niif'],
															...
															'debito'         => $debito,
															'credito'        => $credito,
															'cuenta_pago'    => (($arrayCuenta['tipo']=='M')? false : $cuenta_pago)
