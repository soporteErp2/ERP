<?php
	...
	if($cont==0){ response_error(array('estado' => 'error','msj'=>'la cuenta de pago no existe en el sistema!')); }

	// INSERTAR LA CABECERA DEL DOCUMENTO
	$random_documento = responseUnicoRanomico();

	if($cont==0){ response_error(array('estado' => 'error','msj'=>'no hay resolucion dian para los consecutivos de las facturas!')); }
	...
	$query = mysql_query($sql,$link);
	$cont  = mysql_result($query,0,'cont');
    if ($cont>0) { response_error(array('estado' => 'error','msj'=>'ya existe una factura con ese numero de documento!')); }
    ...
    foreach ($arrayWs['cuentas'] as $key => $arrayResul) {
		...
		$acumuladoDebito  += $arrayResul['debito'];
		$acumuladoCredito += $arrayResul['credito'];
		...
		if ($arrayResul['cuenta_niif']=='') {
			...
			if ($arrayResul['cuenta_niif']=='') {		// SINO EXISTE LA CUENTA NIIF
				rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
				return array('estado' => 'error','msj'=> "La cuenta $arrayResul[cuenta_colgaap] niif no existe en el sistema");
			}
		}
		...
		if ($idPuc==0) {
			rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
			return array('estado' => 'error','msj'=> "La cuenta $arrayResul[cuenta_colgaap] colgaap no existe en el sistema");
		}
		if ($idPucNiif==0) {
			rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
			return array('estado' => 'error','msj'=> "La cuenta $arrayResul[cuenta_colgaap] niif no existe en el sistema");
		...
		if ($arrayResul['codigo_centro_costos']>0) {
		  	...
		  	if(substr($arrayResul['cuenta_colgaap'], 0,1)=='4' || substr($arrayResul['cuenta_colgaap'], 0,1)=='5' || substr($arrayResul['cuenta_colgaap'], 0,1)=='6') {
				...
				if ($cont==0) {
					rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
					return array('estado' => 'error','msj'=> "El centro de costos para La cuenta $arrayResul[cuenta_colgaap] no existe en el sistema");
				}
		  	}
    ...
    if ($saldoTotal!=0){
    	rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    	return array('estado' => 'error','msj'=> "la suma de lo saldos de las cuentas no son iguales!");
    }
    ...
    if ($valueInsertCuentasColgaap =='' || $valueInsertCuentasColgaap =='') {
    	rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    	return array('estado' => 'error','msj'=> "no hay cuentas para insertar!");
    }
	...
	if (!$queryInsert ) {
    	rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    	return array('estado' => 'error','msj'=> "no se insertaron los asientos colgaap!");
    }
	...
	$sql   = "UPDATE $tablaPrincipal
				SET estado=1,
					total_factura=$valor_cuenta_pago,
					total_factura_sin_abono=$valor_cuenta_pago
				WHERE activo=1
					AND id_empresa=$arrayWs[id_empresa]
					AND id=$id_documento";
	$query = mysql_query($sql,$link);
    ...
    if (!$query) {
    	rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    	return array('estado' => 'error','msj'=> "no se genero el consecutivo del documento");
    }
    ...
    if ($tipo_documento=='FV') {
    	...
    	if (!$query) {
    		// rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    		return array('estado' => 'error','msj'=> "no se actualizo el consecutivo del documento");
    	}
    }
    else{
    ...
    if ($cont==0) {
    	// rollback($tablaPrincipal,$id_documento,$arrayWs['id_empresa'],$link,$tipo_documento);
    	return array('estado' => 'error','msj'=> "no se pudo consultar el consecutivo del documento del documento! comuniquese con el administrador del sistema!");
    }
    ...
    if (!$queryColgaap || !$queryNiif) {
    	return array('estado' => 'error','msj'=> "no se actualizo el consecutvo a los asientos contables! comuniquese con el administradr del sistema");
    }
	...
	function rollback($tablaPrincipal,$id_documento,$id_empresa,$link,$tipo_documento=''){
