<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory, $link, $id,$id_empresa,$id_usuario,$tabla,$tabla_id){
            ...
            $sqlInsert = "INSERT INTO $tabla ($tabla_id,nombre_archivo,ext,fecha_creacion,id_empresa,id_usuario)
                            VALUES ($id,'$filename','$ext',now(),'$id_empresa','$id_usuario')";
    ...
    $ruta2 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/'.$folder1;
    $url   = $ruta2.'/';
    if(!file_exists($ruta2)){ mkdir ($ruta2); }

    $ruta3 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/'.$folder1.'/'.$folder2;
    $url   = $ruta3.'/';
    if(!file_exists($ruta3)){ mkdir ($ruta3); }

    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($url, $link, $id,$_SESSION['EMPRESA'],$_SESSION['IDUSUARIO'],$tabla,$tabla_id);
