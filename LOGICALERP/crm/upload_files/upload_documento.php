...
<script>
	...
	function createUploader(){
		var uploader = new qq.FileUploader({
            ...
            action 		: 'upload_files/guarda_archivo_adjunto.php',
            debug		: true,
			params		:
			{
				id  : '<?php echo $id; ?>',	
			...
			onComplete	: function(id, fileName, responseJSON){
								...
								table.innerHTML = table.innerHTML+'<tr id="archivo_adjunto_'+RespJson.idRow+'">'+
																		'<td style="width:40%;text-overflow:ellipsis;overflow:hidden;">'+RespJson.filename+'.'+RespJson.ext+'</td>'+
																		'<td style="width:20%">'+RespJson.fecha+'</td>'+
																		'<td style="width:20%">'+RespJson.usuario+'</td>'+
																		'<td style="width:10%"><img src="img/view.png" title="Ver Archivo" onclick="ver_documentos_adjuntos(\''+RespJson.idRow+'\',\''+RespJson.filename+'\',\''+RespJson.ext+'\',\'<?php echo $folder ?>\')"></td>'+
																		'<td style="width:10%"><img src="img/delete_file.png" title="Eliminar Archivo" onclick="eliminarArchivoAdjunto(\''+RespJson.idRow+'\',\''+RespJson.filename+'.'+RespJson.ext+'\',\'<?php echo $folder ?>\')" ></td>'+
																	'</tr>';
