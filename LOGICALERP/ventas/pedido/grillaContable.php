<?php
    ...
    // $sqlFecha     = "SELECT dias_vencimiento FROM ventas_pedidos_configuracion WHERE activo=1  AND id_empresa='$id_empresa'";
...
<?php
    ...
    if(isset($idConsecutivoCotizacionPedido)){
        ...
        $sqlConsulOrden   = "SELECT nit, id_cliente,cod_cliente,cliente,observacion,documento_vendedor,nombre_vendedor FROM $tablaCarga WHERE consecutivo='$idConsecutivoCotizacionPedido' AND id_sucursal='$id_sucursal' AND id_bodega='$filtro_bodega' AND id_empresa='$id_empresa'";
        ...
        $random_factura = responseUnicoRanomico();      //id unico

        //insertar datos de la tabla principal
        $sqlInsertFactura   ="INSERT INTO $tablaPrincipal(id_empresa,random,nit,id_cliente,cod_cliente,cliente,fecha_inicio,id_sucursal,id_bodega,observacion,documento_vendedor,nombre_vendedor)
                                VALUES('$id_empresa','$random_factura','$nit','$id_cliente_cotizacion','$cod_cliente_cotizacion','$cliente_cotizacion','$fecha','$id_sucursal','$filtro_bodega','$observacion_cotizacion','$documento_vendedor','$nombre_vendedor')";
        $queryInsertFactura = mysql_query($sqlInsertFactura,$link);


        if (!$queryInsertFactura) { echo '<script>alert("Error!\nSe produjo un error y no se cargo el pedido.");</script>'; exit; }         //verificamos que se inserto en la tabla
        ...
        $sqlConsultaInventario = "SELECT COI.id,COI.id_inventario,COI.codigo,COI.nombre,COI.cantidad,COI.costo_unitario,
    ...
    //======================= SI NO EXISTE EL PEDIDO SE CREA EL ID UNICO =======================//
    ...
    //============================ SI EXISTE EL PEDIDO =============================//
        ...
        if ($opcGrillaContable=='PedidoVenta' && $estado=='2') { echo "ESTE PEDIDO DE VENTA ESTA CERRADO "; exit; }
...
<script>
   ...
   UpdateFormaPago<?php echo $opcGrillaContable; ?>(document.getElementById("fechaFinal<?php echo $opcGrillaContable; ?>").value);
    ...
    function ventanaBuscarCotizacionPedido<?php echo $opcGrillaContable; ?>(){
        ...
        var titulo      = "Seleccione la Cotizacion"
        ,   tablaGrilla = "ventas_cotizaciones";
    ...
    function guardarNewArticulo<?php echo $opcGrillaContable; ?>(cont){
        ...
        Ext.getCmp("btnNueva<?php echo $opcGrillaContable; ?>").enable();       //HABILITA BTN NUEVO

        if (accion=="agregar") { calcTotalDocCompraVenta<?php echo $opcGrillaContable ?>(cantArticulo,descuentoArticulo,costoArticulo,accion,tipoDesc,iva,cont); }  //CALC TOTAL FACTURA
    }
    ...
    function guardar<?php echo $opcGrillaContable; ?>(){
        ...
        if (validacion==0) { alert("No hay articulos por guardar en el presente pedido de venta!!"); return; }
    ...
    function ajaxVerificaCantidadArticulo<?php echo $opcGrillaContable; ?>(cont,cantidad,opc){
        ...
        Ext.Ajax.request({
            ...
            success :function (result, request){
                        ...
                        else if (result.responseText=='false'){ alert("Aviso!\nNo se verifico la cantidad del Articulo\nSi el problema persiste comuniquese con el administrador del sistema"); }
