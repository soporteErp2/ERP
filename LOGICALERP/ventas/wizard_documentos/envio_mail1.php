<?php
	...
	//echo '<script>alert("'.$_SESSION['ID_HOST'].'")</script>';

	//CONSULTAMOS LOS DATOS DE LA ORDEN DE COMPRA
    $campoConsec = 'consecutivo';
	if($tablaBuscar=='ventas_facturas'){
		$campoConsec = 'numero_factura_completo';
	}

	$sqlOrden    = "SELECT $campoConsec AS consecutivo,cliente,id_empresa,sucursal,bodega,usuario,id_vendedor FROM $tablaBuscar WHERE id= '$id_intercambio' AND activo = 1 LIMIT 0,1";
	...
	$sqlEmpresa   = "SELECT nombre FROM empresas WHERE id = '$id_empresa' AND activo = 1 LIMIT 0,1";
	...
	$Subject  = $textEmail.$arrayTabla['consecutivo'].' para '.$arrayTabla['cliente'];
	$Debug    = true;

	//CONSULTAMOS LA INFORMACION DEL VENDEDOR

	$sqlCont   = "SELECT nombre,celular_empresa AS telefono,email_empresa AS email FROM empleados WHERE id= '$arrayTabla[id_vendedor]' AND activo = 1 LIMIT 0,1";
	$queryCont = mysql_query($sqlCont,$link);

	$nombre_vendedor   = mysql_result($queryCont, 0, 'nombre');
	$telefono_vendedor = mysql_result($queryCont, 0, 'telefono');
	$email_vendedor    = mysql_result($queryCont, 0, 'email');

	if($email_vendedor!=''){
		$tdE = '<tr>
        		  <td><b>Email:</b></td><td>'.$email_vendedor.'</td>
        	  </tr>';
	}
	if($telefono_vendedor!=''){
		$tdT = '<tr>
            	  <td><b>Telefono:</b></td><td>'.$telefono_vendedor.'</td>
               </tr>';
	}
    ...
    $body = '<div style="font-size:16px;padding-bottom:16px;font-weight:bold;padding-left:5px">'.$empresa.'</div>
            <table style="width:350px">
            	...
            	<tr>
            		<td><b>Usuario:</b></td><td>'.$arrayTabla['usuario'].'</td>
            	</tr>
            	<tr>
            		<td><b>&nbsp;</b></td><td>&nbsp;</td>
            	</tr>
            	<tr>
            		<td><b>Vendedor:</b></td><td>'.$nombre_vendedor.'</td>
            	</tr>'.$tdT.$tdE.'
            </table>
            <br>
            	<br><br>'.$body.'
