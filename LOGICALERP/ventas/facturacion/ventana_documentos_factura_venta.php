<?php
  ...
  //NOMBRE DE LA GRILLA
    $grilla->GrillaName         = 'ventasFacturasDocumentos';     						//NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
  //QUERY
    $grilla->TableName          = 'ventas_facturas_documentos';       				//NOMBRE DE LA TABLA EN LA BASE DE DATOS
    $grilla->MyWhere            = "activo = 1 AND id_factura_venta = '$id_factura_venta'";     //WHERE DE LA CONSULTA A LA TABLA "$TableName"
  ...
  //CONFIGURACION DE CAMPOS EN LA GRILLA
    ...
    $grilla->AddRowImage('','<center><div id="renderDeleteDocumentoFactura_[id]" style="float:left; max-width:20px; overflow:hidden;"></div><img src="img/delete.png" style="cursor:pointer" style="width:16px; height:16px;" onclick="deleteDocumentoFactura([id],\'[nombre]\',\'[ext]\');" /></center>','20');
    $grilla->AddRowImage('Ver','<center><img src="img/ver.png" style="cursor:pointer" style="width:16px; height:16px;" onclick="viewDocumentoFactura([id],\'[nombre]\',\'[ext]\');" /></center>','25');
  ...
  /**//////////////////////////////////////////////////////////////**/
	...
	if(!isset($opcion)){ ?>
	  ...
	  <script>
	    function deleteDocumentoFactura(id_documento,nombre,ext){
	      if(!confirm("Aviso,\nEsta seguro de eliminar un documento cargado?")){
					return;
				}

        Ext.get('renderDeleteDocumentoFactura_' + id_documento).load({
          url     : 'facturacion/bd/bd.php',
          scripts : true,
          nocache : true,
          params  : {
					            opc         : 'deleteDocumentoVentasFacturas',
	      ...
	      var uploader = new qq.FileUploader({
	        ...
	        action  					: 'facturacion/upload_file/upload_file.php',
	        debug   					: false,
	        params  					: {
	        ...
	        onComplete        : function(id, fileName, responseJSON){
                                ...
                                if(responseJSON.idInsert > 0){
																	Inserta_Div_ventasFacturasDocumentos(responseJSON.idInsert);
																}
	                            },
	    ...
	    function viewDocumentoFactura(id,nameFile,ext){
        if(!validaImagen(ext)){
          window.open("facturacion/bd/bd.php?opc=downloadFile&nameFile=" + nameFile + "&ext=" + ext + "&id=" + id);
        }
				else{
          ...
          if(ext == 'pdf'){
            ...
            Ext.Ajax.request({
              ...
              success : function(response){
                          ...
                          var alto  = response.alto, ancho = response.ancho, fileName = response.file;
        ...
        var titulo = (document.getElementById('div_ventasFacturasDocumentos_nombre_'+id).innerHTML)+'.'+(document.getElementById('div_ventasFacturasDocumentos_ext_'+id).innerHTML);
