	...
	*/
    ...
    public $mysql;
      ...
      $this->nitVF 										 = $this->mysql->result($queryVentasFacturas,0,'nit');
			...
			$this->consecutivo_resolucionVF  = $this->mysql->result($queryVentasFacturas,0,'consecutivo_resolucion');
      ...
      $this->codigo_tipo_documentoT      = $this->mysql->result($queryTerceros,0,'codigo');
			...
			$sqlTercerosDireccion  = "SELECT
																	...
																	M.id AS codigo_ciudad
																FROM terceros_direcciones AS TD
																LEFT JOIN	ubicacion_departamento AS UD ON	TD.id_departamento = UD.id
																LEFT JOIN	ubicacion_ciudad AS UC ON	TD.id_ciudad = UC.id
    ...
    public function construirJSON(){
      ...
      $arrayPrincipal = array(
				"number" => (int) $this->numero_facturaVF,
				"type_document_id" => 1,
				"resolution_number" => $this->consecutivo_resolucionVF,
				"date" => date('Y-m-d'),
				"time" => date('H:i:s'),
				"sendmail" => true,
				"customer" => array(
					"identification_number" => (int) str_replace(array(".","-"),"",$this->numero_identificacionT),
					"dv" => $this->dvT,
					"name" => $this->quitarTildes($this->nombreT),
					"phone" => $this->telefono1TD,
					"address" => $this->quitarTildes($this->direccionTD),
					"email" => $emails,
					"merchant_registration" => ($this->numero_matricula_mercantilTD == null)? "" : $this->numero_matricula_mercantilTD,
					"type_document_identification_id" => $this->codigo_tipo_documentoT,
					"type_organization_id" => $this->tipo_persona_codigoT,
					"municipality_id" => $this->codigo_ciudadTD,
					"type_regime_id" => $this->codigo_regimen_dianT
				),
				"payment_form" => array(
					"payment_form_id" => ($this->forma_pagoVF == "Contado")? 1 : 2,
					"payment_method_id" => $this->codigo_metodo_pago_dianVF,
					"payment_due_date" => $this->fecha_vencimientoVF,
					"duration_measure" => (string) $this->dias_pagoVF
				),
				"allowance_charges" => [
					array(
						"discount_id" => 1,
				...
				"legal_monetary_totals" => array(
					"line_extension_amount" => (string) round($subTotal,$_SESSION['DECIMALESMONEDA']),
					"tax_exclusive_amount" => ($totalIVA != null)? (string) round($totalIVA,$_SESSION['DECIMALESMONEDA']) : "0",
					"tax_inclusive_amount" => (string) (round($subTotal,$_SESSION['DECIMALESMONEDA']) + round($totalIVA,$_SESSION['DECIMALESMONEDA'])),
					"allowance_total_amount" => "0.00",
					"charge_total_amount" => "0.00",
					"payable_amount" => (string) (round($subTotal,$_SESSION['DECIMALESMONEDA']) + round($totalIVA,$_SESSION['DECIMALESMONEDA']))
				),
				"tax_totals" =>
				[
					array(
