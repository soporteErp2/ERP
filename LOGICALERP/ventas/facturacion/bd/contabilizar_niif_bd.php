<?php
    ...
    function contabilizarSinPlantillaNiif($arrayAnticipo,$arrayCuentaPago,$idCcos,$fechaFactura,$consecutivoFactura,$idBodega,$idSucursal,$idEmpresa,$idFactura,$idCliente,$exento_iva,$link){
		...
		$sqlDoc = "SELECT VF.id,
						...
						IF(I.cuenta_venta like '2408%', 'si', 'no') as IVA
		...
		foreach ($arrayInventarioFactura AS $valArrayInventario) {
			...
			if($cuentaPrecio > 0){
				...
				$arrayAsiento[$cuentaPrecio]['idCcos'] = ($idCcos<>'')? $idCcos : $valArrayInventario['id_centro_costos'];
				...
				if($cuentaImpuesto > 0 && $valArrayInventario['impuesto'] > 0){
					...
					$acumIVA					+= ($valArrayInventario['IVA'] == 'si')? $valArrayInventario['impuesto'] : 0;
				}
				...
				if($contraPrecio > 0){
					...
					//ARRAY ASIENTO CONTABLE
					if($arrayAsiento[$contraPrecio][$estado] > 0){ $arrayAsiento[$contraPrecio][$estado] += ROUND($contraSaldo,$_SESSION['DECIMALESMONEDA']); }
					else{ $arrayAsiento[$contraPrecio][$estado] = ROUND($contraSaldo,$_SESSION['DECIMALESMONEDA']); }
			...
			if( $cuentaCosto > 0 && $contraCosto > 0 && $valArrayInventario['nombre_referencia'] != 'Remision' ){
				...
				$arrayAsiento[$contraCosto]['idCcos'] = ($idCcos<>'')? $idCcos : $valArrayInventario['id_centro_costos'];
		...
		while($rowRetenciones = mysql_fetch_array($queryRetenciones)){
			...
			if($tipoRetencion == "ReteIva"){																		//CALCULO RETEIVA
				if ($exento_iva=='Si' || $acumIVA < $valorBase) { continue; }

				$acumRetenciones += ROUND($acumIVA * $valorRetencion/100, $decimalesMoneda);
				$arrayAsiento[$codigoRetencion][$estadoRetencion] += ROUND($acumIVA * $valorRetencion/100, $decimalesMoneda);
			}
			else{ 																									//CALCULO RETE, RETECREE Y RETEICA
		...
		if($saldoAnticipo > 0 && $saldoAnticipo > $saldoClientes){
			if(!(in_array('301', $_SESSION["PERMISOS"])) || count($arrayAnticipo['anticipos']) > 1){echo'<script>
					alert("Aviso.\nLos anticipos no pueden ser mayores a la factura de venta! Niff \n saldo clientes: '.$saldoClientes.'");
					document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
				</script>';
			exit;
			}
			else{
				$arrayAsiento[$acumCuentaClientes][$estadoCuentaClientes] -= $saldoClientes;
				foreach ($arrayAnticipo['anticipos'] as $idAnticipo => $datosAnticipo) {
					$arrayAnticipo['anticipos'][$idAnticipo]["valor"] = $saldoClientes;
				}
			}
		}
