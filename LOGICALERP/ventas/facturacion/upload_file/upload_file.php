...
class qqFileUploader {
  ...
  function handleUpload($uploadDirectory,$id_documento,$id_empleado){
    ...
    if($id_factura_venta != 0){
      $sqlInsertDocumento = "INSERT INTO ventas_facturas_documentos (id_factura_venta, nombre, ext, id_usuario, fecha) VALUES ('$id_factura_venta','$filename','$ext','$id_usuario',NOW())";
    }
...
$ruta2 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/ventas';
$url   = $ruta2.'/';
if(!file_exists($ruta2)){ mkdir ($ruta2); }

$ruta3 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/ventas/facturacion';
$url   = $ruta3.'/';
if(!file_exists($ruta3)){ mkdir ($ruta3); }
$uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
$result   = $uploader->handleUpload($url,$id_documento,$id_empleado);
