<?php
	...
	function cargaFormatoDocumento($id_empresa, $id_sucursal, $tipo_documento){
		$query  = "SELECT id,texto FROM configuracion_documentos_erp WHERE tipo='$tipo_documento' AND id_empresa='$id_empresa' AND id_sucursal='$id_sucursal' LIMIT 0,1";
	...
	function reemplazarVariables($texto,$body,$id_empresa,$id_sucursal,$id_documento,$id_resolucion='',$opcGrillacontable=''){
		...
		while (1 == 1) {	///// RECOJE LAS VARIABLES USADAS EN EL DOCUMENTO EN UN ARRAY
			...
			if($posIni!=""){
				...
				if($variable == ''){ break; }
				$arrayVariables[$cont] = $variable; /// LAS VARIABLES SON APILADAS EN UN ARRAY PARA LUEGO HACER LA BUSQUEDA UNA A UNA
				$cont++;
			}
			else{ break; }
		}
		...
		if($cont>0){
			...
			$whileVariables = '';
			$arrayVariables = array_unique($arrayVariables); 	// QUITA LAS VARIABLES REPETIDAS ASI NO EXISTEN BUSQUEDAS REPETIDAS DE VARIABLES
			$whileVariables = "nombre='".implode("' OR nombre='", $arrayVariables)."'";

			$sql   = "SELECT campo,tabla,nombre FROM variables WHERE ($whileVariables) GROUP BY nombre";
			$query = mysql_query($sql);
			while ($rowVar = mysql_fetch_assoc($query)) {
				$tabla    = $rowVar['tabla'];
				$campo    = $rowVar['campo'];
			...
			while ($rowDigitos = mysql_fetch_assoc($query)) {
				switch ($rowDigitos['documento']) {
					...
					case 'factura':
						$arrayDigitos['ventas_facturas']["numero_factura"] = 0;
						break;
				}
			}

			if ($opcGrillacontable=='factura') {
				$sql="SELECT digitos,consecutivo_resolucion FROM ventas_facturas_configuracion WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_resolucion";
				$query=mysql_query($sql);
				$arrayDigitos['ventas_facturas']["numero_factura"]       = mysql_result($query,0,'digitos');
				$arrayDatos['ventas_facturas']["consecutivo_resolucion"] = mysql_result($query,0,'consecutivo_resolucion');
			}
			foreach ($arrayTable as $tabla => $arrayCampos) {
				...
				switch ($tabla) {
					...
					case 'ventas_facturas_configuracion':
						$whereTabla = "activo=1 AND id_empresa=$id_empresa  AND id=$id_resolucion ORDER BY id DESC";
						break;
				...
				foreach ($arrayCampos as $campo=>$variable) {
					$varReplace = ($arrayDatos[$tabla][$campo]=='')? mysql_result($query,0,$campo) : $arrayDatos[$tabla][$campo] ;
					if($arrayDigitos[$tabla][$campo] >= 2){ $varReplace = str_pad($varReplace, $arrayDigitos[$tabla][$campo], "0", STR_PAD_LEFT); }
	...
	*/
	function num2letras($num, $fem = false, $dec = true) {
		$float=explode('.',$num);
	   	...
	   	$num2=$float[1];
	   	// return $num2;
	   	$end_num = convertir($num, $fem = false, $dec = true);
	   	$end_num2 = convertir($num2, $fem = false, $dec = true);
	   	if ($end_num2<>'') {
