...
<script>
    ...
    Ext.getCmp("BtnGroup_carga_activos_Deterioro").hide();
...
<?php
    ...
    $acumScript .= (user_permisos(205,'false') == 'true')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").enable();' : '';        //guardar
    $acumScript .= (user_permisos(206,'false') == 'true')? 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").enable();' : 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").disable();';       //cancelar
    $acumScript .= (user_permisos(207,'false') == 'true')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").enable();' : 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").disable();';       //cancelar
    $acumScript .= (user_permisos(208,'false') == 'true')? 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").enable();' : 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").disable();';       //cancelar
    ...
    $sql   = "SELECT consecutivo,
                    ...
                    estado
                FROM $tablaPrincipal
                WHERE id='$id_deterioro' AND activo = 1";
    ...
    $acumScript .=  '
                    ...
                    document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML     = "Consecutivo N.<br>'.$consecutivo.'";
    ...
    $bodyArticle = cargaArticulosSave($id_deterioro,$observacion,$estado,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$link);
...
//OCULTAR EL BUTTON GROUP DE CONTABILIZAR CON NIIF
    ...
    function ventanaBuscar<?php echo $opcGrillaContable; ?>(){
        ...
        Win_Ventana_buscar_<?php echo $opcGrillaContable; ?> = new Ext.Window({
            ...
            autoLoad    :
            {
                url     : 'deterioro/bd/buscarGrillaContable.php',
                scripts : true,
                nocache : true,
                params  :
                {
    ...
    function imprimir<?php echo $opcGrillaContable; ?> (){
        window.open("deterioro/bd/imprimirGrillaContable.php?id=<?php echo $id_deterioro; ?>&opcGrillaContable=<?php echo $opcGrillaContable; ?>&tablaPrincipal=<?php echo $tablaPrincipal; ?>&idTablaPrincipal=<?php echo $idTablaPrincipal; ?>&tablaInventario=<?php echo $tablaInventario; ?>&tablaRetenciones=<?php echo $tablaRetenciones; ?>");
    }

    //================================== IMPRIMIR EN EXCEL ============================================//
    function imprimir<?php echo $opcGrillaContable; ?>Excel (){
       window.open("deterioro/bd/exportarExcelGrillaContable.php?id=<?php echo $id_deterioro; ?>&opcGrillaContable=<?php echo $opcGrillaContable; ?>&tablaPrincipal=<?php echo $tablaPrincipal; ?>&idTablaPrincipal=<?php echo $idTablaPrincipal; ?>&tablaInventario=<?php echo $tablaInventario; ?>&tablaRetenciones=<?php echo $tablaRetenciones; ?>");
    }
