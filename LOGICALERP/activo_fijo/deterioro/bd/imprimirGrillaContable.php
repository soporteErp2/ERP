<?php
	...
	while($row = mysql_fetch_array($consul)){
		...
		// if ($opcGrillaContable=='RemisionesVenta') {
			...
			$titulo      = 'DETERIORO ACTIVOS FIJOS';
		...
		while ($array= mysql_fetch_array($queryArticulos)) {
			...
			$articulos.='<tr style="'.$estilo.' '.$styleBorder.'">
							...
							<td style="width: 80px; text-align:right;">'.number_format ($costo,$_SESSION['DECIMALESMONEDA']).'</td>
						 	<td style="width: 80px; text-align:right;">'.number_format (((($costo-$valor)<0 || $valor==0)? 0 : $costo-$valor),$_SESSION['DECIMALESMONEDA']).'</td>
						</tr>';
