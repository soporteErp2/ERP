<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory,$id_sucursal,$id_bodega,$mysql){
            ...
            if ($this->file->save($uploadDirectory.$filename.'.'.$ext)){
                ...
                require("load_excel.php");

                // return array('success'=>true, 'ok' => $body );

                // exit;
                // unlink($uploadDirectory . $filename . '.' . $ext);

                if($errorLoadFile <> 'true'){ return array('success'=>true, 'idUpload' => "$idUpload" ); }
                else{ return array('error'=> ''.$resulProcess, 'debug'=> "$resulProcess"); }

                // return array('success'=>true, 'nombreArchivo'=>$filename);
            }
    ...
    $url  = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/activos_fijos/';
    if(!file_exists($url)){ mkdir ($url); }

    $url = $url.'carga_activos/';
    if(!file_exists($url)){ mkdir ($url); }

    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($url,$sucursal,$bodega,$mysql);
