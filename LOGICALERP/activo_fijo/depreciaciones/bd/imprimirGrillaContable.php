<?php
	...
	$sqlImagen   = "SELECT nombre,ext FROM configuracion_imagenes_documentos WHERE activo=1 AND id_empresa='$idEmpresa'";
	...
	echo$SQL    = "SELECT * FROM $tablaPrincipal WHERE id='$id' AND activo=1 AND id_empresa='$idEmpresa'";
	...
	while($row = mysql_fetch_array($consul)){
		...
		// if ($opcGrillaContable=='RemisionesVenta') {
			$documento   = 'Depreciacion';
			$titulo      = 'DEPRECIACION ACTIVOS FIJOS';
		...
		while ($array= mysql_fetch_array($queryArticulos)) {
			...
			$articulos.='<tr style="'.$estilo.' '.$styleBorder.'">
							...
							<td style="width: 80px; text-align:left; padding-left:5px;">'.$array["unidad"].'</td>
							<td style="width: 60px; text-align:right;">'.number_format ($array["depreciacion_acumulada"],$_SESSION['DECIMALESMONEDA']).'</td>
							<td style="width: 80px; text-align:right;">'.$array["dias_depreciar"].' </td>
							<td style="width: 80px; text-align:right;">'.number_format ($array["costo"],$_SESSION['DECIMALESMONEDA']).'</td>
						 	<td style="width: 80px; text-align:right;">'.number_format ($array["valor"],$_SESSION['DECIMALESMONEDA']).'</td>
						</tr>';

			if($array["observaciones"] != ''){
				$articulos.='<tr style="'.$estilo.' border-bottom:1px solid #000;">
								<td style="width:100%;" colspan="6">'.str_replace("\n",'<br>',$array["observaciones"]).'</td>
							 </tr>';
			}		//IVA
		...
		$sqlCliente        = "SELECT direccion,telefono1,ciudad FROM terceros WHERE id_empresa ='$idEmpresa' AND tercero = 1 AND id=".$row["id_cliente"];
		...
		$contenido = '<table class="articlesTable">
						<thead>
							<tr>
								...
								<td style="width:60px;">DEPREC. ACUM.</td>
								<td style="width:80px;">PERIODICIDAD</td>
								<td style="width:80px;">COSTO</td>
								<td style="width:80px; border-right: 1px solid;">VALOR DEPRECIACION</td>
							</tr>
						</thead>
						<tbody>'.$articulos.'</tbody>
					</table>

					<div style="float:left; margin:10px; width:48%; font-size:13px;">TOTAL EN LETRAS: <span style="text-transform: capitalize;">'.num2letras(round($totalDocumento,$_SESSION['DECIMALESMONEDA'])).'</span></div>

					<div style=" float:left; margin:0px; width:50%;">

						<table style="border-bottom: 1px solid; border-top: 1px solid; font-style:normal; font-size:11px; margin-left:5px;">
							<tr style="font-weight:bold;">
								<td style="width:170px; text-align:center; font-weight:bold;">TOTAL DEPRECIACION</td>
								<td style="width:10px; font-weight:bold;">$</td>
								<td style="text-align:right; font-weight:bold; width:170px;">'.number_format ($totalDocumento,$_SESSION['DECIMALESMONEDA']).'</td>
							</tr>
						</table>
					</div>
					<br>
	...
	else{ $MS = 50 ; $MD = 10;$MI = 15;$ML = 10; }		//con imagen ms=86 sin imagen ms=71
