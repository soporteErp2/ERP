<?php
	//============= CARGAR TODOS LOS ACTIVOS FIJOS DE UNA SUCURSAL =============//
	...
	function cargaArticulosSave($id,$observacion,$estado,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$link){
		...
		$sql = "SELECT id,id_activo_fijo,codigo_activo AS codigo,nombre,unidad,numero_piezas,costo,dias_depreciar,depreciacion_acumulada,valor
		...
		if($estado < 1){
			while($row = mysql_fetch_array($query)){
				...
				$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
										'.cargaDivsUnidadesSave($cont,$opcGrillaContable,$row['id'],$row['id_activo_fijo'],$row['codigo'],$row['nombre'],$row['unidad'],$row['numero_piezas'],$row['dias_depreciar'],$row['costo'],$row['depreciacion_acumulada'],$row['valor'],$estado).'
									</div>';
			}
			$cont++;
			$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
			...
			while($row = mysql_fetch_array($query)){
				$cont++;
				$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
										'.cargaDivsUnidadesBloqueadas($cont,$opcGrillaContable,$row['id'],$row['id_activo_fijo'],$row['codigo'],$row['nombre'],$row['unidad'],$row['numero_piezas'],$row['dias_depreciar'],$row['costo'],$row['depreciacion_acumulada'],$row['valor'],$estado).'
									</div>';
			}
			$mostarBoton = 'disable()';
	...
	function cargaDivsUnidadesSave($cont,$opcGrillaContable,$id = 0,$id_activo_fijo = '',$codigo = '',$nombre = '',$unidad = '',$numero_piezas = '',$dias_depreciar = '1',$costo = 0,$depreciacion_acumulada = 0,$valor = 0,$estado = ''){
		...
		$body =  '<div class="campo" style="width:40px !important;">
							...
							<div class="campo"><input type="text" id="unidades'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly '.$deshabiltar.'value="'.$unidades.'" /></div>
							<div class="campo"><input type="text" id="depreciacionAcumulada'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly '.$deshabiltar.'value="'.round($depreciacion_acumulada,$_SESSION['DECIMALESMONEDA']).'" /></div>
							...
							<div class="campo"><input type="text" id="costoArticulo'.$opcGrillaContable.'_'.$cont.'" '.$deshabiltar.' '.$readonly_precio.' value="'.round($costo,$_SESSION['DECIMALESMONEDA']).'" readonly /></div>

							<div class="campo"><input type="text" id="costoTotalArticulo'.$opcGrillaContable.'_'.$cont.'" '.$deshabiltar.'  value="'.round($valor,$_SESSION['DECIMALESMONEDA']).'" onKeyup="validarNumberArticulo'.$opcGrillaContable.'(event,this,\'double\',\''.$cont.'\');"/></div>
							...
							<div style="float:right; min-width:80px;">
								...
								<div onclick="deleteArticulo'.$opcGrillaContable.'('.$cont.')" id="deleteArticulo'.$opcGrillaContable.'_'.$cont.'" title="Eliminar Articulo" style="width:20px; float:left; margin-top:3px; '.$displayBtns.'cursor:pointer;"><img src="images/delete.png"></div>
							</div>
							...
							<script>
								...
								calculaValorTotalesDocumento("agregar",'.round($valor,$_SESSION['DECIMALESMONEDA']).');
							</script>';
	...
	function cargaDivsUnidadesBloqueadas($cont,$opcGrillaContable,$id = 0,$id_activo_fijo = '',$codigo = '',$nombre = '',$unidad = '',$numero_piezas = '',$dias_depreciar = '1',$costo = 0,$depreciacion_acumulada = 0,$valor = 0,$estado = ''){
		...
		$body =  '<div class="campo" style="width:40px !important;">
							...
							<div class="campo"><input type="text" id="unidades'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly value="'.$unidad.' x '.$numero_piezas.'" readonly /></div>
							<div class="campo"><input type="text" id="depreciacionAcumulada'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly value="'.round($depreciacion_acumulada,$_SESSION['DECIMALESMONEDA']).'" /></div>
							<div class="campo"><input type="text" id="cantArticulo'.$opcGrillaContable.'_'.$cont.'" value="'.$dias_depreciar.'" readonly /></div>

							<div class="campo"><input type="text" id="costoArticulo'.$opcGrillaContable.'_'.$cont.'" '.$readonly_precio.' value="'.round($costo,$_SESSION['DECIMALESMONEDA']).'" readonly value="0"/></div>

							<div class="campo"><input type="text" id="costoTotalArticulo'.$opcGrillaContable.'_'.$cont.'"   readonly  value="'.round($valor,$_SESSION['DECIMALESMONEDA']).'" /></div>
