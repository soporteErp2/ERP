<?php
	...
	switch($option){
		case 'deterioro':
			$sqlWhere =  "SELECT id_activo_fijo
									  FROM activos_fijos_deterioro_inventario
										WHERE activo = 1
										AND id_deterioro = $id_deterioro";
			$query = mysql_query($sqlWhere,$link);
			while($row = mysql_fetch_array($query)){
				$whereAF .= ' AND id <> ' . $row['id_activo_fijo'];
			}
			$whereAF .= ' AND deteriorable = "Si"';
			break;
		case 'traslados':
			$sqlWhere =  "SELECT id_activo_fijo
		...
		default:
			$sqlWhere =  "SELECT id_activo_fijo
										FROM activos_fijos_depreciaciones_inventario
										WHERE activo = 1
										AND id_depreciacion = $id_depreciacion";
			$query = mysql_query($sqlWhere,$link);
			while($row = mysql_fetch_array($query)){
				$whereAF .= ($whereAF == '')? 'id <> ' . $row['id_activo_fijo'] : ' AND id <> ' . $row['id_activo_fijo'];
			}
			$whereAF = ($whereAF<>'')? " AND ($whereAF) " : "";
			$whereAF .= ($contabilidad == 'niif')? " AND depreciable_niif = 'Si'" : " AND depreciable = 'Si'";
			break;
	}
	...
	//CONFIGURACION///////////////////////////////////////////////////////////////
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			...
			$grilla->AddRow('Valor Salvamento Niif','valor_salvamento_niif',120);
			$grilla->AddRow('Depreciacion Niif','depreciacion_acumulada_niif',100);
