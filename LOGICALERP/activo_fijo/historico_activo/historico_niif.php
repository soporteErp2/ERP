...
$sql="SELECT nombre_equipo,costo,depreciacion_acumulada_niif FROM activos_fijos WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_activo";
...
$sql="SELECT id,fecha_inicio,consecutivo_niif FROM activos_fijos_depreciaciones WHERE activo=1 AND id_empresa=$id_empresa AND ($whereId) AND estado=1 AND sinc_nota='niif' ";
$query=mysql_query($sql,$link);
$cont=1;
while ($row=mysql_fetch_array($query)) {
	$bodyFilas.='<div class="filaTabla">
 					...
 					<div class="campo0 campo1">'.$row['consecutivo_niif'].'</div>
