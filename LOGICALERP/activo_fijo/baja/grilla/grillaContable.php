...
<?php
  $acumScript .= (user_permisos(232,'false') == 'true')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").enable();
                                                       ...
                                                       : 'Ext.getCmp("btnNueva'.$opcGrillaContable.'").disable();
                                                          document.getElementById("contenidoHijoBaja").style.display="none";';

  $acumScript .= (user_permisos(234,'false') == 'true')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").enable();' : '';

  //======================== SI NO EXISTE EL DOCUMENTO =======================//
  if(!isset($id_baja) || $id_baja == ''){
    ...
    $query      = $mysql->query($sql,$mysql->link);
    $id_baja    = $mysql->result($query,0,'id');
  ...
  //========================= SI EXISTE EL DOCUMENTO =========================//
    ...
    $acumScript .= 'new Ext.form.DateField({
                    ...
                    document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML   = "Baja<br>No. '.$consecutivo.'";
...
<div class="contenedorOrdenCompra" id="contenidoHijoBaja">
  <div class="bodyTop">
...
<script>
  ...
  function ventanaBuscarArticulo<?php echo $opcGrillaContable; ?>(cont){
    var id_sucursal = document.getElementById('filtro_sucursal_Baja').value
    ,   id_bodega   = document.getElementById('filtro_ubicacion_Baja').value
  ...
  function guardarNewArticulo<?php echo $opcGrillaContable; ?>(cont){
    ...
    Ext.get(divRender).load({
      ...
      params  : {
                  ...
                  idBaja                    : '<?php echo $id_baja; ?>', //ID DEL BAJA
                  ...
                  idInsertArticulo          : idInsertArticulo //ID DEL DETALLE DEL BAJA
                }
    });
