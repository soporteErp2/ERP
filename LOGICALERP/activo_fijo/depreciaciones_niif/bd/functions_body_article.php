<?php
	...
	function cargarActivosFijosSucursal($id_documento,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$id_empresa,$id_sucursal,$link){
		...
		while ($row=mysql_fetch_array($query)) {
			$id                          = $row['id'];
			$metodo_depreciacion_colgaap = $row['metodo_depreciacion_niif'];
			$valor_salvamento            = $row['valor_salvamento_niif'];
			$costo_sin_depreciar_anual   = $row['costo_sin_depreciar_anual'];
			$fecha_inicio_depreciacion   = $row['fecha_inicio_depreciacion_niif'];
			$vida_util                   = $row['vida_util_niif'];
			$costo                       = $row['costo'];
			$depreciacion_acumulada      = $row['depreciacion_acumulada_niif'];
  ...
  //============ CALCULAR EL VALOR DE LA DEPRECIACION DE UN ACTIVO ===========//
	...
	function cargaArticulosSave($id,$observacion,$estado,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$link){
		...
		$sql = "SELECT id,id_activo_fijo,codigo_activo,nombre,unidad,numero_piezas,costo,dias_depreciar,depreciacion_acumulada_niif,valor
		...
		if($estado < 1){
			while($row = mysql_fetch_array($query)){
				...
				$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
										'.cargaDivsUnidadesSave($cont,$opcGrillaContable, $row['id'],$row['id_activo_fijo'],$row['codigo_activo'],$row['nombre'],$row['unidad'],$row['numero_piezas'],$row['dias_depreciar'],$row['costo'],$row['depreciacion_acumulada_niif'],$row['valor'],$estado).'
									</div>';
			}
			$cont++;
			$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
			...
			while($row = mysql_fetch_array($query)){
				$cont++;
				$body .= '<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
										'.cargaDivsUnidadesBloqueadas($cont,$opcGrillaContable,$row['id'],$row['id_activo_fijo'],$row['codigo_activo'],$row['nombre'],$row['unidad'],$row['numero_piezas'],$row['dias_depreciar'],$row['costo'],$row['depreciacion_acumulada_niif'],$row['valor'],$estado).'
									</div>';
			}
			$mostarBoton = 'disable()';
