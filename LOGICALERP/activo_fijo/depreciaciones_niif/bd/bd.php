<?php
	...
	switch ($opc) {
		...
		case 'modificarDocumentoGenerado':
			modificarDocumentoGenerado($idDocumento,$opcGrillaContable,$id_empresa,$id_sucursal,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$carpeta,$mysql);
			break;
		...
		case 'restaurarDocumento':
			restaurarDocumento($idDocumento,$opcGrillaContable,$carpeta,$id_sucursal,$id_sucursal,$id_empresa,$tablaPrincipal,$mysql);
			break;
		...
		case 'calculaValorDepreciacion':
			calculaValorDepreciacion($opcGrillaContable,$id_activo,$accion,$cont,$id_empresa,$mysql);
			break;
	...
	function buscarArticulo($id,$campo,$valorArticulo,$idArticulo,$id_empresa,$opcGrillaContable,$whereBodega,$link){
		...
		$sqlArticulo = "SELECT id,codigo_activo,nombre_equipo,unidad,costo,depreciacion_acumulada_niif,valor_salvamento_niif
							FROM activos_fijos
						WHERE  activo = 1
							AND estado=1 AND depreciable_niif='Si'
							$whereBodega
	...
	function retrocederArticulo($id,$idRegistro,$cont,$id_empresa,$opcGrillaContable,$tablaInventario,$idTablaPrincipal,$link){
		...
		$sqlArticulo ="SELECT id_activo_fijo,codigo,nombre,unidad,costo,dias_depreciar,valor
    ...
    $queryActivosDepreciados = $mysql->query($sqlActivosDepreciados,$mysql->link);
		while($row = $mysql->fetch_array($queryActivosDepreciados)){
			if(($row['costo'] - $row['depreciacion_acumulada_niif'] - $row['valor_salvamento']) < $row['valor']){
		...
		if($query){
			...
			if ($consecutivo>0) {
				...
				$sql = "INSERT INTO log_documentos_contables(id_documento,id_usuario,usuario,actividad,tipo_documento,descripcion,id_sucursal,id_empresa,ip,fecha,hora)
								VALUES ($id,'".$_SESSION['IDUSUARIO']."','".$_SESSION['NOMBREUSUARIO']."','Generar','DA','Depreciacion Activos Fijos Niif','".$_SESSION['SUCURSAL']."','".$_SESSION['EMPRESA']."','".$_SERVER['REMOTE_ADDR']."','$fecha_actual','$hora_actual')";
				$query = $mysql->query($sql,$mysql->link);
				echo '<script>
	...
	function moverCuentasDocumento($id_documento,$consecutivo,$fecha,$accion,$id_sucursal,$id_empresa,$mysql){
		if ($accion=='contabilizar') {
			...
			$sql="SELECT
						...
						cuenta,
						descripcion_cuenta
					FROM activos_fijos_cuentas WHERE activo=1 AND id_empresa=$id_empresa AND ($whereIdActivos) AND contabilidad='niif' AND descripcion='depreciacion' ";
			$query=$mysql->query($sql,$mysql->link);
			while ($row=$mysql->fetch_array($query)) {
			...
			$query=$mysql->query($sql,$mysql->link);
			while ($row=$mysql->fetch_array($query)) {
				$arrayCuentasCcosNiif[$row['cuenta']]   = $row['centro_costo'];
			}
			...
			foreach ($arrayActivo as $id_activo => $arrayActivos) {
				foreach ($arrayActivos['cuentas'] as $key => $arrayResult) {
					...
					if($arrayCuentasCcosNiif[$arrayResult['cuenta']] == 'Si' && ($arrayActivos['datos']['id_centro_costos'] == 0 || $arrayActivos['datos']['id_centro_costos'] == '')){
						$sql = "UPDATE activos_fijos_depreciaciones SET estado = 0 WHERE activo = 1 AND id_empresa = $id_empresa AND id = $id_documento";
						$query = $mysql->query($sql,$mysql->link);
						echo '<script>
					...
					$arrayAsientosNiif[$arrayActivos['datos']['id_centro_costos']][$arrayResult['cuenta']][$arrayResult['estado']] += $arrayActivos['datos']['valor'];
				}
			}
			...
			foreach ($arrayAsientosNiif as $id_centro_costos => $arrayAsientosNiifCuentas) {
				foreach ($arrayAsientosNiifCuentas as $cuenta => $arrayAsientosNiifpResul) {
					foreach ($arrayAsientosNiifpResul as $caracter => $saldo) {
						...
						$id_centro_costos_insert =($arrayCuentasCcosNiif[$cuenta]=='Si')? $id_centro_costos : 0 ;
						...
						$valueInsertAsientosNiif .= "('$id_documento',
													...
													'$consecutivo',
													'DA',
													'Depreciacion Activos Fijos Niif',
													'".$fecha."',
													'".$debito."',
													'".$credito."',
													'".$cuenta."',
	...
	function modificarDocumentoGenerado($idDocumento,$opcGrillaContable,$id_empresa,$id_sucursal,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$carpeta,$mysql){
		...
		$sql   = "INSERT INTO log_documentos_contables(id_documento,id_usuario,usuario,actividad,tipo_documento,descripcion,id_sucursal,id_empresa,ip,fecha,hora)
						  VALUES ($idDocumento,'".$_SESSION['IDUSUARIO']."','".$_SESSION['NOMBREUSUARIO']."','Editar','DA','Depreciacion Activos Fijos Niif','$id_sucursal','$id_empresa','".$_SERVER['REMOTE_ADDR']."','$fecha_actual','$hora_actual')";
		$query = $mysql->query($sql,$mysql->link);
	...
	function cancelarDocumento($id,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$id_sucursal,$id_empresa,$mysql){
		...
		$consecutivo = $mysql->result($query,0,'consecutivo_niif');
		...
		if (!$query) {
			...
			$sql = "INSERT INTO log_documentos_contables(id_documento,id_usuario,usuario,actividad,tipo_documento,descripcion,id_sucursal,id_empresa,ip,fecha,hora)
					VALUES ($id,'".$_SESSION['IDUSUARIO']."','".$_SESSION['NOMBREUSUARIO']."','Cancelar','DA','Depreciacion Activos Fijos Niif',$id_sucursal,'".$_SESSION['EMPRESA']."','".$_SERVER['REMOTE_ADDR']."','$fecha_actual','$hora_actual')";
			$query=$mysql->query($sql,$mysql->link);
			echo'<script>
	...
	function calculaValorDepreciacion($opcGrillaContable,$id_activo,$accion,$cont,$id_empresa,$mysql){
		//DATOS PRINCIPALES PARA CALCULAR DEPRECIACION
		$sql = "SELECT
		...
		$fecha_inicio_depreciacion   = $mysql->result($query,0,'fecha_inicio_depreciacion_niif');
		...
		//CALCULAMOS LA DEPRECIACION MINIMA
		$depreciacionMinima = $costo - $valor_salvamento_niif - $depreciacion_acumulada_niif - $deterioro_acumulado;

		//VERIFICAMOS SI EXISTE DETERIORO NIIF
		if($deterioro_acumulado != "" || $deterioro_acumulado != NULL){
			//RECALCULAMOS COSTO
			$costo = ($costo - $valor_salvamento_niif) - ($depreciacion_acumulada_niif + $deterioro_acumulado);

			//RECALCULAMOS VIDA UTIL
			$sql = "SELECT
								AFD.id
							FROM
			...
			$query = $mysql->query($sql,$mysql->link);
			$periodos = $mysql->num_rows($query);
			$vida_util_niif = $vida_util_niif - $periodos;
		}
		else{
			$costo = $costo - $valor_salvamento_niif;
		}

		if($metodo_depreciacion_niif == 'linea_recta'){															// DEPRECIACION LINEA RECTA
			$depreciacionMes = round(($costo / $vida_util_niif),$_SESSION['DECIMALESMONEDA']);
		}
		else if($metodo_depreciacion_niif == 'reduccion_saldos'){										// DEPRECIACION REDUCCION DE SALDOS
			$tasaDepreciacion = 1 - (POW(($valor_salvamento_niif / $costo),(1 / $vida_util_niif)));
			...
			$sumaDigitos     = round(($vida_util_niif * (($vida_util_niif + 1) / 2)),$_SESSION['DECIMALESMONEDA']);
			$factor          = ($vida_util_niif - $diferenciaFecha->y) / $sumaDigitos;
			$depreciacionMes = round(($costo * $factor) / 12,$_SESSION['DECIMALESMONEDA']);
		}

		//VALIDAMOS SI LA DEPRECIACION MINIMA ES INFERIOR A LA DEPRECIACION DEL MES
		if($depreciacionMinima < $depreciacionMes){
			$depreciacionMes = $depreciacionMinima;
		}

		//MOSTRAMOS EL VALOR DE LA DEPRECIACION
		if($accion == 'mostrar'){
			echo '<script>
