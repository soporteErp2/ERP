<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//QUERY
			...
			$grilla->MyWhere			 = "activo = 1  AND id_sucursal = $id_sucursal AND id_empresa = $id_empresa AND (id in(SELECT $idTablaPrincipal FROM $tablaInventario WHERE activo = 1) OR consecutivo != '')";	//WHERE DE LA CONSULTA A LA TABLA "$TableName"
...
if(!isset($opcion)){  ?>
	...
	<script>
      ...
      , 	titulo	= 'Traslado';
      if(estado == 'images/estado_doc/0.png'){
				direccionRender = 'traslados/grilla/grillaContable.php';
			} else{
				direccionRender = 'traslados/bd/grillaContableBloqueada.php';
			}
	    ...
	    Ext.get("contenedor_<?php echo $opcGrillaContable; ?>").load({
			...
			consecutivoDoc = document.getElementById('div_<?php echo $opcGrillaContable; ?>_consecutivo_'+id).innerHTML;
	 		...
	 		if(consecutivoDoc != ''){ document.getElementById('titleDocumento<?php echo $opcGrillaContable; ?>').innerHTML = titulo+'<br>No. '+consecutivoDoc; }
