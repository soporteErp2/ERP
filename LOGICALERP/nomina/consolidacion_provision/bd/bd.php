<?php
	...
	if (isset($id_planilla)) {
		...
		if ($opc<>'guardarFechaPlanilla') {
			verificaCierre($id_planilla,'nomina_planillas_consolidacion_provision',$id_empresa,$link);
		}
	}
	...
	switch ($opc) {
		...
		case 'updateVacaciones':
			updateVacaciones($vacaciones,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link);
			break;
		...
		case 'guardarInfoVacaciones':
			guardarInfoVacaciones($fecha_inicio_contrato,$fecha_inicio_periodo_vacaciones,$fecha_final_periodo_vacaciones,$fecha_inicio_vacaciones_disfrutadas,$fecha_final_vacaciones_disfrutadas,$id_concepto_vacaciones,$concepto_vacaciones,$base_vacaciones,$dias_vacaciones_disfrutadas,$valor_base_vacaciones,$valor_vacaciones_disfrutadas,$fecha_inicio_labores,$dias_vacaciones_compensadas,$valor_vacaciones_compensadas,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link);
			break;
		...
		case 'calculaValorVacacionesDisfrutadas':
			calculaValorVacacionesDisfrutadas($id_input,$id_concepto_vacaciones,$base_vacaciones,$dias,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link);
			break;
		case 'cambiarTipoPlanilla':
			cambiarTipoPlanilla($saldo_inicial,$id_planilla,$id_empresa,$link);
			break;
	...
	function guardarFechaPlanilla($id_planilla,$opcGrillaContable,$fecha,$campo,$id_empresa,$link){
		...
		if ($campo!='fecha_documento') {
			...
			$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE id_planilla=$id_planilla";
			$query=mysql_query($sql,$link);

			//ELIMINAR LOS EMPLEADOS QUE ESTAN EN LA PLANILLA
			$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados WHERE id_planilla=$id_planilla";
			$query=mysql_query($sql,$link);
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision SET $campo='$fecha' WHERE activo=1 AND id=$id_planilla AND id_empresa=$id_empresa";
		$query=mysql_query($sql,$link);
		if (!$query) {
	...
	function guardarTipoLiquidacion($id_planilla,$id_tipo_liquidacion,$id_empresa,$link){
		$sql="UPDATE nomina_planillas_consolidacion_provision SET id_tipo_liquidacion='$id_tipo_liquidacion' WHERE activo=1 AND id=$id_planilla AND id_empresa=$id_empresa";
		$query=mysql_query($sql,$link);
		if (!$query) {
			echo '<script>alert("Error!\nNo se guardo la fecha, intentelo de nuevo\nSi el problema continua comuniquese con el administrador del sistema");</script>';
		}
	...
	function agregarEmpleado($id_planilla,$id_contrato,$cont,$id_empresa,$link){
		...
		$sql="SELECT fecha_inicio,fecha_final,id_sucursal,saldo_inicial FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query=mysql_query($sql,$link);
		...
		$sql="INSERT INTO nomina_planillas_consolidacion_provision_empleados(id_planilla,id_empleado,tipo_documento,documento_empleado,nombre_empleado,id_contrato,dias_laborados,terminar_contrato,id_sucursal,id_empresa)
		...
		if ($query) {
			...
			// SI LA PLANILLA ES SALDO INICIAL NO SE INSERTAN LOS CONCEPTOS
			if ($saldo_inicial=='false') {
				...
				$sql="INSERT INTO nomina_planillas_consolidacion_provision_empleados_conceptos
						(id_planilla,
						id_empleado,
						id_contrato,
						id_concepto,
	...
	function eliminarEmpleado($cont,$id_contrato,$id_empleado,$id_planilla,$id_empresa,$link){
		...
		$sql="DELETE FROM nomina_planillas_consolidacion_provision_conceptos_deducir WHERE
				activo=1 AND
		...
		$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE
				activo=1 AND
				id_planilla='$id_planilla' AND
				id_empleado='$id_empleado' AND
		...
		if ($query) {
			...
			$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND
					id_planilla = '$id_planilla' AND
					id_empleado = '$id_empleado' AND
					id_contrato = '$id_contrato' AND
					id_empresa = '$id_empresa'";
	...
	function cargarTodosEmpleados($id_planilla,$id_empresa,$sucursal,$link){
		...
		$sql="SELECT dias_liquidacion,fecha_inicio,fecha_final,id_sucursal,saldo_inicial  FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query=mysql_query($sql,$link);
		...
		$sql="SELECT id_contrato,id_empleado,nombre_empleado,documento_empleado FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
		$query=mysql_query($sql,$link);
		$whereIdEmpleados='';
		...
		if ($query) {
			...
			// SI LA PLANILLA NO ES DE SALDO INICIAL SE INSERTAN LOS CONCEPTOS CONSOLIDADOS
			if ($saldo_inicial=='false') {
	...
	function renderizarEmpleados($id_planilla,$id_empresa,$link){
		...
		$sql="SELECT id_contrato,id_empleado,nombre_empleado,documento_empleado,verificado FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
		$query=mysql_query($sql,$link);
		$whereIdEmpleados='';
		while ($row=mysql_fetch_array($query)) {
			$whereIdEmpleados.=' AND id_empleado<>'.$row['id_empleado'].' ';
	...
	function cargarConceptosEmpleado($id_contrato,$id_empleado,$id_planilla,$id_empresa,$link){
		...
		$sql="SELECT estado FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query=mysql_query($sql,$link);
		...
		$sql="SELECT id,dias_laborados, terminar_contrato,id_motivo_fin_contrato,motivo_fin_contrato,nombre_empleado,vacaciones,observaciones
				FROM nomina_planillas_consolidacion_provision_empleados
				WHERE
				activo=1
				AND id_empresa=$id_empresa
		...
		$sql="SELECT * FROM nomina_planillas_consolidacion_provision_empleados_conceptos
				WHERE activo=1
				AND	id_contrato='$id_contrato'
				AND id_empleado='$id_empleado'
				AND id_planilla='$id_planilla'
		...
		$select_vacaciones=($estado==1 || $estado==3)? '<input type="text" readonly value="'.$vacaciones.'">' : '<select id="vacaciones" onchange="updateVacaciones(this.value,\''.$id_empleado.'\',\''.$id_contrato.'\')">
                	        																							    <option value="No">No</option>
                	        																							    <option value="Si">Si</option>
                	        																							</select>' ;

		$observacion_empleado=($estado==1 || $estado==3)? $observaciones : '<textarea id="observacionEmpleado" style="width: 100%;height: 100%;padding:0px;" onkeydown="inputObservacionEmpleadoPlanillaConsolidacionProvision(event,this,'.$id_registro.')">'.$observaciones.'</textarea>' ;
		...
		while ($row=mysql_fetch_array($query)) {
            ...
            $campoValorConcepto='readonly';
     		...
     		$camposBody=($row['naturaleza']=='Provision')?
						...
						<input type="text" style="width:calc(100% - 30px - 90px);padding-left: 0px;/*margin-left: -3px;*/" name="valor_concepto_'.$row['id_concepto'].'"  id="valor_concepto_'.$cont.'" value="'.$row['valor_concepto_ajustado'].'" '.$evento_input.' '.$readonly.'>'
     					:
     					'<div style="float:left;width:25px;height:100%;border-right:1px solid #d4d4d4;text-align:right;padding-right:5px">
							'.$row['dias_laborados'].'
						</div>
						<div style="float:left;width:85px;height:100%;border-right:1px solid #d4d4d4;text-align:right;padding-right:5px"></div>
						<input type="text" style="width:calc(100% - 30px - 90px);padding-left: 0px;/*margin-left: -3px;*/" name="valor_concepto_'.$row['id_concepto'].'"  id="valor_concepto_'.$cont.'" value="'.$row['valor_concepto'].'" '.$evento_input.' '.$readonly.'>';
		...
		$bodyConceptos.='</div>
							...
							<div class="contenedorConceptos" style="height:auto;margin-top:10px;width: 50%;">
								...
								<div class="bodyDivNominaPlanilla" style="background-color:#FFF;border-bottom:none;width:100%;float: left;">
									<div class="campoHeadConceptos" style="padding-right:5px;padding-left:5px;text-align: center;background-color : #F3F3F3;">Total  Empleado</div>
	...
	function eliminarConcepto($id_concepto,$id_contrato,$id_empleado,$cont,$id_planilla,$naturaleza,$valor_concepto,$id_empresa,$link){
		...
		$sql="SELECT codigo_concepto FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE activo= 1 AND
		...
		$sql="SELECT concepto FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE activo= 1 AND
		...
		$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE activo=1 AND
																id_concepto = '$id_concepto' AND
																id_contrato = '$id_contrato' AND
																id_empleado = '$id_empleado' AND
																id_planilla = '$id_planilla' AND
		...
		$query=mysql_query($sql,$link);
		if ($query) {
			$sql="DELETE FROM nomina_planillas_consolidacion_provision_conceptos_deducir
					WHERE
					activo=1
	...
	function guardarConcepto($input_calculo,$formula,$nivel_formula_concepto,$id_concepto,$id_contrato,$id_empleado,$cont,$id_planilla,$valor_concepto,$naturaleza,$id_empresa,$link){
		...
		//$sql="INSERT INTO nomina_planillas_consolidacion_provision_empleados_conceptos (id_planilla,id_empleado,id_contrato,id_concepto,valor_concepto,id_empresa) VALUES ('$id_planilla','$id_empleado','$id_contrato','$id_concepto','$valor_concepto','$id_empresa')";
		$query=mysql_query($sql,$link);
		if ($query) {
	...
	function actualizarconcepto($id_insert,$input_calculo,$formula,$nivel_formula_concepto,$id_concepto,$id_contrato,$id_empleado,$cont,$id_planilla,$valor_concepto,$naturaleza,$id_empresa,$link){
		...
		$sql="SELECT valor_concepto,valor_concepto_ajustado,id_prestamo,id_empleado FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE id_concepto = '$id_concepto' AND
						id_contrato = '$id_contrato' AND
						id_empleado = '$id_empleado' AND
						id_planilla = '$id_planilla' AND
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision_empleados_conceptos SET $campoUpdate saldo_restante='$valor_concepto', valor_campo_texto='$input_calculo'
	...
	function updateFinalizaContrato($terminar_contrato,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link){
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision_empleados SET terminar_contrato='$terminar_contrato' $campo_update WHERE
				id_empleado = '$id_empleado' AND
				id_contrato = '$id_contrato' AND
				id_planilla = '$id_planilla' AND
				id_empresa  = '$id_empresa' ";
	...
	function updateMotivoFinContrato($id_motivo_finalizacion,$motivo_finalizacion,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link){
		$sql="UPDATE nomina_planillas_consolidacion_provision_empleados SET id_motivo_fin_contrato=$id_motivo_finalizacion,motivo_fin_contrato='$motivo_finalizacion' WHERE
				id_empleado = '$id_empleado' AND
				id_contrato = '$id_contrato' AND
				id_planilla = '$id_planilla' AND
				id_empresa  = '$id_empresa' ";
	...
	function updateDiasLaborados($dias,$id_empleado,$id_contrato,$id_planilla,$id_empresa,$link){
		...
		$sql="SELECT dias_liquidacion FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query=mysql_query($sql,$link);
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision_empleados SET dias_laborados='$dias' WHERE
				id_empleado = '$id_empleado' AND
				id_contrato = '$id_contrato' AND
				id_planilla = '$id_planilla' AND
				id_empresa  = '$id_empresa' ";
	...
	function verificaEmpleado($check,$id_contrato,$id_empleado,$id_planilla,$cont,$id_empresa,$link){
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision_empleados SET verificado='$valor' WHERE
				id_empleado = '$id_empleado' AND
				id_contrato = '$id_contrato' AND
				id_planilla = '$id_planilla' AND
				id_empresa  = '$id_empresa' ";
	...
	function ventanaConfigurarCuentasConcepto($id_planilla,$id_concepto,$id_contrato,$id_empleado,$id_empresa,$link){
		...
		$sql="SELECT concepto,
					...
					descripcion_cuenta_niif_liquidacion,
					naturaleza
					FROM nomina_planillas_consolidacion_provision_empleados_conceptos
					WHERE activo=1 AND
					id_planilla = '$id_planilla' AND
					id_concepto = '$id_concepto' AND
					id_contrato = '$id_contrato' AND
	...
	function updateCuentasConcepto($id_cuenta_colgaap,$id_cuenta_niif,$id_cuenta_contrapartida_colgaap,$id_cuenta_contrapartida_niif,$id_concepto,$id_contrato,$id_empleado,$id_planilla,$id_empresa,$link){
		...
		$sql="SELECT naturaleza FROM nomina_planillas_consolidacion_provision_empleados_conceptos
				WHERE activo=1 AND
				id_concepto = '$id_concepto' AND
				id_contrato = '$id_contrato' AND
				id_empleado = '$id_empleado' AND
		...
		if ($naturaleza=='Provision') {
			$sql  ="UPDATE nomina_planillas_consolidacion_provision_empleados_conceptos
						SET
	...
	function calculaValorConceptoFormulaInput($id_insert_concepto,$id_planilla,$id_concepto,$id_contrato,$id_empleado,$cont,$variable,$id_empresa,$link){
		...
		if ($id_insert_concepto>0) {
			...
			$sql="SELECT dias_laborados FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla AND id_empleado=$id_empleado ";
			$query=mysql_query($sql,$link);
			$dias_laborados=mysql_result($query,0,'dias_laborados');

			// CONSULTAR LA FORMULA DEL CONCEPTO
			$sql="SELECT formula_original FROM nomina_planillas_consolidacion_provision_empleados_conceptos
			...
			if ($search_var_concepto===false) {
				...
				$sql="UPDATE nomina_planillas_consolidacion_provision_empleados_conceptos SET formula='$formula',valor_concepto=$valor_concepto,valor_campo_texto=$variable
						WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla AND id_concepto=$id_concepto AND id_contrato=$id_contrato AND id_empleado=$id_empleado";
				$query=mysql_query($sql,$link);
				if (!$query) {
			...
			// SI EXISTEN VARIABLES DE OTROS CONCEPTOS, CONSULTAR EN LA BASE DE DATOS Y REEMPLAZAR
				...
				$sql="SELECT id_concepto,codigo_concepto,concepto,valor_concepto FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE activo=1
										AND id_planilla = '$id_planilla'
										AND id_empleado = '$id_empleado'
										AND id_contrato = '$id_contrato'
										AND id_empresa  = '$id_empresa'";
	...
	function recalcularValoresConceptos($id_planilla,$id_concepto,$id_contrato,$id_empleado,$id_empresa,$link){
		...
		if ($id_concepto=='DL') {
			...
			$sql="SELECT id FROM nomina_planillas_consolidacion_provision_empleados_conceptos
			...
			$sql="SELECT codigo_concepto,nivel_formula FROM nomina_planillas_consolidacion_provision_empleados_conceptos
		...
		$sql="SELECT dias_laborados FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla AND id_empleado=$id_empleado AND id_contrato=$id_contrato";
		$query=mysql_query($sql,$link);
		$dias_laborados=mysql_result($query,0,'dias_laborados');
		...
		foreach ($arrayConceptos as $nivel_formula => $arrayConceptosArray) {
			...
			foreach ($arrayConceptosArray as $id_concepto_array => $arrayConceptosResul) {
				...
				if ($search_var_concepto===false) {
					...
					$sql="UPDATE nomina_planillas_consolidacion_provision_empleados_conceptos SET
	...
	function actualizarSucursal($id_planilla,$sucursal,$id_empresa,$link){
		$sql="UPDATE nomina_planillas_consolidacion_provision SET id_sucursal=$sucursal WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla ";
		$query=mysql_query($sql,$link);
		if (!$query) {
	...
	function eliminarConceptoDeducir($cont_deducir,$id_empleado,$id_concepto,$id_prestamo,$id_concepto_deducir,$id_planilla,$id_contrato,$id_empresa,$link){
		echo$sql="DELETE FROM nomina_planillas_consolidacion_provision_conceptos_deducir
				WHERE activo=1
				AND id_empleado         = '$id_empleado'
				AND id_concepto         = '$id_concepto'
	...
	function buscarEmpleadoCargado($id_planilla,$filtro,$estado,$id_empresa,$link){
	    ...
	    $sql='SELECT id_empleado,documento_empleado,nombre_empleado,id_contrato,verificado,email_enviado
	    		FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_empresa='.$id_empresa.' AND id_planilla='.$id_planilla.' '.$filtro;
	    $query=mysql_query($sql,$link);
	    $bodyEmpleados='';
	    $cont=1;
	    while ($row=mysql_fetch_array($query)) {
	...
	function guardarObservacionEmpleado($observacion,$id_planilla,$id,$id_empresa,$link){
		...
		$sql   = "UPDATE nomina_planillas_consolidacion_provision_empleados SET  observaciones='$observacion' WHERE id_planilla='$id_planilla' AND id=$id AND id_empresa=$id_empresa";
		$query = mysql_query($sql,$link);
	...
	// ==================== ACTUALIZAR LA PLANILLA SI ES O NO DE SALDO INICIAL ===============//
	function cambiarTipoPlanilla($saldo_inicial,$id_planilla,$id_empresa,$link){
		//ELIMINAR LOS CONCEPTOS QUE ESTAN EN LA PLANILLA
		$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE id_planilla=$id_planilla AND id_empresa=$id_empresa";
		$query=mysql_query($sql,$link);

		//ELIMINAR LOS EMPLEADOS QUE ESTAN EN LA PLANILLA
		$sql="DELETE FROM nomina_planillas_consolidacion_provision_empleados WHERE id_planilla=$id_planilla AND id_empresa=$id_empresa";
		$query=mysql_query($sql,$link);
		...
		$sql="UPDATE nomina_planillas_consolidacion_provision SET saldo_inicial='$saldo_inicial' WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla ";
		$query=mysql_query($sql,$link);

		if (!$query) {
			echo '<script>
					alert("Error!\nNo se actualizo el tipo de planilla, intentelo de nuevo\nSi el problema continua comuniquese con el administrador del sistema");
					document.getElementById(\'planilla_saldo_inicial\').value=("'.$saldo_inicial.'"==\'true\')? \'false\' : \'true\' ;
				</script>';
		}
		else{
			echo $script;
    ...
    function terminarGenerar($id_planilla,$id_empresa,$id_sucursal,$link){
    	...
    	// CONSULTAR SI LA PLANILLA ES DE SALDO INICIAL
    	$sql="SELECT saldo_inicial FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
    	$query=mysql_query($sql,$link);
    	$saldo_inicial = mysql_result($query,0,'saldo_inicial');

    	// CONSULTAR SI LA LIQUIDACION TIENE CONCEPTOS TIPO DEDUCCION Y SI ESTAN TOTALMENTE CONFIGURADOS
    	$sql="SELECT id_empleado,documento_empleado,nombre_empleado FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_planilla=$id_planilla AND id_empresa=$id_empresa ";
    	$query=mysql_query($sql,$link);
    	while ($row=mysql_fetch_array($query)) {
    	...
    	$sql="SELECT
			...
			FROM
				nomina_planillas_consolidacion_provision_empleados_conceptos AS NPLEC,
    	...
    	while ($row=mysql_fetch_array($query)){
    		...
    		if ($naturaleza=='Provision') {
				...
				if ($valor_concepto==$valor_concepto_ajustado) {
					...
					$arrayNominaContabilizacion[$id_tercero_cruce_liquidacion][$cuenta_colgaap_liquidacion]['credito'] += ($valor_concepto_ajustado-$arrayDeduccionescolgaap[$id_empleado][$id_concepto][$cuenta_colgaap_liquidacion]['valor_deducir']);
    	...
    	if ($cont_conceptos==0) {
    		echo '<script>
					alert("Aviso!\nNo hay conceptos en la planilla de Consolidacion!");
					document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
				</script>';
			exit;
    	}
    	...
    	$sql="UPDATE nomina_planillas_consolidacion_provision SET estado=1,fecha_generacion='$fecha_generacion' WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query=mysql_query($sql,$link);
		if (!$query) {
			echo '<script>
		...
		$sql   = "SELECT consecutivo,fecha_documento,fecha_inicio,fecha_final,id_sucursal FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
		$query = mysql_query($sql,$link);
    		...
    		foreach ($arrayResul as $cuenta => $arrayResul1) {
    			...
    			if ($arrayResul1['debito']>0) {
    				$valueInsertAsientos .= "('$id_planilla',
										...
										'$consecutivo',
										'PCP',
										'Planilla Consolidacion Provision',
										'".$fecha_documento."',
										'".$arrayResul1['debito']."',
										'0',
										'".$cuenta."',
    	...
    	if (($acumDebito-$acumCredito)!=0) {
    		$sql="UPDATE nomina_planillas_consolidacion_provision SET estado=0  WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
			$query=mysql_query($sql,$link);
    		echo '<script>
		...
		$queryColgaap=($saldo_inicial=='true')? true : mysql_query($sqlColgaap,$link);
		...
		$queryNiif=($saldo_inicial=='true')? true : mysql_query($sqlNiif,$link);
		...
		echo '<script>
				Ext.get("contenedor_PlanillaConsolidacionProvision").load({
					...
					params  :
					{
						...
						opcGrillaContable : "PlanillaConsolidacionProvision",
					}
				});
				document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
			</script>';
    ...
    function moverSaldoDiasPlanillas($accion,$id_planilla,$id_empresa,$link){
    	...
    	$sql="SELECT fecha_inicio,fecha_final FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_planilla";
    	$query=mysql_query($sql,$link);
    	...
    	// $sql="SELECT id_empleado FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_planilla=$id_planilla AND id_empresa=$id_empresa";
    ...
    function administrarContratos($accion,$id_planilla,$fecha_final,$id_empresa,$link){
    	// ACTUALIZAR LOS REGISTROS DE LS VACACIONES DE LA PLANILLA
			...
			$sql="UPDATE empleados_contratos AS EC,
					(
					    SELECT
							...
							motivo_fin_contrato,
							IF(terminar_contrato='Si' OR vacaciones='Si',IF(terminar_contrato='Si',1,2),0) AS estado_contrato
						FROM
							nomina_planillas_consolidacion_provision_empleados
						WHERE
							activo      = 1 AND
							id_planilla = $id_planilla
							AND id_empresa  = $id_empresa
					...
					SET EC.estado=NPE.estado_contrato,EC.fecha_cancelacion='$fecha_final',EC.id_motivo_cancelacion=NPE.id_motivo_fin_contrato,EC.motivo_cancelacion=NPE.motivo_fin_contrato
					WHERE EC.activo=1
    ...
    function moverCuentasPlanilla($accion,$id_planilla,$id_empresa,$link){
    	if ($accion=='eliminar') {
    		//ASIENTOS COLGAAP
    		$sql="DELETE FROM nomina_planillas_empleados_contabilizacion WHERE id_empresa='$id_empresa' AND id_planilla='$id_planilla' AND tipo_planilla='PCP' ";
    		$queryColgaap=mysql_query($sql,$link);
    		//ASIENTOS COLGAAP
    		$sql="DELETE FROM asientos_colgaap WHERE id_empresa='$id_empresa' AND id_documento='$id_planilla' AND tipo_documento='PCP'";
    		$queryColgaap=mysql_query($sql,$link);
    		//ASIENTOS NIIF
    		$sql="DELETE FROM asientos_niif WHERE id_empresa='$id_empresa' AND id_documento='$id_planilla' AND tipo_documento='PCP'";
    ...
    function cancelarPlanillaNomina($id_planilla,$id_empresa,$link){
    	...
    	$sql="SELECT estado FROM nomina_planillas_consolidacion_provision WHERE activo=1 AND id='$id_planilla' AND id_empresa='$id_empresa'";
   		$query=mysql_query($sql,$link);
   		$estado=mysql_result($query,0,'estado');
   		if ($estado==3) {
   			echo '<script>
   	...
   	function calcularValoresPlanilla($id_planilla,$id_empresa,$link){
   		$sql="SELECT valor_concepto,valor_concepto_ajustado,naturaleza FROM nomina_planillas_consolidacion_provision_empleados_conceptos WHERE activo=1 AND id_planilla=$id_planilla AND id_empresa=$id_empresa";
   		$query=mysql_query($sql,$link);
   		$acumDevengo=0;
   		$acumDeduce=0;
   		$acumApropiacion=0;
	...
	function guardarObservacion($observacion,$id,$id_empresa,$link){
		...
		$observacion=str_replace("[\n|\r|\n\r]", '<br>', $observacion);
		$sql   = "UPDATE nomina_planillas_consolidacion_provision SET  observacion='$observacion' WHERE id='$id' AND id_empresa=$id_empresa";
		$query = mysql_query($sql,$link);
		if($query){ echo 'true'; }
		else{ echo'false'; }
	}
	...
	function enviarPlanillaNomina($id_planilla,$id_empresa,$link){
		...
		$sql="SELECT id_empleado,id_contrato,nombre_empleado FROM nomina_planillas_consolidacion_provision_empleados WHERE activo=1 AND id_planilla=$id_planilla AND id_empresa=$id_empresa";
		$query=mysql_query($sql,$link);
		while ($row=mysql_fetch_array($query)) {
	...
	function enviarVolanteUnicoEmpleado($id_planilla,$id_contrato,$id_empleado,$id_empresa,$link){
		...
		if ($fun['return']=='true') {
			$sql="UPDATE nomina_planillas_consolidacion_provision_empleados SET email_enviado='true' WHERE activo=1 AND id_planilla=$id_planilla AND id_contrato=$id_contrato AND id_empleado=$id_empleado AND id_empresa=$id_empresa";
			$query=mysql_query($sql,$link);
	...
	function validaDocumentoCruce($idDocumento,$id_empresa,$link){
		$id_sucursal=$_SESSION['SUCURSAL'];
		$sqlNota    = "SELECT consecutivo_documento, tipo_documento FROM asientos_colgaap WHERE id_documento_cruce = '$idDocumento' AND tipo_documento_cruce='PCP' AND tipo_documento<>'PCP' AND activo=1 AND id_empresa = '$id_empresa' AND id_sucursal = '$id_sucursal' GROUP BY id_documento, tipo_documento";
