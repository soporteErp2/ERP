...
<?php
    ...
    $acumScript .= (user_permisos(164,'false') == 'true')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").enable();' : '';
    $acumScript .= (user_permisos(165,'false') == 'true')? 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").enable();'   : '';
    $acumScript .= (user_permisos(166,'false') == 'true')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").enable();' : '';
    $acumScript .= (user_permisos(167,'false') == 'true')? 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").enable();' : '';
    ...
    if($id_planilla==''){
        ...
        $acumScript .=  'new Ext.form.DateField({
                        ...
                        Inserta_Div_nomina_planillas_ajuste('.$id_planilla.');';
    }
    //============================ SI EXISTE LA PLANILLA =============================//
        ...
        if ($consecutivo!='') {
            $acumScript.='document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML="Planilla de Ajuste de Nomina <br>N. '.$consecutivo.'";';
        }
        ...
        $sql="SELECT id_empleado,documento_empleado,nombre_empleado,id_contrato,verificado FROM nomina_planillas_ajuste_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
        $query=mysql_query($sql,$link);
        $bodyEmpleados='';
        $cont=1;
        while ($row=mysql_fetch_array($query)) {
    ...
    <div class="bodyArticulos" id="bodyArticulos<?php echo $opcGrillaContable; ?>">
        <div class="renderFilasArticulo" id="renderizaNewArticulo<?php echo $opcGrillaContable; ?>">
            <div class="contenedorEmpleados">
                ...
                <div onclick="ventanaBuscarEmpleados()" title="Buscar" class="iconBuscar">
            ...
            <div class="contenedor_totales">
                ...
                <div class="contenedorConceptos" style="height:auto;margin-top:10px;width:auto;">
                    <div class="bodyDivNominaPlanilla" style="background-color:#FFF;">
                        <div class="campoHeadConceptos" style="text-align: center;background-color : #F3F3F3;border-right:none;width:calc(100% - 20px - 20px);">TOTAL PLANILLA DE AJUSTE</div>
                        <div style="float:left;width:20px;height:22px;background-color : #F3F3F3;overflow:hidden;" id="divLoadTotalesPlanilla"></div>
                        <div class="button_options_nomina" title="Ver en detalle" onclick="ventana_ver_detalle('<?php echo $id_planilla ?>',0,0,'global');"></div>
                        <!-- <img src="../../../temas/clasico/images/BotonesTabs/buscar_doc_detail.png" style="width:15px;right:0px;"> -->
                    </div>
                ...
                <!-- <div class="button-options" title="Ver en detalle" onclick="ventana_ver_detalle('<?php echo $id_planilla ?>','0','0','global');">
                    Valores detallados
                </div> -->
            </div>
...
<script>
            ...
            [
                {
                    ...
                    [
                            ...
                            [
                                {
                                    ...
                                    autoLoad    :
                                    {
                                        ...
                                        {
                                            renderizaBody : 'true',
                                            url_render    : '<?php echo $carpeta; ?>/bd/grillaBuscarEmpleados.php',
                                            opc           : 'buscar_empleado',
                        ...
                        },
                        {
                            ...
                            width     : 60,
                            height    : 56,
                            text      : 'Empleados con<br>contrato',
                            scale     : 'large',
                            iconCls   : 'doc_con_contrato',
                            iconAlign : 'top',
                            handler   : function(){ cargaEmpleadosEstadoContratos(0) }
                        },
                        {
                            xtype     : 'button',
                            id        : 'btn_sin_contrato',
                            width     : 60,
                            height    : 56,
                            text      : 'Empleados sin<br>contrato',
                            scale     : 'large',
                            iconCls   : 'doc_sin_contrato',
                            iconAlign : 'top',
                            handler   : function(){ cargaEmpleadosEstadoContratos(1) }
                        },
                        {
                            xtype       : 'button',
                            width       : 60,
    ...
    function cargaEmpleadosEstadoContratos(estado) {
        var sucursal=document.getElementById('id_sucursal').value;
        if (estado==1) {
            Ext.getCmp('btn_sin_contrato').hide();
            Ext.getCmp('btn_con_contrato').show();
        }
        else{
            estado = '';
            Ext.getCmp('btn_con_contrato').hide();
            Ext.getCmp('btn_sin_contrato').show();
        }

        // console.log(Ext.getCmp('btn_sin_contrato'));

        Ext.get('contenedor_buscar_empleado').load({
            url     : '<?php echo $carpeta; ?>/bd/grillaBuscarEmpleados.php',
            scripts : true,
            nocache : true,
            params  :
            {
                ...
                opcGrillaContable :'buscar_empleado',
                cargaFuncion      : 'guardarEmpleado(id)',
                id_planilla       : "<?php echo $id_planilla; ?>",
                sql_estado        : estado,
            }
        });
    ...
    function guardarEmpleado(id_contrato) {
        var  btn_con_contrato = Ext.getCmp('btn_con_contrato').hidden
        ,   opc_contrato =(btn_con_contrato==false)? 'sin_contrato' : '';
    ...
    function ventanaBuscarConceptos(cont) {
        ...
        // var id_contrato = document.getElementById('id_contrato_concepto_'+cont).value;
         ...
         Win_Ventana_bucar_concepto = new Ext.Window({
             ...
             autoLoad    :
             {
                 url     : '<?php echo $carpeta; ?>/bd/grillaBuscarConceptos.php',
                 scripts : true,
                 nocache : true,
                 params  :
                 {
    ...
    function ventanaConfigurarConceptoDeduccion(cont){
    ...
    function ventanaBuscarConceptosConceptoDeduccion(cont,id_empleado,id_contrato) {
        Win_Ventana_ventana_buscar_concepto = new Ext.Window({
                ...
                autoLoad    :
                {
                    url     : '<?php echo $carpeta; ?>/bd/buscar_concepto.php',
                    scripts : true,
                    nocache : true,
                    params  :
                    {
    ...
    function responseVentanaBuscarConcepto(id,cont) {
            ...
            var id_buscar = document.getElementById('id_concepto_buscar_'+id).value;
    ...
    function guardarConceptoDeducir(cont_deducir,cont,id_empleado,id_contrato,id_concepto){
    ...
    function eliminarConceptoDeducir(cont_deducir,id_empleado,id_contrato,id_concepto) {
        var id_concepto_deducir =document.getElementById('id_concepto_deducir_'+cont_deducir).value;
    ...
    function guardarPlanilla(){
        ...
        cargando_documentos('Generando Planilla de Ajuste...','');
    ...
    function cancelarPlanillaLiquidacion(){
        ...
        cargando_documentos('Cancelando Planilla de Ajuste...','');
        Ext.get('render_btns_PlanillaAjusteNomina').load({
            url     : '<?php echo $carpeta; ?>/bd/bd.php',
            scripts : true,
            nocache : true,
    ...
    function  descargar_plantilla() {
        window.open('<?php echo $carpeta; ?>/bd/exportar_plantilla_excel.php?id_planilla=<?php echo $id_planilla; ?>');
    }
    ...
    function createUploader(){
        ...
        var uploader = new qq.FileUploader({
            ...
            debug   : false,
            params  : { opcion: 'loadExcelNota', id_planilla: '<?php echo $id_planilla; ?>' },
            button            : null,
            multiple          : false,
            maxConnections    : 3,
