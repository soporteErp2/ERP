<?php
	...
	$sql="SELECT consecutivo,fecha_inicio,fecha_final,estado FROM nomina_planillas_ajuste WHERE activo=1 AND id_empresa=$idEmpresa AND id=$id_planilla";
	...
	$sql="SELECT id_empleado,id_contrato,dias_laborados,terminar_contrato FROM nomina_planillas_ajuste_empleados WHERE activo=1 AND id_empresa=$idEmpresa AND id_planilla=$id_planilla ";
	$query=mysql_query($sql,$link);
	$whereId='';
	...
	$sql="SELECT id_empleado,id_contrato,id_concepto,concepto,valor_concepto,valor_concepto_ajustado,naturaleza FROM nomina_planillas_ajuste_empleados_conceptos WHERE activo=1  AND id_planilla=$id_planilla AND ($whereIdConceptos) ORDER BY naturaleza ASC";
	$query=mysql_query($sql,$link);
	$conceptos     ='';
	$acumValorConcepto   =0;
	$acumValorConceptoAjustado =0;
	while ($row=mysql_fetch_array($query)) {
		...
		foreach ($arrayResul as $id_contrato => $resul) {
			...
			if ($id_empleado_OLD!=$id_empleado) {
					if ($id_empleado_OLD!=0) {
						...
						$contenido.='<tr style="border-top:1px solid;">
									<td>TOTALES</td>
									<td style="text-align:right;">'.number_format ($acumValorConcepto,$_SESSION['DECIMALESMONEDA']).'</td>
									<td style="text-align:right;">'.number_format ($acumValorConceptoAjustado,$_SESSION['DECIMALESMONEDA']).'</td>
								</tr>';
			...
			$contenido.='<table class="articlesTable">
					<thead>
						<tr>
							...
							<td style="width:25%;">VALOR ACUMULADO</td>
							<td style="width:25%">VALOR AJUSTADO</td>
						</tr>
					</thead>
					<tbody>
			...
			foreach ($arrayConceptosEmpleados[$id_empleado][$id_contrato] as $id_concepto => $resulConcept) {
				...
				$contenido.='<tr>
								...
								<td style="text-align:right;">'.$resulConcept['valor_concepto'].'</td>
								<td style="text-align:right;">'.$resulConcept['valor_concepto_ajustado'].'</td>
							</tr>';
				$acumValorConcepto              += $resulConcept['valor_concepto'] ;
				$acumValorConceptoAjustado      += $resulConcept['valor_concepto_ajustado'] ;

				$acumValorConceptoTotal         += $resulConcept['valor_concepto'] ;
				$acumValorConceptoAjustadoTotal += $resulConcept['valor_concepto_ajustado'] ;
				$acumDiferencia = abs($acumValorConcepto - $acumValorConceptoAjustado);
	...
	if ($id_empleado_OLD!=0) {
		...
		// 				<td>NETO EMPLEADO</td>
		// 				<td colspan="2" style="text-align:center;">'.number_format (($acumValorConcepto-$acumValorConceptoAjustado),$_SESSION['DECIMALESMONEDA']).'</td>
		// 			</tr>';
	...
	$header = '<div id="body_pdf" style="width:100%; font-style:normal;">
					...
					<div style="float:left;width:30%;text-align:center;font-size:16px;">
						<b>PLANILLA DE AJUSTE<br> N. '.$consecutivo.'</b><br>
	...
	$contenido.= '
				<table class="articlesTable" style="font-size:12px;margin-top:30px;">
					<thead>
						<tr>
							<td style="width:25%;">TOTAL VALOR ACUMULADO</td>
							<td style="width:25%;">TOTAL VALOR AJUSTADO</td>
							<td style="width:50%">DIFERENCIA - AJUSTE</td>
						</tr>
					</thead>
					<tbody>
						<tr >
							<td style="text-align:center;">'.number_format ($acumValorConceptoTotal,$_SESSION['DECIMALESMONEDA']).'</td>
							<td style="text-align:center;">'.number_format ($acumValorConceptoAjustadoTotal,$_SESSION['DECIMALESMONEDA']).'</td>
							<td style="text-align:center;">'.number_format ($acumDiferencia,$_SESSION['DECIMALESMONEDA']).'</td>
						</tr>
					</tbody>
				</table>
