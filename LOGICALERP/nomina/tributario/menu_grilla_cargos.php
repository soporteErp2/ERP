<?php
	...
	$sql            = "SELECT COUNT(id) as cont FROM empleados_cargos WHERE activo=1 AND id_empresa='$id_empresa'";
	...
	for ($i=1; $i <= $paginas; $i++) {
 	...
 	$sqlCuentas   = "SELECT id,nombre FROM empleados_cargos WHERE activo=1 AND id_empresa = '$id_empresa' LIMIT $limit";
	...
	while ($rowCuentas = mysql_fetch_array($queryCuentas)) {
		...
		$filaInsertBoleta .= '<div class="filaBoleta" id="fila_boleta_'.$opcGrillaContable.'_'.$contFilaCuenta.'">
								...
								<div class="campo2"  id="cargo_'.$rowCuentas['id'].'" title="'.$rowCuentas['nombre'].'" onclick="cargaDefinicionesTributarias(\''.$rowCuentas['id'].'\',\''.$rowCuentas['nombre'].'\')">
									'.$rowCuentas['nombre'].'
								</div>
							</div>';
...
<style>
	#contenedor_formulario_tributario{
		overflow      : hidden;
		width         : 90%;
		height        : 90%;
		margin        : 10% auto;
	...
	.toolbar_grilla_manual_tributario{
		overflow         : hidden;
		width            : 100%;
		height           : 34px;
	...
	.div_input_busqueda_grilla_manual_tributario{
		width            : calc(100% - 44px);
		height           : 28px;
		padding-top      : 5px;
		padding          : 5 0 0 6;
	...
	.div_input_busqueda_grilla_manual_tributario input{
	...
	.div_input_busqueda_grilla_manual_tributario > div{
		float: left;
	}

	.div_img_actualizar_datos_grilla_manual_tributario{
		float  : left;
		width  : 35px;
		height : 40px;
	}

	.div_img_actualizar_datos_grilla_manual_tributario>img{
		padding : 7 0 0 7;
		cursor  : pointer;
	}

	#contenedor_tabla_boletas_tributario{
		overflow              : hidden;
		width                 : calc(100% - 2px);
	...
	.divInputFormulario_tributario{
		float       : left;
		width       : calc(100% - 100px - 20px - 10px);
		margin-left : 5px;
	}

	.divInputFormulario_tributario input, .divInputFormulario_tributario select, .divInputFormulario_tributario textarea{
		min-height            : 22px;
		width                 : 100%;
	...
	#bodyTablaBoletas_tribitario > div{

	}

	/*#bodyTablaBoletas_tribitario > div > div { height: 18px; background-color : #FFF; padding-top: 4px; }*/
	#bodyTablaBoletas_tribitario >  div:hover {}
	...
	.div_img_actualizar_datos_grilla_manual_tributario>img:hover {
...
<div id="contenedor_formulario_tributario">


	<div class="toolbar_grilla_manual_tributario">
		<div class="div_input_busqueda_grilla_manual_tributario">
			<div>
				CARGOS
			</div>
			<div>
				<input type="text" id="inputBuscarGrillaManual" onkeyup="inputBuscarGrillaManual(event,this);" placeholder="Buscar...">
			</div>
		</div>
		<div class="div_img_actualizar_datos_grilla_manual_tributario">
			<img src="img/reload_grilla_new.png" onclick="actualizarDatosGrillaManual();" title="Actualizar Lista">
		</div>

	</div>
	<div id="contenedor_tabla_boletas_tributario">
		<!-- <div class="headTablaBoletas">
			<div class="campo0">&nbsp;</div>
			<div class="campo2" >Cargo</div>
		</div> -->
		<div class="divSeparador"></div>
		<div id="bodyTablaBoletas_tribitario"><?php echo $filaInsertBoleta; ?></div>
...
function pag_Terceros(accion){
	var MyParent = 'bodyTablaBoletas_tribitario';

	var valor = document.getElementById('inputBuscarGrillaManual').value;
	var filtro = (valor!='')?'AND (nombre LIKE "%'+valor+'%" ) ' : '';
...
function cargaDefinicionesTributarias(id,nombre){

	Ext.get('divContenedorDerecho').load({
		url     : 'tributario/panel_contenedor_conceptos_cargos.php',
		scripts : true,
		nocache : true,
...
<script>
	$("#bodyTablaBoletas_tribitario").mCustomScrollbar({
		theme:'3d-dark',
	});
	// console.log($("#contenedor_tabla_boletas_tributario").mCustomScrollbar());
</script>
<?php
	...
	function busquedaTerceroPaginacion($id_documento,$opcGrillaContable,$pagina,$limite,$limit,$rows_registros,$paginas,$imprimeVar,$filtro,$link,$id_empresa){
		...
		if ($filtro!='') {
			$sql="SELECT COUNT(id) as cont  FROM empleados_cargos WHERE activo=1 $filtro AND id_empresa='$id_empresa' ";
			$query=mysql_query($sql,$link);
			$rows_registros=mysql_result($query,0,'cont');
		...
		$sqlCuentas   = "SELECT id,nombre FROM empleados_cargos WHERE activo=1 $filtro AND id_empresa = '$id_empresa' LIMIT $limit";
		$queryCuentas = mysql_query($sqlCuentas,$link);
