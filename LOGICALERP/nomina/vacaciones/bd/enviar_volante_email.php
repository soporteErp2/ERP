...
function imprimirEnviaVolante($id_planilla,$id_empleado,$id_contrato,$mail,$mpdf,$link){
	...
	$sql="SELECT documento_empleado,nombre_empleado,numero_contrato,grupo_trabajo,fecha_inicio_contrato,tipo_contrato
								...
								<div style="float:left; width:90%; margin:0px 5px 0px 10px;">
									<div style="float:left; width:100%;">
										<table style="font-size:12px;">
											...
											<tr><td><b>Fecha Retiro: </b></td><td>'.$fecha_final.'</td></tr>
										</table>
									</div>
								</div>
							</div>';
	...
	$sql="SELECT concepto,valor_concepto,valor_concepto_ajustado,naturaleza,dias_laborados,base
	...
	while ($row=mysql_fetch_array($query)) {
		...
		$conceptos.='<tr>
						<td style="'.$style.'">'.$row['concepto'].'</td>
						<td style="'.$style.'text-align:right;">'.$row['dias_laborados'].'</td>
						<td style="'.$style.'text-align:right;">'.$row['base'].'</td>
