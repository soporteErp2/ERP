...
<?php
    ...
    $sql   = "SELECT consecutivo,
                    ...
                    tipo_liquidacion,
                    sucursal,
                    consecutivo_planilla_liquidacion
                FROM $tablaPrincipal
    ...
    $consecutivo_planilla_liquidacion = mysql_result($query,0,'consecutivo_planilla_liquidacion');
    ...
    $acumScript .= 'document.getElementById("fechaDoc'.$opcGrillaContable.'").value          = "'.$fechaDoc.'";
                    ...
                    //document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML="Planilla de Nomina <br>N. '.$consecutivo.'";
    ...
    $acumScript.=($consecutivo>0)? 'document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML="Planilla de Nomina <br>N. '.$consecutivo.'";' : '' ;

     if ($consecutivo_planilla_liquidacion>0) {
        $acumScript.='document.getElementById("observacionNominaPlanilla").readOnly = true;';
        $divDocsCruce='<div class="renglonTop">
                            <div class="labelTop">Provision de</div>
                            <div class="campoTop" >
                                <div style="float:left;border-right:1px solid #EEE;padding:3px 5px 0px 5px;height:100%;font-weight:bold;" title="Planilla de N.">
                                    LE '.$consecutivo_planilla_liquidacion.'
                                </div>
                            </div>
                        </div>';
    }
    ...
    $sql="SELECT id_empleado,documento_empleado,nombre_empleado,id_contrato,verificado,email_enviado FROM nomina_planillas_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
    $query=mysql_query($sql,$link);
    $bodyEmpleados='';
    $cont=1;
    ...
    if ($estado==1) {
        ...
        $divImagen = '<img src="img/candado44.png" style="float:right; width: 20px; height: 30px; margin:10px 0 10px 2px;" title="Documento Cruzado">';

        // CONSULTAR LSO DOCUMENTOS QUE CRUZA
        $divDocsCruce = '';
        $sql="SELECT id_planilla_cruce,tipo_planilla_cruce FROM nomina_planillas_empleados_conceptos
                WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla GROUP BY id_planilla_cruce,tipo_planilla_cruce";
        $query=mysql_query($sql,$link);
        while ($row=mysql_fetch_array($query)) {
            if ($row['tipo_planilla_cruce']=='LE') {
                $sql="SELECT consecutivo,estado FROM nomina_planillas_liquidacion WHERE activo=1 AND estado=1 AND id_empresa=$id_empresa AND id=$row[id_planilla_cruce]";
                $query2=mysql_query($sql,$link);
                $consecutivo = mysql_result($query2,0,'consecutivo');
                $estado = mysql_result($query2,0,'estado');
                if ($estado<>1) { continue; }
                $divLE.='<div style="float:left;border-right:1px solid #EEE;padding:3px 5px 0px 5px;height:100%;font-weight:bold;" title="Planilla de N.">LE - '.mysql_result($query2,0,'consecutivo').'</div>';
            }
            else if ($row['tipo_planilla_cruce']=='PCP') {
                $sql="SELECT consecutivo,estado FROM nomina_consolidacion_provision WHERE activo=1 AND estado=1 AND id_empresa=$id_empresa AND id=$row[id_planilla_cruce]";
                $query2=mysql_query($sql,$link);
                $consecutivo = mysql_result($query2,0,'consecutivo');
                $estado = mysql_result($query2,0,'estado');
                if ($estado<>1) { continue; }
                $divPCP.='<div style="float:left;border-right:1px solid #EEE;padding:3px 5px 0px 5px;height:100%;font-weight:bold;" title="Planilla de N.">PCP - '.mysql_result($query2,0,'consecutivo').'</div>';
            }
        }

        if ($divLE<>'' || $divPCP<>'') {
            $divDocsCruce='<div class="renglonTop">
                                <div class="labelTop">Documentos Cruzados</div>
                                <div class="campoTop" >
                                    '.$divLE.$divPCP.'
                                </div>
                            </div>';
    ...
    $acumScript .= (user_permisos(96,'false') == 'false')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").disable();' : '';
    $acumScript .= (user_permisos(97,'false') == 'false')? 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").disable();'   : '';
    $acumScript .= (user_permisos(98,'false') == 'false')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").disable();' : '';
    $acumScript .= (user_permisos(99,'false') == 'false')? 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").disable();' : '';
...
<div class="contenedorOrdenCompra" style="height:100%;" id="contenedor_NominaPlanilla">
...
<script>
    ...
    function nuevaPlanillaNomina(){}

    function cancelarPlanillaNomina(){
        ...
        cargando_documentos('Cancelando Planilla de Nomina...','');
        Ext.get('render_btns_NominaPlanilla').load({
            url     : 'planilla/bd/bd.php',
            scripts : true,
            nocache : true,
            params  :
    ...
    function imprimirPlanillaNomina(){
        window.open("planilla/bd/imprimirPlanilla.php?id_planilla=<?php echo $id_planilla; ?>");
    }

    function modificarDocumentoPlanillaNomina(){
        cargando_documentos('Editando Planilla de Nomina...','');
        Ext.get('render_btns_NominaPlanilla').load({
            url     : 'planilla/bd/bd.php',
            scripts : true,
            nocache : true,
    ...
    function cerrarPlanilla(opc){
        ...
        if (opc=='delete') { Elimina_Div_nomina_planillas(<?php echo $id_planilla ?>); }
        else{ Actualiza_Div_nomina_planillas(<?php echo $id_planilla ?>); }
    }

    function restaurarPlanillaNomina(){
        cargando_documentos('Restaurando Planilla de Nomina...','');
        Ext.get('render_btns_NominaPlanilla').load({
            url     : 'planilla/bd/bd.php',
            scripts : true,
            nocache : true,
    ...
    function enviarPlanillaNomina(){
        var contenido = '<div id="modal">'+
                            '<div id="experiment" style="position:static !important;">'+
                                ...
                                '<div id="LabelCargando">Enviando Volantes de Nomima...</div>'+
                            '</div>'+
                        '</div>';
    ...
    function enviarVolanteUnicoEmpleado(id_contrato,id_empleado,nombre_empleado) {
        var contenido = '<div id="modal">'+
                            '<div id="experiment" style="position:static !important;">'+
                                '<div style="text-align:center;"><img style="cursor:pointer;" src="img/email.gif"></div>'+
                                '<div id="LabelCargando">Enviando Volante de Nomima a <br>'+nombre_empleado+'...</div>'+
                            '</div>'+
                        '</div>';
    ...
    function imprimirVolanteUnicoEmpleado(id_empleado) {
       window.open('planilla/bd/imprimirVolanteEmpleado.php?id_planilla=<?php echo $id_planilla; ?>&id_empleado='+id_empleado);
    }
    ...
    function generarArchivoPlanoBancolombia(){
      id_planilla = <?php echo $id_planilla; ?>;

      var data =  `opc=generarArchivoPlanoBancolombia
  								&id_planilla=${id_planilla}`

      window.open("../nomina/planilla/bd/descargarArchivoPlano.php?" + data);
    }

    //======================= CONFIGURAR DETALLE CONCEPTOS ==========================//
    function ventanaConfigurarDatosNE(cont,tipo_concepto){
            ...
            [
                {
                    ...
                    [

                        {
                            ...
                            autoLoad :
                            {
                                url     : 'planilla/bd/grillaDetalleConceptosNE.php',
                                scripts : true,
                                nocache : true,
                                params  : 
                                {
