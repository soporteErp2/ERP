		...
		$list .= '
					<td>
						<img 
							...
							style   = "cursor:hand" 
							onclick = \'windowEditData('.$row[id].','.$row["data"].')\' 
						/>	
					</td>
					<td>
						<img 
...
<script>
						...
						[
							{
								...
								iconCls   : 'add_new',
								iconAlign : 'top',
								handler   : function(){BloqBtn(this); windowAddData();}
							},
							{
								xtype     : 'button',
								text      : 'Cerrar',
	...
	function windowAddData(title='agregar',id=0){
    ...
    async function saveData (id){
    	...
    	//proceso para actualizar un registro, primero se elimina, luego se almacena de forma normal
    	if (id>0) {
            ...
            success :function (result, request){
    					...
    					row += `<td>
									<img 
										...
										title   = "editar registro" 
										style   = "cursor:hand" 
										onclick = 'windowEditData(${response.lastId},${JSON.stringify(data)})' 
									/>	
								</td>
								<td>
									<img 
										...
										title   = "eliminar registro" 
										style   = "cursor:hand" 
										onclick = "deleteData(${response.lastId})"
									/>	
								</td>	
								<tr>`;
    ...
    function windowEditData(id,json){
    	windowAddData("Editar",id);
