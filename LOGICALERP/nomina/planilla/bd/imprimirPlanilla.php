<?php
	...
	$sql="SELECT consecutivo,fecha_inicio,fecha_final,estado FROM nomina_planillas WHERE activo=1 AND id_empresa=$idEmpresa AND id=$id_planilla";
	...
	$sql="SELECT id_empleado,id_contrato,dias_laborados,terminar_contrato FROM nomina_planillas_empleados WHERE activo=1 AND id_empresa=$idEmpresa AND id_planilla=$id_planilla ";
	$query=mysql_query($sql,$link);
	$whereId='';
	...
	$sql="SELECT id_empleado,id_contrato,id_concepto,concepto,valor_concepto,naturaleza,valor_campo_texto FROM nomina_planillas_empleados_conceptos WHERE activo=1  AND id_planilla=$id_planilla AND imprimir_volante='true' AND ($whereIdConceptos) ORDER BY naturaleza ASC";
	$query=mysql_query($sql,$link);
	$conceptos     ='';
	$acumDevengo   =0;
	$acumDeduccion =0;
	while ($row=mysql_fetch_array($query)) {
		$arrayConceptosEmpleados[$row['id_empleado']][$row['id_contrato']][$row['id_concepto']]=array(
	...
	foreach ($arrayEmpleadosContratos as $id_empleado => $arrayResul) {
		...
		foreach ($arrayResul as $id_contrato => $resul) {
			...
			if ($id_empleado_OLD!=$id_empleado) {
					if ($id_empleado_OLD!=0) {
						...
						$contenido.='<tr style="border:1px solid;">
									<td>NETO EMPLEADO</td>
									<td colspan="3" style="text-align:center;">'.number_format (($acumDevengo-$acumDeduccion),$_SESSION['DECIMALESMONEDA']).'</td>
								</tr>';
						$contenido.='</tbody>
									</table>';
						$acumDevengo   = 0;
			...
			$contenido.='<table class="articlesTable">
					<thead>
						<tr>
							...
							<td style="width:17%;"><b>CANTIDAD</b></td>
							<td style="width:17%;">DEVENGO</td>
							<td style="width:17%">DEDUCCION</td>
						</tr>
					</thead>
					<tbody>
			...
			foreach ($arrayConceptosEmpleados[$id_empleado][$id_contrato] as $id_concepto => $resulConcept) {
				$devengo   =($resulConcept['naturaleza']=='Devengo')? number_format ($resulConcept['valor_concepto'],$_SESSION['DECIMALESMONEDA']) : '' ;
				...
				$contenido.='<tr>
								...
								<td style="text-align:center;">'.$resulConcept['valor_campo_texto'].'</td>
								<td style="text-align:right;">'.$devengo.'</td>
								<td style="text-align:right;">'.$deduccion.'</td>
							</tr>';
				$acumDevengo   +=($resulConcept['naturaleza']=='Devengo')? $resulConcept['valor_concepto'] : 0 ;
				$acumDeduccion +=($resulConcept['naturaleza']=='Deduccion')? $resulConcept['valor_concepto'] : 0 ;

				$acumDevengoTotal   +=($resulConcept['naturaleza']=='Devengo')? $resulConcept['valor_concepto'] : 0 ;
