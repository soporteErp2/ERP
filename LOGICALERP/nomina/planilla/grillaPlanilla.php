...
<?php
    ...
    $acumScript .= (user_permisos(96,'false') == 'true')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").enable();' : '';
    $acumScript .= (user_permisos(97,'false') == 'true')? 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").enable();'   : '';
    $acumScript .= (user_permisos(98,'false') == 'true')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").enable();' : '';
    $acumScript .= (user_permisos(99,'false') == 'true')? 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").enable();' : '';
    ...
    $dias_liquidacion='';
    $tipo_liquidacion='<select id="id_tipo_liquidacion" onchange="UpdateTipoNomina(this.value)">';
    ...
    if($id_planilla==''){
        ...
        $sql="SELECT MAX(fecha_final) AS fecha FROM nomina_planillas_ajuste WHERE activo=1 AND id_empresa=$id_empresa AND estado=1";
        $query = mysql_query($sql,$link);
        $fecha_max = mysql_result($query,0,'fecha');

        if (compararFechas($fecha_inicio,$fecha_max)<0) {
            $fecha_inicio = $fecha_max;
            $fecha_final = date($fecha_max, strtotime ("+".$dias_liquidacion."days"));

        }
        // echo $fecha_inicio.'-'.$fecha_max.'='.compararFechas($fecha_inicio,$fecha_max);
        ...
        $acumScript .=  'new Ext.form.DateField({
                        ...
                        Inserta_Div_nomina_planillas('.$id_planilla.');    ';
    }
    //============================ SI EXISTE LA PLANILLA =============================//
        ...
        if ($consecutivo!='') {
            $acumScript.='document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML="Planilla de Nomina <br>N. '.$consecutivo.'";';
        }
        ...
        if ($estado=='2') { echo "ESTA PLANILLA DE NOMINA ESTA CERRADA "; exit; }
        ...
        $sql="SELECT id_empleado,documento_empleado,nombre_empleado,id_contrato,verificado FROM nomina_planillas_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
        $query=mysql_query($sql,$link);
        $bodyEmpleados='';
        $cont=1;
        while ($row=mysql_fetch_array($query)) {
        ...
        // $bodyArticle = cargaArticulosSave($id_planilla,$observacion,$estado,$opcGrillaContable,$tablaPrincipal,$tablaInventario,$idTablaPrincipal,$link);
    }
            ...
            <div class="contenedor_totales">
                ...
                <div class="contenedorConceptos" style="height:auto;margin-top:10px;width:auto;">
                    <div class="bodyDivNominaPlanilla" style="background-color:#FFF;">
                        <div class="campoHeadConceptos" style="text-align: center;background-color : #F3F3F3;border-right:none;width:calc(100% - 20px);">TOTAL PLANILLA DE NOMINA</div>
...
<script>
    ...
    UpdateTipoNomina(document.getElementById('id_tipo_liquidacion').value);
    //==================== FUNCION PARA ACTUALIZAR EL TIPO DE PAGO DE LA NOMINA ==========================//
    function UpdateTipoNomina(valor){
                            ...
                            [
                                {
                                    ...
                                    autoLoad    :
                                    {
                                        ...
                                        {
                                            renderizaBody : 'true',
                                            url_render    : 'planilla/bd/grillaBuscarEmpleados.php',
                                            opc           : 'buscar_empleado',
    ...
    function actualizaRecalcularFormula(id_empleado,id_contrato,recalcular_concepto) {
        Ext.get('divContentFormula').load({
            url     : 'planilla/bd/bd.php',
            scripts : true,
            nocache : true,
            text    : '&nbsp;&nbsp;&nbsp;&nbsp;loading',
            params  :
            {
                opc                 : 'actualizaRecalcularFormula',
                id_contrato         : id_contrato,
                id_empleado         : id_empleado,
                recalcular_concepto : recalcular_concepto,
                id_planilla         : '<?php echo $id_planilla; ?>'
    ...
    function ventanaBuscarConceptos(cont) {
        ...
        // var id_contrato = document.getElementById('id_contrato_concepto_'+cont).value;
         ...
         Win_Ventana_bucar_concepto = new Ext.Window({
             ...
             autoLoad    :
             {
                 url     : 'planilla/bd/grillaBuscarConceptos.php',
                 scripts : true,
                 nocache : true,
                 params  :
                 {
    ...
    function guardarPlanilla(){
        ...
        cargando_documentos('Generando Planilla de Nomina...','');
