...
<?php
    ...
    $acumScript .=($consecutivo>0)? 'document.getElementById("titleDocumento'.$opcGrillaContable.'").innerHTML="Planilla de Liquidacion <br>N. '.$consecutivo.'";' : '';

    //======================= CONSULTAR LOS EMPLEADOS CARGADOS EN LA PLANILLA ==================================//
    $sql="SELECT id_empleado,documento_empleado,nombre_empleado,id_contrato,verificado,email_enviado FROM nomina_planillas_liquidacion_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla";
    $query=mysql_query($sql,$link);
    $bodyEmpleados='';
    $cont=1;
    ...
    $acumScript .= (user_permisos(159,'false') == 'false')? 'Ext.getCmp("Btn_guardar_'.$opcGrillaContable.'").disable();' : '';
    $acumScript .= (user_permisos(160,'false') == 'false')? 'Ext.getCmp("Btn_editar_'.$opcGrillaContable.'").disable();'   : '';
    $acumScript .= (user_permisos(161,'false') == 'false')? 'Ext.getCmp("Btn_cancelar_'.$opcGrillaContable.'").disable();' : '';
    $acumScript .= (user_permisos(162,'false') == 'false')? 'Ext.getCmp("Btn_restaurar_'.$opcGrillaContable.'").disable();' : '';

    $acumScript.='contEmpleados='.$cont;

    $sql   = "SELECT consecutivo FROM nomina_planillas WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla_liquidacion=$id_planilla";
    $query = $mysql->query($sql,$mysql->link);
    $consecutivo_provision = $mysql->result($query,0,'consecutivo' );

    if ($consecutivo_provision>0) {
        $divDocsCruce='<div class="renglonTop">
                                <div class="labelTop">Planilla de Provision</div>
                                <div class="campoTop" >
                                    <div style="float:left;border-right:1px solid #EEE;padding:3px 5px 0px 5px;height:100%;font-weight:bold;" title="Planilla de N.">LN - '.$consecutivo_provision.'</div>
                                </div>
                            </div>';
    }
...
<script>
    ...
    function cancelarPlanillaLiquidacion(){
        ...
        cargando_documentos('Cancelado Planilla de Liquidacion...','');
    ...
    function imprimirPlanillaLiquidacion(){
        window.open("liquidacion/bd/imprimirPlanilla.php?id_planilla=<?php echo $id_planilla; ?>");
    }

    function modificarDocumentoPlanillaLiquidacion(){
        cargando_documentos('Editando Planilla de Liquidacion...','');
        Ext.get('render_btns_PlanillaLiquidacion').load({
            url     : 'liquidacion/bd/bd.php',
            scripts : true,
            nocache : true,
    ...
    function restaurarPlanillaLiquidacion(){
        cargando_documentos('Restaurando Planilla de Liquidacion...','');
        Ext.get('render_btns_PlanillaLiquidacion').load({
            url     : 'liquidacion/bd/bd.php',
            scripts : true,
            nocache : true,
    ...
    function imprimirVolanteUnicoEmpleado(id_empleado) {
       window.open('liquidacion/bd/imprimirVolanteEmpleado.php?id_planilla=<?php echo $id_planilla; ?>&id_empleado='+id_empleado);
    }
