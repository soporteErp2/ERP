<?php
	...
	$sql="SELECT id_empleado,id_contrato,dias_laborados,terminar_contrato,motivo_fin_contrato FROM nomina_planillas_liquidacion_empleados WHERE activo=1 AND id_empresa=$idEmpresa AND id_planilla=$id_planilla ";
	$query=mysql_query($sql,$link);
	$whereId='';
	while ($row=mysql_fetch_array($query)) {
		...
		$arrayInfoEmpleados[$row['id_empleado']][$row['id_contrato']]= array(	'dias_laborados'      => $row['dias_laborados'],
																				...
																				'motivo_fin_contrato' => $row['motivo_fin_contrato'] );
	}
	...
	$sql="SELECT id,id_empleado,documento_empleado,nombre_empleado,numero_contrato,grupo_trabajo,numero_cuenta_bancaria,fecha_cancelacion,fecha_inicio_contrato
			FROM empleados_contratos WHERE activo=1 AND id_empresa=$idEmpresa AND ($whereId)";
	$query=mysql_query($sql,$link);
	while ($row=mysql_fetch_array($query)) {
	...
	$sql="SELECT id_empleado,id_contrato,id_concepto,concepto,valor_concepto,valor_concepto_ajustado,naturaleza,valor_concepto_ajustado,dias_laborados,base,codigo_concepto,dias_adicionales
			FROM nomina_planillas_liquidacion_empleados_conceptos WHERE activo=1  AND id_planilla=$id_planilla AND ($whereIdConceptos) ORDER BY naturaleza ASC";
	$query=mysql_query($sql,$link);
	$conceptos     ='';
	$acumDevengo   =0;
	$acumDeduccion =0;
	while ($row=mysql_fetch_array($query)) {
		...
		$arrayConceptos[$row['id_empleado']][$row['id_contrato']][$row['codigo_concepto']] = $row['valor_concepto_ajustado'];
	}
	// print_r($arrayConceptos);exit;
	$contenido='';
	$id_empleado_OLD=0;
	...
	foreach ($arrayEmpleadosContratos as $id_empleado => $arrayResul) {
		...
		foreach ($arrayResul as $id_contrato => $resul) {
			...
			if ($id_empleado_OLD!=$id_empleado) {
					if ($id_empleado_OLD!=0) {
						...
						$contenido.='<tr style="border:1px solid;">
									<td>NETO EMPLEADO</td>
									<td colspan="4" style="text-align:center;">'.number_format (($acumDevengo-$acumDeduccion),$_SESSION['DECIMALESMONEDA']).'</td>
								</tr>';
						$contenido.='</tbody>
									</table>';
						$acumDevengo   = 0;
			...
			$finaliza_contrato=($arrayInfoEmpleados[$id_empleado][$id_contrato]['terminar_contrato']=='Si')? '<div style="float:left; width:90%; margin:15px 5px 0px 10px;">
																												<div style="float:left; width:100%;"><i><b>TERMINACION DE CONTRATO</b></i></div>
																											</div>
																											<div style="float:left; width:90%; margin:0px 5px 0px 10px;">
																												<div style="float:left; width:30%;"><b>Fecha Ingreso:</b></div>
																												<div style="float:left; width:60%;">'.$resul['fecha_inicio_contrato'].' </div>
																											</div>
																											<div style="float:left; width:90%; margin:0px 5px 0px 10px;">
																												<div style="float:left; width:30%;"><b>Fecha Retiro:</b></div>
																												<div style="float:left; width:60%;">'.$resul['fecha_cancelacion'].' </div>
																											</div>
																											<div style="float:left; width:90%; margin:0px 5px 0px 10px;">
																												<div style="float:left; width:30%;"><b>Motivo Retiro:</b></div>
																												<div style="float:left; width:60%;">'.$arrayInfoEmpleados[$id_empleado][$id_contrato]['motivo_fin_contrato'].' </div>
																											</div>' : '' ;
			...
			$contenido.='<table class="articlesTable">
					<thead>
						<tr>
							...
							<td style="width:10%;">DIAS LABORADOS</td>
							<td style="width:10%;">BASE</td>
							<td style="width:15%;">DEVENGO</td>
							<td style="width:15%">DEDUCCION</td>
						</tr>
					</thead>
					<tbody>
			...
			foreach ($arrayConceptosEmpleados[$id_empleado][$id_contrato] as $id_concepto => $resulConcept) {
				...
				if ($resulConcept['codigo_concepto']=='ISC') {
					$base = $arrayConceptos[$id_empleado][$id_contrato]['CS'];
				}
				else{
					$base = ($resulConcept['base']/$resulConcept['dias_laborados'])*30;
				}



				$valor_concepto = ($resulConcept['valor_concepto_ajustado']>0)? $resulConcept['valor_concepto_ajustado'] : $resulConcept['valor_concepto']  ;
				$devengo        = ($resulConcept['naturaleza']=='Devengo' || $resulConcept['naturaleza']=='Provision')? number_format ($valor_concepto,$_SESSION['DECIMALESMONEDA']) : '' ;
				$deduccion      = ($resulConcept['naturaleza']=='Deduccion')? number_format ($valor_concepto,$_SESSION['DECIMALESMONEDA']) : '' ;
				$contenido.='<tr>
								<td>'.$resulConcept['concepto'].'</td>
								<td style="text-align:right;">'.($resulConcept['dias_laborados']+$resulConcept['dias_adicionales']).'</td>
								<td style="text-align:right;">'.number_format ($base,$_SESSION['DECIMALESMONEDA']).'</td>
								<td style="text-align:right;">'.$devengo.'</td>
								<td style="text-align:right;">'.$deduccion.'</td>
							</tr>';
				$acumDevengo   +=($resulConcept['naturaleza']=='Devengo' || $resulConcept['naturaleza']=='Provision')? $valor_concepto : 0 ;
				$acumDeduccion +=($resulConcept['naturaleza']=='Deduccion')? $valor_concepto : 0 ;

				$acumDevengoTotal   +=($resulConcept['naturaleza']=='Devengo' || $resulConcept['naturaleza']=='Provision')? $valor_concepto : 0 ;
				$acumDeduccionTotal +=($resulConcept['naturaleza']=='Deduccion')? $valor_concepto : 0 ;
	...
	$header = '<div id="body_pdf" style="width:100%; font-style:normal;">
					...
					<div style="float:left;width:30%;text-align:center;font-size:16px;">
						<b>PLANILLA DE LIQUIDACION<br> N. '.$consecutivo.'</b><br>
