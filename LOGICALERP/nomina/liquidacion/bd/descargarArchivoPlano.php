<?php
  ...
  header("Content-Disposition: attachment; filename=bancolombia_liquidacion_".date(Y_m_d).".txt");
  ...
  $descripcionProposito = str_pad("PAGOLIQUID",10," ",STR_PAD_RIGHT);
  ...
  $sql = "SELECT naturaleza,saldo_restante
  ...
  while($row = mysql_fetch_array($query)){
    if($row['naturaleza'] == "Devengo" || $row['naturaleza'] == "Provision"){
      $suma += $row['saldo_restante'];
    } else{
      $resta += $row['saldo_restante'];
    }
  }
  ...
  $sql = "SELECT E.id,E.documento,E.nombre,E.email_empresa,EC.numero_cuenta_bancaria,NPLEC.naturaleza,NPLEC.saldo_restante
          FROM nomina_planillas_liquidacion_empleados_conceptos AS NPLEC
          LEFT JOIN empleados AS E
          ON NPLEC.id_empleado = E.id
          LEFT JOIN empleados_contratos AS EC
          ON NPLEC.id_contrato = EC.id
          WHERE NPLEC.activo = 1
          AND NPLEC.id_empresa = $id_empresa
          AND NPLEC.id_planilla = $id_planilla
          AND (NPLEC.naturaleza != 'Apropiacion')";
  $query = mysql_query($sql,$link);

  while($row = mysql_fetch_array($query)){
    if(is_array($arrayEmpleados[$row['id']])){
      if($row['naturaleza'] == "Devengo"){
        $arrayEmpleados[$row['id']]['devengo'] += $row['saldo_restante'];
      }
      if($row['naturaleza'] == "Provision"){
        $arrayEmpleados[$row['id']]['provision'] += $row['saldo_restante'];
      }
      if($row['naturaleza'] == "Deduccion"){
        $arrayEmpleados[$row['id']]['deduccion'] += $row['saldo_restante'];
      }
    }
    else{
      $arrayEmpleados[$row['id']] = array(
                                            ...
                                            'devengo' => (($row['naturaleza'] == "Devengo")? $row['saldo_restante'] : "0.00" ),
                                            'provision' => (($row['naturaleza'] == "Provision")? $row['saldo_restante'] : "0.00" ),
                                            'deduccion' => (($row['naturaleza'] == "Deduccion")? $row['saldo_restante'] : "0.00" ),
                                          );
    }
  }
  ...
  foreach($arrayEmpleados as $key){
    ...
    // VALOR TRANSACCION
    $valorTransaccion = ($key['devengo'] + $key['provision']) - $key['deduccion'];
