...
function imprimirEnviaVolante($id_planilla,$id_empleado,$id_contrato,$mail,$mpdf,$link){
	...
	$sql="SELECT dias_laborados,terminar_contrato,motivo_fin_contrato FROM nomina_planillas_liquidacion_empleados WHERE activo=1 AND id_empresa=$idEmpresa AND id_planilla=$id_planilla ";
	$query=mysql_query($sql,$link);
	...
	$sql="SELECT documento_empleado,nombre_empleado,numero_contrato,grupo_trabajo,fecha_inicio_contrato,tipo_contrato,fecha_cancelacion
	...
	// 	$finaliza_contrato = '<div style="float:right;width:40%;;">
	...
	$sql="SELECT codigo_concepto,concepto,valor_concepto,valor_concepto_ajustado,naturaleza,dias_laborados,base,dias_adicionales
	...
	$sql="SELECT email_personal,email_empresa,email_notificaciones FROM empleados WHERE activo=1 AND id_empresa=$idEmpresa AND id=$id_empleado";
	$query=mysql_query($sql,$link);
	...
	*/
	...
	if(!$mail->Send()) { return array('return' => 'false', 'mensaje' => 'No se envio el email, intentelo de nuevo\nSi el problema continua comuniquese con el administrador del sistema' ); }
	else { return array('return' => 'true', 'mensaje' => '' ); }
	$mail->ClearAddresses();
}
