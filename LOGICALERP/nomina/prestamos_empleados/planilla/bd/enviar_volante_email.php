...
function imprimirEnviaVolante($id_planilla,$id_empleado,$id_contrato,$mail,$mpdf,$link){
	...
	$sql   = "SELECT dias_laborados,terminar_contrato
	...
	$sql = "SELECT concepto,valor_concepto,naturaleza
	...
	$sql_email     = "SELECT email_empresa FROM empleados WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_empleado";
	...
	$contenido = '<table class="articlesTable">
					<thead>
						<tr>
							...
							<td style="width:25%; text-align:right;">DEVENGO</td>
							<td style="width:25%; text-align:right;">DEDUCCION</td>
						</tr>
					</thead>
					<tbody>
						'.$conceptos.'
	...
	$mpdf->Output($documento."volante_nomina.pdf",'F');		///OUTPUT A VISTA
	...
	$mail->MsgHTML($body);
	$mail->AddAttachment("volante_nomina.pdf");
