<?php
				...
				foreach ($arrayConceptosArray as $id_concepto => $arrayConceptosResul) {
					...
					// SI LA FORMULA NO CONTIENE EL CAMPO DE TEXTO, ENTONCES SE BUSCA EN EL ARRAY O EN LA BASE DE DATOS LOS VALORES DE LOS CONCEPTOS DE LA FORMULA
						...
						if ($search_var_concepto!==false) {
							...
							$sql   = "SELECT id_concepto,codigo_concepto,concepto,valor_concepto
	...
	function cargarTodosEmpleados($id_planilla,$id_empresa,$sucursal,$link){
		...
		$sql   = "SELECT id,id_empleado,tipo_documento_empleado,documento_empleado,nombre_empleado,numero_contrato,salario_basico,fecha_inicio_nomina,id_grupo_trabajo,valor_nivel_riesgo_laboral,IF(fecha_fin_contrato <= '$fecha', 'Si', 'No') AS terminar_contrato,id_sucursal
					...
					WHERE activo=1
						...
						AND fecha_inicio_nomina<='$fecha_final' $whereIdEmpleados";
		$query = mysql_query($sql,$link);
	...
	function cargarConceptosEmpleado($id_contrato,$id_empleado,$id_planilla,$id_empresa,$link){
		...
		$sql   = "SELECT dias_laborados,dias_laborados_empleado, terminar_contrato FROM nomina_planillas_empleados WHERE activo=1 AND id_empresa=$id_empresa AND id_planilla=$id_planilla AND id_empleado='$id_empleado' AND id_contrato='$id_contrato'";
		$query = mysql_query($sql,$link);
		...
		while ($row=mysql_fetch_array($query)) {
			...
			$botones=($estado==1 || $estado==3 )? '' : '<div style="float:right; min-width:80px;margin-left:5px;">
			...
			$bodyConceptos .= '<div class="bodyDivNominaPlanilla">
	                        	    ...
	                        	    <div class="campo1" style="width:calc(100% - 50% - 70px - 18px );text-indent:0;">
										...
										<input type="text" style="width:calc(100% - 50px);padding-left: 0px;margin-left: -3px;" '.$evento_input.' id="valor_concepto_'.$cont.'" value="'.$row['valor_concepto'].'" '.$readonly.'>
	                        	    </div>
		...
		if ($estado==0) {
			$bodyConceptos .= '<div class="bodyDivNominaPlanilla">
	                        	    ...
	                        	    <div class="campo1" style="width:calc(100% - 50% - 70px - 18px );text-indent:0;">
										...
										<input type="text" style="width:calc(100% - 50px);padding-left: 0px;margin-left: -3px;" '.$evento_input.' '.$readonly.' id="valor_concepto_'.$cont.'">
	                    	    	</div>
	...
	function calculaValorConceptoFormulaInput($id_insert_concepto,$id_planilla,$id_concepto,$id_contrato,$id_empleado,$cont,$variable,$id_empresa,$link){
		...
		// SI EXISTEN VARIABLES DE OTROS CONCEPTOS, CONSULTAR EN LA BASE DE DATOS Y REEMPLAZAR
		// else{
			$sql="SELECT id_concepto,codigo_concepto,concepto,valor_concepto FROM nomina_planillas_empleados_conceptos WHERE activo=1
					AND id_planilla = '$id_planilla'
					AND id_empleado = '$id_empleado'
					AND id_contrato = '$id_contrato'
					AND id_empresa  = '$id_empresa'";
    ...
    function terminarGenerar($id_planilla,$id_empresa,$id_sucursal,$link){
    	...
    	while ($row=mysql_fetch_array($query)){
			...
			$arrayAsientosColgaap[$id_empleado][$cuenta_colgaap][$caracter]                                           += $row['valor_concepto'];
			$arrayAsientosColgaap[$id_empleado_contrapartida][$cuenta_contrapartida_colgaap][$caracter_contrapartida] += $row['valor_concepto'];
			...
			$arrayAsientosNiif[$id_empleado][$cuenta_niif][$caracter]                                           += $row['valor_concepto'];
			$arrayAsientosNiif[$id_empleado_contrapartida][$cuenta_contrapartida_niif][$caracter_contrapartida] += $row['valor_concepto'];
    ...
    function cancelarPlanillaNomina($id_planilla,$id_empresa,$link){
		...
		$sql    = "SELECT estado FROM nomina_planillas WHERE activo=1 AND id='$id_planilla' AND id_empresa='$id_empresa'";
		$query  = mysql_query($sql,$link);
		$estado = mysql_result($query,0,'estado');
