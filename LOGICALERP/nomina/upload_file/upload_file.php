<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory,$id_planilla,$mysql){
            ...
            if ($this->file->save($uploadDirectory.$filename.'.'.$ext)){
                ...
                if($resulProcess <> 'false'){ return array('success'=>true, 'ok' => $resulProcess ); }
                else{ return array('error'=> 'Se produjo un error', 'debug'=> "$resulProcess"); }
    ...
    $serv = $rutaServer."/";
    $url  = $serv.'ARCHIVOS_PROPIOS/documentos_nomina/';
    if(!file_exists($url)){ mkdir ($url); }

    $url  = $serv.'ARCHIVOS_PROPIOS/documentos_nomina/ajuste/';
    if(!file_exists($url)){ mkdir ($url); }
    ...
    $result   = $uploader->handleUpload($url,$id_planilla,$mysql);
