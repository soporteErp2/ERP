<?php
	...
	// CONSULTAR SI EN ESE MISMO DIA HAY PRECIERRE
	$fechaPrecierre = date("Y-m-d");
	$sql   = "SELECT id,fecha FROM ventas_pos_auditoria_precierre WHERE activo=1 AND estado=1 AND fecha='$fechaPrecierre' ";
	$query = $mysql->query($sql);
	$idPrecierre    = $mysql->result($query,0,'id');
	if ($idPrecierre>0) {
		$bodyPrecierre = '<div class="form_separador" style="background-color: #db5957 !important;color: #FFF !important; ">INCONSISTENCIAS PRECIERRE</div>
							<table class="simpleTable">
								<tbody>
									<tr>
										<td><b>Ya se realizo el precierre para este dia</b></td>
									</tr>
								</tbody>
							</table>
							';
	...
	$sql="SELECT id,consecutivo,fecha_documento,cliente,usuario,detalle_estado FROM ventas_pos WHERE activo=1 AND estado='500' ";
	$query=$mysql->query($sql);
	while ($row=$mysql->fetch_array($query)) {
		$bodyErrorFacturas .= "<tr>
									...
									<td >
										<i class='material-icons' style='cursor:pointer;' title='Reprocesar' onClick='sincFactura($row[id])' >autorenew</i>
									</td>
								</tr>";
		// $habilitaBoton = false;
	}
...
<body>
	...
	<script type="text/javascript">
		<?php
			if ($habilitaBoton) {
				echo "\$W.Element('btnPrecierre').enable();";
			}
		?>

		var cerrarCaja = (id_row) =>{
		...
		var sincFactura= (id_documento='')=>{
			$W.Loading();
			params       = { id : id_documento }
			fetch(`../../../backend/pos/Controller.php?method=generateTiquet&nit=<?= $arrayEmpresa[0] ?>`, {
				method  : 'POST', // or 'PUT'
			...
			.then((responseJson)=>{
				...
				$W.Loading();
				let cuentas = [];
				if (responseJson.cuentas) {
					Object.keys(responseJson.cuentas.colgaap).forEach( (key) =>{
						debito = (responseJson.cuentas.colgaap[key].debito!==undefined)? responseJson.cuentas.colgaap[key].debito : 0 ;
						credito = (responseJson.cuentas.colgaap[key].credito!==undefined)? responseJson.cuentas.colgaap[key].credito : 0 ;
						idCcos = (responseJson.cuentas.colgaap[key].idCcos!==undefined)? responseJson.cuentas.colgaap[key].idCcos : '' ;
						cuentas[key]= {};
						cuentas[key].Debito  = debito
						cuentas[key].Credito = credito
						cuentas[key].Norma = 'colgaap'
						cuentas[key].centro_costos = idCcos
					})
					console.table(cuentas);
					Object.keys(responseJson.cuentas.niif).forEach( (key) =>{
						debito = (responseJson.cuentas.niif[key].debito!==undefined)? responseJson.cuentas.niif[key].debito : 0 ;
						credito = (responseJson.cuentas.niif[key].credito!==undefined)? responseJson.cuentas.niif[key].credito : 0 ;
						idCcos = (responseJson.cuentas.niif[key].idCcos!==undefined)? responseJson.cuentas.niif[key].idCcos : '' ;
						cuentas[key]= {};
						cuentas[key].Debito  = debito
						cuentas[key].Credito = credito
						cuentas[key].Norma = 'niif'
						cuentas[key].centro_costos = idCcos
					})
					console.table(cuentas);
				}
