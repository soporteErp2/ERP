<?php
	...
	$whereEmpresa = "documento = '$arrayWs[nit_empresa]'";
	$sqlEmpresa   = "SELECT id FROM empresas WHERE activo=1 AND $whereEmpresa";
	$queryEmpresa = mysql_query($sqlEmpresa,$conexion);

	if(!$queryEmpresa){ echo json_encode(array("estado" => 'error', "msj" => "No existe la empresa en ERP")); exit; }					//ERROR EN LA CONSULTA MYSQL
	...
	function insertUpdateTerceroEmpresa($arrayWs,$idEmpresaErp,$conexion){
		...
		else{ echo json_encode(array("estado"=>"error", "msj"=>"Codigo de ubicacion ciudad no valido")); exit; }
		...
		else{ echo json_encode(array("estado"=>"error", "msj"=>"Codigo de ubicacion ciudad documento no valido")); exit; }
		...
		else{ echo json_encode(array("estado"=>"error", "msj"=>"Codigo tipo de identificacion no valido")); exit; }
		...
		else{ echo json_encode(array("estado"=>"error", "msj"=>"Codigo tipo de Regimen no valido")); exit; }
		...
		// echo json_encode(array("estado"=>"true", "msj"=>"");
	}
