<?php
	...
	if($cont==0){ echo json_encode(array('estado' => 'error','msj'=>'la cuenta de pago no existe en el sistema!')); exit; }
	...
	if ($arrayWs['tipo_documento']=='factura_venta') {
		...
		if($cont==0){ echo json_encode(array('estado' => 'error','msj'=>'no hay resolucion dian para los consecutivos de las facturas!')); exit; }
		...
		if($cont==0){ echo json_encode(array('estado' => 'error','msj'=>'no existe el centro de costos en el sistema!')); exit; }
	...
	$query = mysql_query($sql,$conexion);
	$cont  = mysql_result($query,0,'cont');
    if ($cont>0) { echo json_encode(array('estado' => 'error','msj'=>'ya existe una factura con ese numero de documento!')); exit; }
