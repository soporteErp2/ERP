<?php
	...
	function getItems($nit,$usuario,$password,$id_bodega){
		...
		$arrayItem = array();
		$sqlItems  = "SELECT IT.codigo,
							IT.nombre_equipo AS nombre,
							IT.costos AS costo,
							IT.code_bar AS codigo_barras,
							IT.cantidad AS cantidad,
							IF(I.item_produccion='true','P','I') AS tipo,
							CONCAT(C.cod_familia,C.cod_grupo,C.codigo) AS agrupacion
						FROM inventario_totales AS IT
						INNER JOIN items AS I ON (
								IT.id_item = I.id
								AND I.activo=1
							)
						INNER JOIN items_familia_grupo_subgrupo AS C ON(
								C.activo = 1
								AND C.id_empresa = '$id_empresa'
								AND C.id = IT.id_subgrupo
							)
						WHERE IT.id_empresa='$id_empresa' AND IT.id_ubicacion='$id_bodega' AND IT.estado_venta='true'";
						// WHERE IT.id_empresa='$id_empresa' AND IT.id_ubicacion='$id_bodega' AND IT.inventariable='true' AND IT.estado_venta='true'";
		...
		while ($row = $mysql->fetch_assoc($queryItems)) {
			$arrayItem[] = array(
									...
									"familia_grupo_subgrupo"=>$row['agrupacion'],
									"tipo"=>utf8_encode($row['tipo']),
									"opc"=>""
								);
		}
		return json_encode($arrayItem);
	}
