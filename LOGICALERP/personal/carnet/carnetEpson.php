<?php
	...
	$ORIENTACION = 'L';
	$tamano_hoja2 = array(number_format(325/3.8,2),number_format(205/3.8,2));
	$tamano_hoja = array(number_format(135),number_format(215.9));
	$pdf->AddPage($ORIENTACION,$tamano_hoja);

	$pdf->Image('../../../ARCHIVOS_PROPIOS/temp/CarnetTempPdf.'.$ext, $margen_I, $margen_S, $tamano_hoja2[0], $tamano_hoja2[1], '', '', 'T', false, 300, '', false, false, 1, false, false, false);
	...
	while($row = mysql_fetch_array($consul2)){
			...
			$top	= substr($row['top'], 0, -2)/3.8 + $margen_S;
			$left	= substr($row['left'], 0, -2)/3.8 + $margen_I;	
			...
			if($row['campo'] == 'Fotografia'){
				...
				if($row['codebar']=='true'){
					$style = array(
								'position' => '','align' => 'C','border' => false,'padding' => 0,	'fgcolor' => array(0,0,0),
