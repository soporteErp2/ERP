<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory, $link, $id_contrato,$id_usuario){
            ...
            $sqlInsert = "INSERT INTO empleados_contratos_documentos (id_contrato,randomico_archivo,ext,fecha_hora,id_usuario)
                            VALUES ($id_contrato,'$idUnico','$ext',now(),'$id_usuario')";
            if(mysql_query($sqlInsert,$link)){ $idRow = mysql_insert_id($link);

                if ($this->file->save($uploadDirectory.$idUnico.'.'.$ext)){

                    $sqlUpdate = "UPDATE empleados_contratos SET vencimiento_firmado = 'true',usuario_vencimiento_firmado = '$id_usuario',fecha_vencimiento_firmado = NOW(),archivo_vencimiento_firmado = '$idUnico.$ext' WHERE id = '$id_contrato'";
                    mysql_query($sqlUpdate,$link);
        			return array('success'=>true, 'idRow'=>$idRow,'filename'=> $idUnico,'ext'=> $ext,'fecha'=>date('Y-m-d'),'usuario'=>ucwords(strtolower($nombre_usuario)) );
    ...
    $ruta3 = $serv.'ARCHIVOS_PROPIOS/empresa_'.$id_host.'/empleados/contratos_vencimientos';
    $url   = $ruta3.'/';
    if(!file_exists($ruta3)){ mkdir ($ruta3); }

    $uploader = new qqFileUploader($allowedExtensions, $sizeLimit);
    $result   = $uploader->handleUpload($url, $link, $id_contrato,$id_usuario);
