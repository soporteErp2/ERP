<?php
	...
	switch ($opc) {
		...
		case 'ventanaVerImagenDocumentoVencimiento':
			ventanaVerImagenDocumentoVencimiento($nombreImage,$nombreDocumento,$type,$id_host);
			break;
		case 'consultaSizeImageDocumentoVencimiento':
			consultaSizeImageDocumentoVencimiento($id_host,$nombre);
			break;
	...
	function ventanaVerImagenDocumentoVencimiento($nombreImage,$nombreDocumento,$type,$id_host){
		if (file_exists($_SERVER['DOCUMENT_ROOT'].'/ARCHIVOS_PROPIOS/empresa_'.$id_host.'/empleados/contratos_vencimientos/'.$nombreImage)) {
			$url = '../../../ARCHIVOS_PROPIOS/empresa_'.$id_host.'/empleados/contratos_vencimientos/'.$nombreImage;
		}
		else{
			$url = '';
		}
		...
		if($type!='pdf'){
			...
			echo'<div style="margin:10px auto 10px auto; width:95%; height:90%; background-color:#FFF; display:table">
					<div style="display: table-cell; vertical-align: middle; text-align:center;">
						<iframe src="'.$url.'" id="iframeViewDocumentoVencimiento"></iframe>
					</div>
				</div>
				<script>
					cambiaViewPdf();
					function cambiaViewPdf(){
						var iframe=document.getElementById("iframeViewDocumentoVencimiento");
	...
	function consultaSizeImageDocumentoVencimiento($id_host,$nombre){
		if (file_exists($_SERVER['DOCUMENT_ROOT'].'/ARCHIVOS_PROPIOS/empresa_'.$id_host.'/empleados/contratos_vencimientos/'.$nombre)) {
			$url = $_SERVER['DOCUMENT_ROOT'].'/ARCHIVOS_PROPIOS/empresa_'.$id_host.'/empleados/contratos_vencimientos/'.$nombre;
		}
		else{
			$url = '';
		}
	...
	function eliminarArchivoAdjunto($id,$nombre,$id_host,$mysql){
		...
		if ( unlink($url) ) {
			$sql="DELETE FROM empleados_contratos_documentos WHERE id=$id";
