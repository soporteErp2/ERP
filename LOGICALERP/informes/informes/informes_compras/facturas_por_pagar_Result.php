<?php
    ...
    class CarteraEdades
    {
         ...
         * @param json $arrayProveedoresJSON Array json con los clientes a filtrar
         ...
         */
        function __construct($MyInformeFiltroFechaFinal,$MyInformeFiltroFechaInicio,$arrayProveedoresJSON,$arrayCuentasPagoJSON,$tipo_fecha_informe,$agrupacion,$tipo_informe,$sqlCheckbox,$sucursal,$order_by,$IMPRIME_PDF=null,$IMPRIME_XLS=null,$separador_miles,$separador_decimales,$mysql){
            $this->MyInformeFiltroFechaFinal  = $MyInformeFiltroFechaFinal;
            $this->MyInformeFiltroFechaInicio = $MyInformeFiltroFechaInicio;
            $this->arrayProveedoresJSON          = json_decode($arrayProveedoresJSON);
        ...
        public function setFilters(){
            ...
            if (!empty($this->arrayProveedoresJSON)) {
                foreach ($this->arrayProveedoresJSON as $indice => $id_proveedor) {
                    $whereProveedor .= ($whereProveedor=='')? ' CF.id_proveedor='.$id_proveedor : ' OR CF.id_proveedor='.$id_proveedor;
                }
                    $whereProveedor   = " AND (".$whereProveedor.")";
            }
            $this->whereInnerJoin .= $whereProveedor;
            if (!empty($arrayCuentasPagoJSON)) {
                foreach ($arrayCuentasPagoJSON as $indice => $cuenta_pago) {
            ...
            $this->whereInnerJoin .= ($this->sucursal!='' && $this->sucursal!='global')? " AND CF.id_sucursal=$this->sucursal " : "" ;

            // // FILTRO TEMP TABLE
            // $this->whereTemp = ($this->tipo_fecha_informe=='corte')? " AND fecha <= '$this->MyInformeFiltroFechaFinal' " : " AND fecha BETWEEN '$this->MyInformeFiltroFechaInicio' AND '$this->MyInformeFiltroFechaFinal' ";
            // $this->whereTemp .= " AND (".str_replace("A.", "", $whereCuentas).")";
        ...
        public function getCuentasPago(){
            $whereTemp = "";
            $sql   = "SELECT cuenta FROM configuracion_cuentas_pago WHERE id_empresa=$this->id_empresa AND activo=1 AND tipo='Compra' AND estado='Credito'";
            $query = $this->mysql->query($sql,$this->mysql->link);
        ...
        public function getInformData(){
            $tempTable = "tempAccount$_SESSION[ID_HOST]";

            echo $sql = "CREATE TEMPORARY TABLE $tempTable
                                SELECT
                                    SUM(haber - debe) AS saldo,
                                    id_documento_cruce,
                                    codigo_cuenta,
                                    activo,
                                    fecha,
            ...
            echo $sql = "SELECT
                        ...
                        DATEDIFF('$this->MyInformeFiltroFechaFinal',CF.fecha_final) AS dias,
                        CF.id,
                        CF.id_proveedor,
                        CF.nit,
                        CF.proveedor,
                      ...
                      LEFT JOIN terceros AS T ON(
                        CF.id_proveedor = T.id
                        AND T.id_empresa = $this->id_empresa
                      )
                      ORDER BY
                        $this->order_by, CF.proveedor ASC, CF.fecha_final ASC";
            $query=$this->mysql->query($sql,$this->mysql->link);
            while ($row=$this->mysql->fetch_array($query)) {
                ...
                $arrayTemp[$row['id']] = array(
                                                ...
                                                "fecha_vencimiento"       => $row['fecha_final'],
                                                "consecutivo"             => $row['consecutivo'],
                                                "numero_factura_completo" => ($row['prefijo_factura']<>'')? "$row[prefijo_factura] $row[numero_factura]" : $row['numero_factura'],
            ...
            echo $sql = "DROP TEMPORARY TABLE $tempTable";
            $query = $this->mysql->query($sql,$this->mysql->link);
            return $arrayTemp;
        ...
        public function getTitle(){
            $title = "<table align='center' style='text-align:center;margin-bottom:10px;'>
                        ...
                        <tr><td style='width:100%; font-weight:bold; font-size:14px; text-align:center;'>Cartera de Proveedores</td></tr>
                    </table>";
            return $title;
        }

        public function getExcel(){
            if ($this->agrupacion == 'Proveedores'){
                foreach ($arrayFacturas as $id => $arrayResult){
                    $arrayFacturas[$arrayResult['nit']][] = $id;
                    $arrayTerceros[$arrayResult['nit']]['nombre']    = $arrayResult['proveedor'];
                    $arrayTerceros[$arrayResult['nit']]['telefono1'] = $arrayResult['telefono1'];
                    $arrayTerceros[$arrayResult['nit']]['celular1']  = $arrayResult['celular1'];
                }
                ...
                foreach ($arrayFacturas as $nit => $values) {
                    $acumuladoSaldo               = 0;
                    ...
                    $bodyTable .= "<tr class='total'>
                                        <td colspan='16'><b>DOCUMENTO: $nit PROVEEDOR: ".$arrayTerceros[$nit]['nombre']." Tel: ".$arrayTerceros[$nit]['telefono1']." Cel: ".$arrayTerceros[$nit]['celular1']." </b></td>
                                    </tr>";
        ...
        public function getContent($arrayFacturas){
            ...
            if ($this->agrupacion == 'Proveedores'){
                ...
                foreach ($arrayFacturas as $nit => $values) {
                    ...
                    $bodyTable .= "<tr class='total'>
                                        <td colspan='16'><b>DOCUMENTO: $nit PROVEEDOR: ".$arrayTerceros[$nit]." ".$arrayTerceros[$nit]['nombre']." Tel: ".$arrayTerceros[$nit]['telefono1']." Cel: ".$arrayTerceros[$nit]['celular1']."</b></td>
                                    </tr>";
                    ...
                    if ($this->tipo_informe<>'totalizado_edades') {
                        $bodyTable .= "<tr class='total'>
                                            <td colspan='6'>TOTAL PROVEEDOR</td>
                                            <td style='text-align:right;'>".number_format($acumuladoSaldo,$_SESSION['DECIMALESMONEDA'],$this->separador_decimales,$this->separador_miles)."</td>
                                            <td style='text-align:right;'>".number_format($acumuladoporVencer,$_SESSION['DECIMALESMONEDA'],$this->separador_decimales,$this->separador_miles)."</td>
                                            <td style='text-align:right;'>".number_format($acumuladounoAtreinta,$_SESSION['DECIMALESMONEDA'],$this->separador_decimales,$this->separador_miles)."</td>
                                            <td style='text-align:right;'>".number_format($acumuladotreintayunoAsesenta,$_SESSION['DECIMALESMONEDA'],$this->separador_decimales,$this->separador_miles)."</td>
    ...
    $informe = new CarteraEdades($MyInformeFiltroFechaFinal,$MyInformeFiltroFechaInicio,$arrayProveedoresJSON,$arrayCuentasPagoJSON,$tipo_fecha_informe,$agrupacion,$tipo_informe,$sqlCheckbox,$sucursal,$order_by,$IMPRIME_PDF,$IMPRIME_XLS,$separador_miles,$separador_decimales,$mysql);
    $informe->generate();
