<?php
    ...
    if ($MyInformeFiltroFechaFinal!='' && $MyInformeFiltroFechaFinal!='') {
        ...
        $script = 'localStorage.MyInformeFiltroFechaInicioFacturas  = "";
                ...
                tercerosConfigurados_FCIR.length   = 0;
                arrayVendedores_FCIR.length        = 0;
                vendedoresConfigurados_FCIR.length = 0;';
    }
    ...
    $sql   = "SELECT A.id_documento_cruce,
                    ...
                    C.total_factura,
                    IF(C.prefijo_factura<>'', CONCAT(C.prefijo_factura,' ',C.numero_factura),C.numero_factura) AS factura
                FROM asientos_colgaap AS A LEFT JOIN compras_facturas AS C ON(
                    C.id=A.id_documento_cruce
                    )
                WHERE A.tipo_documento_cruce='FC'
    ...
    while ($row = mysql_fetch_assoc($query)) {
        ...
        $acumFila = '<td>'.$row['sucursal'].'</td>
                    <td>'.$row['nit'].'</td>
                    <td style="width:200px;">'.$row['proveedor'].'</td>
                    <td>'.$row['factura'].'</td>
                    <td>'.$row['fecha'].'</td>
    ...
    $tableBody .= '     </tr>
                        '.$acumBody.'
                        <tr class="filaTotal">
                            <td colspan="5">TOTAL IMPUESTOS Y RETENCIONES</td>
                            <td>'.$totalFacturas.'</td>';
