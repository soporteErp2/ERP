...
<script>
    ...
    if (typeof(localStorage.MyInformeFiltroFechaInicioOrdenesCompraArchivosAdjuntos)!="undefined")
        if (localStorage.MyInformeFiltroFechaInicioOrdenesCompraArchivosAdjuntos!="")
            document.getElementById("MyInformeFiltroFechaInicio").value=localStorage.MyInformeFiltroFechaInicioOrdenesCompraArchivosAdjuntos;

    if (typeof(localStorage.MyInformeFiltroFechaFinalOrdenesCompraArchivosAdjuntos)!="undefined")
        if (localStorage.MyInformeFiltroFechaFinalOrdenesCompraArchivosAdjuntos!="")
            document.getElementById("MyInformeFiltroFechaFinal").value=localStorage.MyInformeFiltroFechaFinalOrdenesCompraArchivosAdjuntos;

    if (typeof(localStorage.contenido_ordenes_compra_archivos_adjuntos)!="undefined")
        if (localStorage.contenido_ordenes_compra_archivos_adjuntos!="")
            document.getElementById("contenido").value=localStorage.contenido_ordenes_compra_archivos_adjuntos;

    //RECORRER EL ARRAY PARA RENDERIZAR LOS TERCEROS DEL FILTRO
    tercerosConfiguradosOC.forEach(function(elemento) {rows += elemento;});
