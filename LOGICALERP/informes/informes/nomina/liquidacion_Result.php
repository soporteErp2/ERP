<?php
    ...
    if($IMPRIME_XLS=='true'){
       ...
       header("Content-Disposition: attachment; filename=informe_liquidacion_empleados_".date("Y_m_d").".xls");
...
if ((!isset($MyInformeFiltroFechaFinal) || $MyInformeFiltroFechaFinal=='') && (!isset($MyInformeFiltroFechaInicio) || $MyInformeFiltroFechaInicio=='')) {
    ...
    $whereFecha=" AND fecha_documento >= '".date('Y-m-d')."' AND fecha_documento <= '$MyInformeFiltroFechaFinal' ";
    $script='localStorage.MyInformeFiltroFechaFinalLiquidacion         = "";
             localStorage.MyInformeFiltroFechaInicioLiquidacion        = "";
             localStorage.sucursal_liquidacion                         = "";
             localStorage.agrupacion_liquidacion                       = "";
             localStorage.discrimina_planillas_liquidacion_liquidacion = "";

             //VACIAR LOS ARRAY DE LOS DATOS DEL CLIENTE
             arrayEmpleadosLiquidacion.length=0;
             arrayEmpleadosConfiguradosLiquidacion.length=0;
             arrayConceptosLiquidacion.length=0;
            ';
    $agrupacion_liquidacion = 'empleados';


}else{
    // SI SE VA A DETALLAR CADA PLANILLA
    // discrimina_planillas_liquidacion

    $whereFecha=" AND fecha_documento >= '$MyInformeFiltroFechaInicio' AND fecha_documento <= '$MyInformeFiltroFechaFinal' ";
    ...
    if ($tipo_contrato<>'todos') {
            $whereIdEmpleados='';
            $sql="SELECT id_empleado,nombre_empleado FROM empleados_contratos WHERE activo=1 AND id_empresa=$id_empresa AND id_tipo_contrato=$tipo_contrato ";
    ...
    $sql="SELECT id,consecutivo FROM nomina_planillas_liquidacion WHERE activo=1 AND id_empresa=$id_empresa AND estado=1 $whereSucursal $whereFecha";
    $query=mysql_query($sql,$link);
    $wherePlanilla='';
    ...
    while ($row=mysql_fetch_array($query)) {
        ...
        $arrayPlanillas[$row['id']] = $row['consecutivo'];
    }
    ...
    $sql="SELECT id_planilla,id_empleado,id_contrato,tipo_documento,documento_empleado,nombre_empleado,dias_laborados,id_contrato,terminar_contrato
            FROM nomina_planillas_liquidacion_empleados WHERE activo=1 AND id_empresa=$id_empresa AND ($wherePlanilla) $whereIdEmpleados";
    $query=mysql_query($sql,$link);
    ...
    $sql="SELECT id,id_planilla,id_empleado,id_contrato,id_concepto,codigo_concepto,concepto,naturaleza,valor_concepto,valor_concepto_ajustado,valor_campo_texto
            FROM nomina_planillas_liquidacion_empleados_conceptos WHERE activo=1 AND id_empresa=$id_empresa AND ($wherePlanilla) $whereIdConceptos ORDER BY naturaleza";
    $query=mysql_query($sql,$link);

    while ($row=mysql_fetch_array($query)) {
        // VERIFICAR SI SE ESTAN PIDIENDO EL INFORME POR EMPLEADO
        if ($agrupacion_liquidacion=='empleados') {
            ...
            if (isset($arrayPlanillasEmpleadosConceptos[$row['id_empleado']][$row['id_concepto']]['valor_concepto'])) {
                $arrayPlanillasEmpleadosConceptos[$row['id_empleado']][$row['id_concepto']]['valor_concepto']+=($row['naturaleza']=='Provision')? $row['valor_concepto_ajustado'] : $row['valor_concepto'];
            ...
            if (isset($arrayPlanillasEmpleadosConceptosDiscriminados[$row['id_empleado']][$row['id_planilla']][$row['id_concepto']]['valor_concepto'])) {
                $arrayPlanillasEmpleadosConceptosDiscriminados[$row['id_empleado']][$row['id_planilla']][$row['id_concepto']]['valor_concepto']+=($row['naturaleza']=='Provision')? $row['valor_concepto_ajustado'] : $row['valor_concepto'];
        ...
        // INFORME POR CONCEPTO
           ...
           // SI YA EXISTE ESE CONCEPTO, ENTONCES LE INCREMENTAMOS EL VALOR DEL CONCEPTO PARA NO SOBREESCRIBIRLO
            if (isset($arrayPlanillasEmpleadosConcepto[$row['id_concepto']][$row['id_empleado']]['valor_concepto'])) {
                $arrayPlanillasEmpleadosConcepto[$row['id_concepto']][$row['id_empleado']]['valor_concepto']+=($row['naturaleza']=='Provision')? $row['valor_concepto_ajustado'] : $row['valor_concepto'];
            ...
            if (isset($arrayPlanillasEmpleadosConceptosDiscriminados[$row['id_concepto']][$row['id_planilla']][$row['id_empleado']]['valor_concepto'])) {
                $arrayPlanillasEmpleadosConceptosDiscriminados[$row['id_concepto']][$row['id_planilla']][$row['id_empleado']]['valor_concepto']+=($row['naturaleza']=='Provision')? $row['valor_concepto_ajustado'] : $row['valor_concepto'];
    ...
    if ($agrupacion_liquidacion=='empleados') {
        ...
        foreach ($arrayPlanillasEmpleados as $id_empleado => $arrayPlanillasEmpleadosResul) {
            ...
            $campoHead=($discrimina_planillas_liquidacion=='true')? '<td style="text-align:center;">PLANILLA</td>' : '' ;
            $colspanEmpleado=($discrimina_planillas_liquidacion=='true')? 7 : 6 ;
            $campoTotal=($discrimina_planillas_liquidacion=='true')? '<td>&nbsp;</td>' : '' ;
            $bodyTable .= '<tr>
            ...
            if ($discrimina_planillas_liquidacion=='true') {
                ...
                foreach ($arrayPlanillasEmpleadosConceptosDiscriminados[$id_empleado] as $id_planilla => $arrayResul) {
                    foreach ($arrayResul as $id_concepto => $arrayPlanillasEmpleadosConceptosResul) {
                        ...
                        $bodyTable.='<tr class="filaConcepto" '.$style.'>
                                        <td style="padding-left: 10px;text-align:center;">'.$arrayPlanillas[$id_planilla].'</td>
                                        ...
                                        <td style="text-align:right;">'.$apropiacion.'</td>
                                        <td style="text-align:right;">'.$provision.'</td>
                                    '.(($IMPRIME_XLS=='true')? '<td>'.$arrayPlanillasEmpleados[$id_empleado]['documento_empleado'].'</td><td>'.$arrayPlanillasEmpleados[$id_empleado]['nombre_empleado'].'</td> </tr>' : '</tr>');;
                    }
    ...
    // SI EL INFORME ES POR CONCEPTO
        ...
        foreach ($arrayConceptos as $id_concepto => $arrayConceptosResul) {
            ...
            $campoHead=($discrimina_planillas_liquidacion=='true')? '<td style="text-align:center;">PLANILLA</td>' : '' ;
            $colspanEmpleado=($discrimina_planillas_liquidacion=='true')? 5 : 4 ;
            $campoTotal=($discrimina_planillas_liquidacion=='true')? '<td>&nbsp;</td>' : '' ;
            $bodyTable .= '<tr>
