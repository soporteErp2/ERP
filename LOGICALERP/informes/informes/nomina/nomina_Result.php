<?php
    ...
    if($IMPRIME_XLS=='true'){
       ...
       header("Content-Disposition: attachment; filename=informe_nomina_".date("Y_m_d").".xls");
    ...
    if ((!isset($MyInformeFiltroFechaFinal) || $MyInformeFiltroFechaFinal=='') && (!isset($MyInformeFiltroFechaInicio) || $MyInformeFiltroFechaInicio=='')) {
        ...
        $script='localStorage.MyInformeFiltroFechaFinalNomina  = "";
                 localStorage.MyInformeFiltroFechaInicioNomina = "";
                 localStorage.sucursal_nomina                  = "";
                 localStorage.agrupacion_nomina                = "";
                 localStorage.discrimina_planillas             = "";

                 //VACIAR LOS ARRAY DE LOS DATOS DEL CLIENTE
                 arrayEmpleados.length=0;
                 arrayEmpleadosNomina.length=0;
                 arrayConceptos.length=0;
                ';
        $agrupacion_nomina = 'empleados';
    }
    else{
    ...
    $sql="SELECT id,consecutivo,sucursal,fecha_documento,fecha_inicio,fecha_final FROM nomina_planillas WHERE activo=1 AND id_empresa=$id_empresa AND (estado=1 OR estado=2) $whereSucursal $whereFecha";
    $query=mysql_query($sql,$link);
    $wherePlanilla='';
    ...
    $sql="SELECT id_planilla,id_empleado,id_contrato,tipo_documento,documento_empleado,nombre_empleado,dias_laborados,id_contrato,terminar_contrato
            FROM nomina_planillas_empleados WHERE activo=1 AND id_empresa=$id_empresa AND ($wherePlanilla) $whereIdEmpleados";
    $query=mysql_query($sql,$link);
    ...
    $sql="SELECT id,id_planilla,id_empleado,id_contrato,id_concepto,codigo_concepto,concepto,naturaleza,valor_concepto,valor_campo_texto
            FROM nomina_planillas_empleados_conceptos WHERE activo=1 AND id_empresa=$id_empresa AND ($wherePlanilla) $whereIdEmpleados $whereIdConceptos ORDER BY naturaleza";
    $query=mysql_query($sql,$link);

    while ($row=mysql_fetch_array($query)) {
        $acumDevengoTotal     += ($row['naturaleza']=='Devengo')?     $row['valor_concepto'] : '' ;
        $acumDeduccionTotal   += ($row['naturaleza']=='Deduccion')?   $row['valor_concepto'] : '' ;
        $acumApropiacionTotal += ($row['naturaleza']=='Apropiacion')? $row['valor_concepto'] : '' ;
        $acumProvisionTotal   += ($row['naturaleza']=='Provision')?   $row['valor_concepto'] : '' ;

        // VERIFICAR SI SE ESTAN PIDIENDO EL INFORME POR EMPLEADO
        if ($agrupacion_nomina=='empleados') {
    ...
    if ($agrupacion_nomina=='empleados') {
        ...
        foreach ($arrayPlanillasEmpleados as $id_empleado => $arrayPlanillasEmpleadosResul) {
            ...
            $campoHead=($discrimina_planillas=='true')? '<td style="text-align:center;">PLANILLA</td>' : '' ;
            $colspanEmpleado=($discrimina_planillas=='true')? 7 : 6 ;
            $campoTotal=($discrimina_planillas=='true')? '<td>&nbsp;</td>' : '' ;
            $bodyTable .= '<tr>
                    ...
                    foreach ($arrayResul as $id_concepto => $arrayPlanillasEmpleadosConceptosResul) {
                        ...
                        $bodyTable.='<tr class="filaConcepto" '.$style.'>
                                    ...
                                    '.(($IMPRIME_XLS=='true')? '<td>'.$arrayPlanillasEmpleados[$id_empleado]['documento_empleado'].'</td>
                                                                ...
                                                                <td>'.$arrayPlanillas[$id_planilla]['fecha_documento'].'</td>
                                                                <td>'.$arrayPlanillas[$id_planilla]['fecha_inicio'].'</td>
                                                                <td>'.$arrayPlanillas[$id_planilla]['fecha_final'].'</td>
                                                                <td>'.$arrayPlanillas[$id_planilla]['sucursal'].'</td>
                                                                </tr>
                                                                '
            ...
            $bodyTable.='<tr class="titulos_totales_empleados">
                            <td colspan="4">NETO EMPLEADO</td>
                            <td style="text-align:right;">&nbsp;</td>
                            '.$campoTotal.'
                            <td style="text-align:right;">'.($acumDevengo-$acumDeduccion).'</td>
                        </tr>
                        <tr><td>&nbsp;</td></tr>';
    ...
    // SI EL INFORME ES POR CONCEPTO
        ...
        foreach ($arrayConceptos as $id_concepto => $arrayConceptosResul) {
            ...
            $campoHead=($discrimina_planillas=='true')? '<td style="text-align:center;">PLANILLA</td>' : '' ;
            $colspanEmpleado=($discrimina_planillas=='true')? 6 : 5;
            $campoTotal=($discrimina_planillas=='true')? '<td>&nbsp;</td>' : '' ;
            $bodyTable .= '<tr>
            ...
            if ($discrimina_planillas=='true') {
                ...
                foreach ($arrayPlanillasEmpleadosConceptosDiscriminados[$id_concepto] as $id_planilla => $arrayPlanillasEmpleadosConceptoResul) {
                    foreach ($arrayPlanillasEmpleadosConceptoResul as $id_empleado => $arrayResul) {
                        ...
                        $bodyTable.='<tr class="filaConcepto" '.$style.'>
                                        <td style="padding-left:10px;text-align:center;">'.$arrayPlanillas[$id_planilla]['consecutivo'].'</td>
                                        <td style="padding-left:10px;text-align:center;">'.$arrayPlanillas[$id_planilla]['fecha_documento'].'</td>
                                        <td>'.$arrayEmpleados[$id_empleado]['documento_empleado'].'</td>
                ...
                foreach ($arrayPlanillasEmpleadosConcepto[$id_concepto] as $id_empleado => $arrayResul) {
                    ...
                    $style=($style!='')? '' : 'style="background:#F7F7F7;"' ;
                    $bodyTable.='<tr class="filaConcepto" '.$style.'>
                                    <td style="padding-left:10px;">'.$arrayEmpleados[$id_empleado]['fecha_documento'].'</td>
