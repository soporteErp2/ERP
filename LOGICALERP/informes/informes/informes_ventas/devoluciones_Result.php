<?php
  ...
  $whereVendedores    = '';
    ...
    $sql = "SELECT
          ...
          LEFT JOIN devoluciones_venta_inventario AS VFI 
          ON(
            VFI.activo = 1
    ...
    while($row = mysql_fetch_array($query)){
      ...
      $acumuladoSubtotal += $row['subtotal'] - $row['descuento_pesos'];
    ...
    $colspanTotal = ($IMPRIME_XLS == 'true')? 'colspan="10"' : 'colspan="4"';
...
<?php
  ...
  $documento = "Informe Devoluciones De Venta";
    ...
    $mpdf->SetHtmlFooter($footer);
		...
		if($PDF_GUARDA == 'true'){
      $mpdf->Output("informe_devoluciones_de_venta_" . date('Y_m_d') . ".pdf",'D');
    }
    else{
      $mpdf->Output("informe_devoluciones_de_venta_" . date('Y_m_d') . ".pdf",'I');
    }
