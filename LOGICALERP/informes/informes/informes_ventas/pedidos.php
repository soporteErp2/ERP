<?php
	...
	$informe->InformeName			=	'pedidos_venta';  //NOMBRE DEL INFORME
	$informe->InformeTitle			=	'Pedidos'; //TITULO DEL INFORME
...
<script>
	...
	function generarPDF_Excel_principal(tipo_documento){
		...
		for ( i = 0; i < arraytercerosPV.length; i++) {
			if (typeof(arraytercerosPV[i])!="undefined" && arraytercerosPV[i]!="") {
				idTerceros=(idTerceros=='')? arraytercerosPV[i] : idTerceros+','+arraytercerosPV[i] ;
			}
		}
		...
		for ( i = 0; i < arrayvendedoresPV.length; i++) {
			if (typeof(arrayvendedoresPV[i])!="undefined" && arrayvendedoresPV[i]!="") {
				idVendedores=(idVendedores=='')? arrayvendedoresPV[i] : idVendedores+','+arrayvendedoresPV[i] ;
			}
		}

		if (typeof(localStorage.MyInformeFiltroFechaInicioPedidosVenta)!="undefined" && typeof(localStorage.MyInformeFiltroFechaFinalPedidosVenta)!="undefined") {
			if (localStorage.MyInformeFiltroFechaInicioPedidosVenta!='' && localStorage.MyInformeFiltroFechaFinalPedidosVenta) {
				MyInformeFiltroFechaFinal  = localStorage.MyInformeFiltroFechaFinalPedidosVenta;
				MyInformeFiltroFechaInicio = localStorage.MyInformeFiltroFechaInicioPedidosVenta;
			}
		}

		if (typeof(localStorage.sucursal_pedidos_venta)!="undefined") {
			if (localStorage.sucursal_pedidos_venta) {
				sucursal=localStorage.sucursal_pedidos_venta;
			}
		}

		window.open("../informes/informes/informes_ventas/pedidos_Result.php?"+tipo_documento+"=true&sucursal="+sucursal+"&MyInformeFiltroFechaFinal="+MyInformeFiltroFechaFinal+"&MyInformeFiltroFechaInicio="+MyInformeFiltroFechaInicio+"&idTerceros="+idTerceros+"&idVendedores="+idVendedores);
	}
	...
	function ventanaConfigurarInforme(){
		...
		Win_Ventana_configurar_informe_facturas = new Ext.Window({
		    ...
		    autoLoad    :
		    {
		        ...
		        params  :
		        {
		            opc : 'cuerpoVentanaConfiguracionPedidosVenta',
	...
	function resetFiltros(){
		...
		localStorage.MyInformeFiltroFechaInicioPedidosVenta = "";
	...
	function generarHtml(){
		...
		var sucursal   = document.getElementById('filtro_sucursal_pedidos_venta').value;
		var idTerceros = '';
		var idVendedores = '';
		...
		Ext.get('RecibidorInforme_pedidos_venta').load({
			url     : '../informes/informes/informes_ventas/pedidos_Result.php',
			text	: 'Generando Informe...',
			scripts : true,
			nocache : true,
			params  :
			{
				nombre_informe             : 'Pedidos',
				sucursal                   : sucursal,
		...
		document.getElementById("RecibidorInforme_pedidos_venta").style.padding = 20;

		localStorage.MyInformeFiltroFechaFinalPedidosVenta  = MyInformeFiltroFechaFinal;
		localStorage.MyInformeFiltroFechaInicioPedidosVenta = MyInformeFiltroFechaInicio;
		localStorage.sucursal_pedidos_venta                  = sucursal;
        ...
        Win_VentanaCliente_terceros = new Ext.Window({
            ...
            autoLoad    :
            {
                ...
                params  :
                {
					...
					id_tercero           : 'id',
					tercero              : tercero,
					opcGrillaContable 	 : 'pedidos_venta',
					cargaFuncion         : '',
					nombre_grilla        : '',
                }
            },
	...
	function checkGrilla(checkbox,cont,tabla){
		...
		if (checkbox.checked ==true) {
            ...
            if (tabla=='empleados') {
            	...
            	vendedoresConfiguradosPV[cont]=fila;
            	...
            	arrayvendedoresPV[cont]=checkbox.value;
            }
            else{
            	var div   = document.createElement('div');
            	...
            	tercerosConfiguradosPV[cont]=fila;
            	//CREAMOS LOS ELEMENTOS DEL ELEMENTO CREADO
            	document.getElementById('fila_cartera_tercero_'+cont).innerHTML=fila;
            	contTercero++;

            	//LENAMOS UN ARRAY CON INDICE EL CONT Y COMO VALOR EL ID TERCERO
            	arraytercerosPV[cont]=checkbox.value;
        	}

		}
		else if (checkbox.checked ==false) {
			if (tabla=='empleados') {
				delete arrayvendedoresPV[cont];
				delete vendedoresConfiguradosPV[cont];
				(document.getElementById("fila_empleado_"+cont)).parentNode.removeChild(document.getElementById("fila_empleado_"+cont));
			}
			else{
				delete arraytercerosPV[cont];
				delete tercerosConfiguradosPV[cont];
