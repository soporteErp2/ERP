<?php
	...
	if($IMPRIME_XLS=='true'){
		...
		header("Content-Disposition: attachment; filename=informe_items_remisionados_".date("Y_m_d").".xls");
	...
	$whereSucursal = '';
	$subtitulo     = 'INFORME ITEMS REMISIONADOS';
	...
	if (isset($MyInformeFiltroFechaFinal) && $MyInformeFiltroFechaFinal!='') {
		...
		$script = 'localStorage.MyInformeFiltroFechaInicioRemisionesVenta = "";
					...
					terceros_config_itemsRemisionados.length   = 0;
					array_vendedores_itemsRemisionados.length  = 0;
					vendedores_config_itemsRemisionados.length = 0;
					array_ccos_itemsRemisionados.length        = 0;
					ccos_config_ItemsRemisionados.length       = 0;
					array_categorias_itemsRemisionados.length  = 0;
					categorias_config_ItemsRemisionados.length = 0;
	...
	$sqlVentas = "SELECT $sqlCampos
					FROM ventas_remisiones AS F
					INNER JOIN ventas_remisiones_inventario AS I ON(
						I.activo=1
						AND I.cantidad > 0
						AND I.id_remision_venta=F.id
					)
