<?php
    ...
    //=======================================// SALDO ANTERIOR //=======================================//
    //**************************************************************************************************//
    $sqlAsientosAnterior = "SELECT SUM(debe-haber) AS saldo,
                                cuenta,
                                $campoCuenta,
                                id_tercero,
                                nit_tercero,
    ...
    while($rowAsientosAnterior = mysql_fetch_array($queryAsientosAnterior)){
        // $rowAsientosAnterior['saldo'] = round($rowAsientosAnterior['saldo'],2);
        // echo $rowAsientosAnterior['saldo']."+<br>";
        //SALDO
    ...
    // rsort($arrayAsiento);
    // arsort($arrayAsiento);
    // ksort($arrayTotalesClase);
    // print_r($arrayAsiento);
    // echo substr('12345678',0,2);
    // $style = 'color:#FFF';
    ...
    foreach ($arrayPuc as $cuenta => $descripcion) {
            ...
            foreach ($arrayAsiento[$cuenta] as $idTercero => $arrayDatos) {
                // foreach ($arrayCuenta as $idTercero => $arrayDatos) {
                    ...
                    $descripcion   = $arrayPuc[$cuenta]/*$arrayDatos['descripcion']*/;

                    // echo var_dump($saldoAnterior).' '.var_dump($debe_actual).' '.var_dump($haber_actual).'  '.var_dump($newSaldo).'<br>';
    ...
    // echo recorta_digitos(20.000001);

    function recorta_digitos($numero){
        $numeros = explode(".", $numero);
        // $numero = $numeros[0].'.'.substr($numeros[1],0;
