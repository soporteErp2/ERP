<?php
    ...
    while ($rowAsientos = mysql_fetch_array($queryAsientos)){
        $id_tercero         = $rowAsientos['id_tercero'];
    ...
    $sqlAsientosAnterior = "SELECT SUM(debe-haber) AS saldo,codigo_cuenta,id_tercero,tercero,consecutivo_documento,tipo_documento,fecha
                                FROM asientos_niif
                            WHERE activo=1
                                AND id_empresa = '$id_empresa'
                                    $whereSucursal
    ...
    while($rowAsientosAnterior = mysql_fetch_array($queryAsientosAnterior)){
        ...
        $id_tercero = $rowAsientosAnterior['id_tercero'];
