<?php
  ...
  if($tipo_balance == 'clasificado'){
      ...
      foreach ($newBalance->getActivo() as $cuenta => $rowActivos) {
      	if($id_pais == 233){
          $valor_activos = abs($rowActivos['saldo']);
        }
        else{
          $valor_activos = $rowActivos['saldo'];
        }
        ...
        $cuerpoActivosT .= '<tr>
                                ...
                                <td class="defaultFont" style="text-align:right;">'.number_format($valor_activos, $_SESSION['DECIMALESMONEDA'],$separador_decimales,$separador_miles).'</td>
                            </tr>';
      ...
      foreach ($newBalance->getPasivo() as $cuenta => $rowPasivos) {
        if($id_pais == 233){
          $valor_pasivos = abs($rowPasivos['saldo']);
        }
        else{
          $valor_pasivos = $rowPasivos['saldo'];
        }

        $cuerpoPasivosT .= '<tr class="defaultFont">
                              <td width="100" class="defaultFont" style="padding-right:10px; text-align:right;">'.$cuenta.'</td>
                              <td class="defaultFont">'.$rowPasivos['descripcion'].'</td>
                              <td class="defaultFont" style="text-align:right;">'.number_format($valor_pasivos, $_SESSION['DECIMALESMONEDA'],$separador_decimales,$separador_miles).'</td>
                            </tr>';
      ...
      foreach ($newBalance->getPatrimonio() as $cuenta => $rowPatrimonio) {
        if($id_pais == 233){
          $valor_patrimonio = abs($rowPatrimonio['saldo']);
        }
        else{
          $valor_patrimonio = $rowPatrimonio['saldo'];
        }

        $cuerpoPatrimonioT .= '<tr class="defaultFont">
                                  <td width="100" class="defaultFont" style="padding-right:10px; text-align:right;">'.$cuenta.'</td>
                                  <td class="defaultFont">'.$rowPatrimonio['descripcion'].'</td>
                                  <td class="defaultFont" style="text-align:right;">'.number_format($valor_patrimonio, $_SESSION['DECIMALESMONEDA'],$separador_decimales,$separador_miles).'</td>
                                </tr>';
      ...
      if($id_pais == 233){
        $acumActivos    = abs($acumActivos);
        $acumPasivos    = abs($acumPasivos);
        $acumPatrimonio = abs($acumPatrimonio);
      }
      else{
        $acumActivos    = $acumActivos;
        $acumPasivos    = $acumPasivos;
        $acumPatrimonio = $acumPatrimonio;
      }
  ...
  //***************************************************************************************//
    ...
    foreach ($arrayCuentas as $cuenta) {
      ...
      if($id_pais == 233){
        $saldoInicial = abs($arrayActivoInicial[$cuenta]['saldo'] * 1);
        $saldoFinal   = abs($arrayActivoFinal[$cuenta]['saldo'] * 1);
      }
      else{
    ...
    foreach ($arrayCuentas as $cuenta) {
      ...
      if($id_pais == 233){
        $saldoInicial = abs($arrayPasivoInicial[$cuenta]['saldo'] * 1);
        $saldoFinal   = abs($arrayPasivoFinal[$cuenta]['saldo'] * 1);
      }
      else{
    ...
    foreach ($arrayCuentas as $cuenta) {
      ...
      if($id_pais == 233){
        $saldoInicial = abs($arrayPatrimonioInicial[$cuenta]['saldo'] * 1);
        $saldoFinal   = abs($arrayPatrimonioFinal[$cuenta]['saldo'] * 1);
      }
      else{
  ...
  class balanceGeneral{
    ...
    */
    private function saldoPyg($id_empresa,$varCortar,$mysql){
      // CONSULTAR LA CUENTA DE CIERRE PARA EXCLUIRLA
