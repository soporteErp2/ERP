<?php
  ...
  $sqlNiveles = "SELECT digitos FROM puc_configuracion WHERE activo = 1 AND id_empresa = $_SESSION[EMPRESA] AND nombre = '$nivel_cuentas'";
  $queryNiveles = mysql_query($sqlNiveles,$link);

  while($row = mysql_fetch_array($queryNiveles)){
    $arrayNiveles[] = $row['digitos'];
  }
  ...
  while($rowAsientos = mysql_fetch_array($queryAsientos)){
    foreach($arrayNiveles as $key => $value){
      $contArrayWherePuc++;
      $arrayWherePuc[$contArrayWherePuc] = substr($rowAsientos['codigo_cuenta'], 0, $value);

      //ArrayAsientos EN FECHAS SELECCIONADAS
      $arrayAsientoActual[substr($rowAsientos['codigo_cuenta'], 0, $value)]['debe'] += $rowAsientos['debe'];
      $arrayAsientoActual[substr($rowAsientos['codigo_cuenta'], 0, $value)]['haber']+= $rowAsientos['haber'];
    }
  }
  ...
  while($rowAsientosAnterior = mysql_fetch_array($queryAsientosAnterior)){
    foreach($arrayNiveles as $key => $value){
      $contArrayWherePuc++;
      $arrayWherePuc[$contArrayWherePuc] = substr($rowAsientosAnterior['codigo_cuenta'], 0, $value);

      //ArrayAsientos EN FECHA ANTERIOR
      $arrayAsientoAnterior[substr($rowAsientosAnterior['codigo_cuenta'], 0, $value)] = $rowAsientosAnterior['saldo'];
      $arrayAsientoNewSaldo[substr($rowAsientosAnterior['codigo_cuenta'], 0, $value)] = $arrayAsientoNewSaldo[substr($rowAsientosAnterior['codigo_cuenta'], 0, $value)] + (($rowAsientosAnterior['saldo']+$arrayAsientoActual[substr($rowAsientosAnterior['codigo_cuenta'], 0, $value)]['debe'])-$arrayAsientoActual[substr($rowAsientosAnterior['codigo_cuenta'], 0, $value)]['haber']);
    }
  }
  ...
  $wherePuc = "";
  foreach($arrayWherePuc as $key => $value){
    if($wherePuc == ""){
      $wherePuc .= " cuenta = $value";
    }
    else{
      $wherePuc .= " OR cuenta = $value";
    }
  }
