	...
	*/
	class depuraDocumentos
	{
		...
		*/
		...
		{
			$sql="SELECT cuenta,cuenta_niif FROM puc WHERE activo=1 AND id_empresa=$this->id_empresa";
			$query=$this->mysql->query($sql,$this->mysql->link);
			while ($row=$this->mysql->fetch_array($query)) {
				$cuenta      = $row['cuenta'];
				$cuenta_niif = $row['cuenta_niif'];
				$this->arrayPuc[$cuenta] = $cuenta_niif;
			}
		}
		...
		*/
		public function getDocumentos()
		{
			...
			$sql = "SELECT
					...
					ORDER BY CAST(codigo_cuenta AS CHAR) ASC LIMIT 0,50000;";
			$query=$this->mysql->query($sql,$this->mysql->link);
			while ($row=$this->mysql->fetch_array($query)) {
				$id           = $row['id'];
				...
				$codigo_cuenta  = $row['codigo_cuenta'];
				$arrayDocumentos['niif'][$id_documento][$tipo_documento][$codigo_cuenta]=$codigo_cuenta;
			}
			...
			$sql = "SELECT
					...
					GROUP BY A.id
					ORDER BY CAST(A.codigo_cuenta AS CHAR) ASC  LIMIT 0,50000;";
			$query=$this->mysql->query($sql,$this->mysql->link);
			while ($row=$this->mysql->fetch_array($query)) {
				$id           = $row['id'];
				$id_documento = $row['id_documento'];
				...
				$tipo_documento = $row['tipo_documento'];
				$codigo_cuenta  = $row['codigo_cuenta'];
				$arrayDocumentos['colgaap'][$id_documento][$tipo_documento][$codigo_cuenta]=$codigo_cuenta;
			...
			// DETECTAR DOCUMENTO SIN CUENTAS HOMOLOGADAS
			foreach ($arrayDocumentos['colgaap'] as $id_documento => $arrayDocumentos1) {
				foreach ($arrayDocumentos1 as $tipo_documento => $arrayDocumentos2) {
					foreach ($arrayDocumentos2 as $codigo_cuenta => $arrayResult) {

						if (!array_key_exists($this->arrayPuc[$codigo_cuenta], $arrayDocumentos['niif'][$id_documento][$tipo_documento])) {
						    $this->arrayDocumentosDescuadrados[$id_documento][$tipo_documento][$codigo_cuenta] = $codigo_cuenta;
						}
