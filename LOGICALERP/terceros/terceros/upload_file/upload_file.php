<?php
    ...
    class qqFileUploader {
        ...
        function handleUpload($uploadDirectory,$typeNota,$sinc_nota){
            ...
            if ($this->file->save($uploadDirectory.$filename.'.'.$ext)){
                ...
                if($_GET['prospecto'] == 'true'){//SI ES UN EXCEL DE PROSPECTOS O DE TERCEROS
                    $load_excel = include_once("load_excel_prospectos.php");
                }
                else{
                    $load_excel = include_once("load_excel.php");
                }
                // unlink($uploadDirectory . $filename . '.' . $ext);

                if(is_array($load_excel)) return $load_excel;
    ...
    $url  = $serv.'ARCHIVOS_PROPIOS/upload_terceros/';
    ...
    $result   = $uploader->handleUpload($url,$typeNota,$sinc_nota);
