<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//NOMBRE DE LA GRILLA
			$grilla->GrillaName	 		= 'busquedaRetencionesTercero';  	//NOMBRE DE LA GRILLA (DEBE SER UNICO POR CADA GRILLA DE LA APLICACION)
		//QUERY
			$grilla->TableName			= 'retenciones';			//NOMBRE DE LA TABLA EN LA BASE DE DATOS
			$grilla->MyWhere			= "activo = 1 AND id_empresa='$id_empresa' AND id NOT IN(SELECT id_retencion FROM terceros_retenciones WHERE activo = 1 AND id_empresa='$id_empresa' AND id_proveedor=$idTercero)";		//WHERE DE LA CONSULTA A LA TABLA "$TableName"
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			$grilla->AddRow('Retencion','retencion',220);
...
if(!isset($opcion)){  ?>
	<div id="renderNewRemision" style="float:left; width:18px; height:18px; overflow:hidden; border:1px solild #000; position:fixed; margin:10px;"></div>
	<script>
		function Editar_busquedaRetencionesTercero(id){

			Ext.get('renderNewRemision').load({
				url     : '../terceros/bd/bd.php',
				scripts : true,
				nocache : true,
				params  :
				{
					op          : 'guardar_retencion',
					idRetencion : id,
					idTercero   : '<?php echo $idTercero; ?>',
				}
			});
			//Win_VentanaBuscarPucRetenciones.close();
		}
	</script>
<?php
} ?>