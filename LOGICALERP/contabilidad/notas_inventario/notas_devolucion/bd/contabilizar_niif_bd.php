<?php
  ...
  function contabilizarNotaFacturaCompraConPlantillaNiif($idNota,$idBodega,$idSucursal,$idEmpresa,$idPlantilla,$idFactura,$idProveedor,$link,$fecha,$numero_documento_cruce){
    	$decimalesMoneda = ($_SESSION['DECIMALESMONEDA'] >= 0)? $_SESSION['DECIMALESMONEDA']: 0;

    	//PLANTILLA CONFIGURACION
		$sqlPlantilla   = "SELECT codigo_puc,caracter,porcentaje,descripcion FROM contabilizacion_compra_venta_niif WHERE id_documento='$idFactura' AND tipo_documento='FC'";
		...
		while($rowRetenciones = mysql_fetch_array($queryRetenciones)){
			...
			if($tipoRetencion == "ReteIva"){ 																		//CALCULO RETEIVA
				...
				if($porcentajeIva == 0){ echo '<script>alert("Aviso.\nLa plantilla ingresada tiene no cuenta Niif con iva. para continuar quite la RETENCION al iva (Reteiva).")</script>'; exit; }
		...
		if($contCuentasPantillas > 1){
			...
			if($globalDebito != $globalCredito){ echo '<script>alert("Aviso.\nLa plantilla Niif ingresada no cumple doble partida, Confirme su configuracion en el modulo panel de control.")</script>'; exit; }
  ...
  function contabilizarNotaFacturaCompraSinPlantillaNiif($estadoCuentaPago,$idNota,$idBodega,$idSucursal,$idEmpresa,$idFactura,$idProveedor,$link,$fecha,$numero_documento_cruce){
  ...
  function contabilizarNotaRemisionVentaNiif($idNota,$idBodega,$idSucursal,$idEmpresa,$idRemision,$idCliente,$link,$fecha,$numero_documento_cruce){
		...
		$consultaCuentasItems = "SELECT DVI.id,DVI.id_inventario, CCV.id_puc, CCV.codigo_puc AS puc, CCV.caracter AS estado, DVI.costo_inventario AS costo, DVI.cantidad, CCV.descripcion
								FROM devoluciones_venta_inventario AS DVI, contabilizacion_compra_venta_niif AS CCV
								WHERE DVI.activo = 1
	...
	function contabilizarNotaFacturaVentaConPlantillaNiif($idNota,$idSucursal,$idEmpresa,$idPlantilla,$idFactura,$idCliente,$exento_iva,$link,$fecha,$numero_documento_cruce,$totalFactura){
		$decimalesMoneda = ($_SESSION['DECIMALESMONEDA'] >= 0)? $_SESSION['DECIMALESMONEDA']: 0;

    	//PLANTILLA CONFIGURACION
		$sqlPlantilla   = "SELECT codigo_puc,caracter,porcentaje,descripcion FROM contabilizacion_compra_venta_niif WHERE id_documento='$idFactura' AND tipo_documento='FV'";
		$queryPlantilla = mysql_query($sqlPlantilla,$link);
		...
		while($rowplantilla = mysql_fetch_array($queryPlantilla)){
			...
			// if($rowplantilla['descripcion'] == 'PRECIO' && $cuenta4 == 4135){ $cuenta = 417501; }
		...
		while($rowRetenciones = mysql_fetch_array($queryRetenciones)){
			...
			if($tipoRetencion == "AutoRetencion"){ 																		//DEVOLUCION SALDO RETECREE NO RESTA
				if(is_nan($cuentaAutoretencion)|| $cuentaAutoretencion < 10000){ echo '<script>alert("Aviso.\nNo se ha configurado la cuenta Niif Autorretencion.")</script>'; exit; }
				$contAutoRetencion++;
				$arrayAutoRetencion[$contAutoRetencion] = $cuentaAutoretencion;
		...
		if($contCuentasPantillas > 1){
			foreach ($arrayCuentas AS $cuentaPuc => $valArrayCuenta){
				...
				else if( $valArrayCuenta['descripcion'] == 'COSTO' || $valArrayCuenta['descripcion'] == 'CONTRAPARTIDA COSTO'){ $saldo = $costoAcumulado; }	//COMERCIO AL MAYOR Y AL POR MENOR -> COSTOS LA EMPRESA
				else{ $saldo = $precioAcumulado; }														//CONTRAPARTIDA-RETENCIONES
