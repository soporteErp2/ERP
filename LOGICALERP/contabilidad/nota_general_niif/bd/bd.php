<?php
	...
	function cargaDivsInsertUnidadesConTercero($formaConsulta,$cont,$opcGrillaContable){
		$body ='<div class="campo" style="width:40px !important; overflow:hidden;">
				...
				<div style="float:right; min-width:80px;">
					...
					<div onclick="retrocederCuenta'.$opcGrillaContable.'('.$cont.')" id="divImageDeshacer'.$opcGrillaContable.'_'.$cont.'" title="Deshacer Cambios" style="width:20px; float:left; margin-top:3px;cursor:pointer;display:none"><img src="img/deshacer.png" id="imgDeshacerArticulo'.$opcGrillaContable.'_'.$cont.'"></div>
					<div onclick="deleteCuenta'.$opcGrillaContable.'('.$cont.')" id="deleteCuenta'.$opcGrillaContable.'_'.$cont.'" title="Eliminar Cuenta" style="width:20px; float:left; margin-top:3px; display:none;cursor:pointer;"><img src="img/delete.png" /></div>
					<div onclick="cambiaCuentaNiif'.$opcGrillaContable.'('.$cont.')" id="configurarCuenta'.$opcGrillaContable.'_'.$cont.'" title="Configurar" style="width:20px; float:left; margin-top:3px;cursor:pointer;display:none;"><img src="img/config16.png" /></div>
	...
	function buscarCuenta($campo,$cuenta,$contFila,$id_empresa,$id_sucursal,$idCliente,$opcGrillaContable,$link){
		...
		*/
		...
		$sqlValidateSucursal   = "SELECT COUNT(id) AS cont_sucursal, id_sucursal FROM puc_niif WHERE cuenta='$cuenta' AND activo=1 AND id_empresa='$id_empresa' LIMIT 0,1";
		...
		if($contSucursal == 0 || $idSucursalDb != 0 && $idSucursalDb != $id_sucursal){
			$textAlert = ($contSucursal == 0)? "no se encuentra asignado en las cuentas NIIF de la empresa": "no esta disponible en la presente sucursal!";
			echo'<script>
		...
		$sqlCuenta   = "SELECT COUNT(id) AS contCuenta FROM puc_niif WHERE activo=1 AND id_empresa='$id_empresa' AND cuenta LIKE '$cuenta%' AND cuenta<>'$cuenta'";
		...
		$sqlCuentaAsientos  = "SELECT id_cuenta,cuenta FROM asientos_niif WHERE codigo_cuenta = '$cuenta' AND id_empresa='$id_empresa'";
		...
		//SI LA CUENTA NO EXISTE ENTONCES SE VA A VALIDAR ANTES DE CONTINUAR
			...
			$sqlValidaAsiento   = "SELECT COUNT(id) AS cont FROM asientos_niif WHERE $where AND id_empresa=$id_empresa";
			...
			$sqlArticulo = "SELECT id,descripcion FROM puc_niif WHERE cuenta='$cuenta' AND (LENGTH(cuenta)>5) AND activo=1 AND id_empresa='$id_empresa' LIMIT 0,1";
	...
	function retrocederCuenta($id,$idCuentaInsert,$cont,$opcGrillaContable,$tablaCuentasNota,$idTablaPrincipal,$link){
		$sql   = "SELECT id_niif,cuenta_niif,descripcion_niif,debe,haber,id_documento_cruce,numero_documento_cruce,tercero,tipo_documento_cruce,prefijo_documento_cruce FROM $tablaCuentasNota WHERE $idTablaPrincipal='$id' AND id='$idCuentaInsert' ";
		$query = mysql_query($sql,$link);

		$idCuenta                = mysql_result($query,0,'id_niif');
		$cuenta                  = mysql_result($query,0,'cuenta_niif');
		$descripcion             = mysql_result($query,0,'descripcion_niif');
	...
	function terminarGenerar($id,$id_empresa,$id_sucursal,$tablaPrincipal,$tablaCuentasNota,$idTablaPrincipal,$id_tercero,$notaCruce,$link){
		...
		if (!$query){
			...
			$sqlConsecutivo        = "SELECT consecutivo_niif FROM $tablaPrincipal WHERE activo=1 AND id='$id' AND id_empresa='$id_empresa'";
			$queryConsecutivo      = mysql_query($sqlConsecutivo,$link);
			$consecutivo_documento = mysql_result($queryConsecutivo,0,'consecutivo_niif');
		...
		$queryLog = mysql_query($sqlLog,$link);
	   	...
	   	echo'<script>
   				Ext.get("contenedor_NotaGeneral").load({
 		            url     : "nota_general_niif/bd/grillaContableBloqueada.php",
 		            scripts : true,
 		            nocache : true,
 		            params  :
 		            {
	...
	function moverDocumentosSaldos($id_empresa,$id_nota,$accion,$link){
		if ($accion=='agregar') {
			$sqlCompra="UPDATE compras_facturas AS CF,
						...
						AND CF.cuenta_pago = NC.cuenta_niif
			...
			$sqlVenta="UPDATE ventas_facturas AS VF,
						...
						AND VF.cuenta_pago = NC.cuenta_niif
	...
	function validaNota($id,$tablaPrincipal,$tablaCuentasNota,$idTablaPrincipal,$id_empresa,$id_sucursal,$id_tercero,$notaCruce,$link){
		...
		$sqlCuentasNota   = "SELECT cuenta_niif,descripcion_niif,debe,haber FROM $tablaCuentasNota WHERE activo=1 AND $idTablaPrincipal='$id'";
	...
	function moverCuentasDocumento($idDocumento,$idEmpresa,$id_sucursal,$tablaCuentasNota,$idTablaPrincipal,$accion,$id_tercero,$link){
		...
		if ($accion=='agregar') {
			...
			while ($row=mysql_fetch_array($query)){
				...
				$row['id_documento_cruce']=($row['id_documento_cruce']=='0' || $row['id_documento_cruce']=='')? $idDocumento : $row['id_documento_cruce'] ;
				$row['tipo_documento_cruce'] = ($row['tipo_documento_cruce']=='')? 'NCG' : $row['tipo_documento_cruce'] ;
				$documento_cruce=($documento_cruce=='0' || $documento_cruce=='')? $consecutivoNota : $documento_cruce ;
	...
	function modificarDocumentoGenerado($idDocumento,$opcGrillaContable,$id_empresa,$id_sucursal,$id_bodega,$tablaPrincipal,$tablaCuentasNota,$idTablaPrincipal,$link){
		...
		$sql   = "SELECT consecutivo,sinc_nota FROM $tablaPrincipal WHERE id='$idDocumento' AND id_empresa='$id_empresa' LIMIT 0,1 ";
		$query = mysql_query($sql,$link);
		...
		if ($sinc_nota!="niif") {
			echo '<script>
					alert("Aviso!\nNo se puede editar por que se genero apartir de contabilidad colgaap\ndebe editarla desde la seleccion contabilidad colgaap");
					document.getElementById("modal").parentNode.parentNode.removeChild(document.getElementById("modal").parentNode);
				</script>';
			exit;
		}
		...
		if($query){
			...
			echo'<script>
				 	Ext.get("contenedor_'.$opcGrillaContable.'").load({
						url     : "nota_general_niif/grilla/grillaContable.php",
						scripts : true,
						nocache : true,
						params  :
						{
	...
	function guardarCuenta($id_empresa,$consecutivo,$id,$cont,$opcGrillaContable,$tablaCuentasNota,$idTablaPrincipal,$idPuc,$cuenta,$debe,$haber,$id_tercero,$terceroGeneral,$id_documento_cruce,$numeroDocumentoCruce,$prefijoDocumentoCruce,$tipoDocumentoCruce,$link){
		...
		if($lastId > 0){
			...
			$sql="SELECT centro_costo FROM puc_niif WHERE activo=1 AND id_empresa=$id_empresa AND id=$idPuc ";
			$query=mysql_query($sql,$link);
	...
	function validaDocumentoCruce($id_empresa,$id_documento_cruce,$documento_cruce,$prefijo_documento_cruce='',$numero_documento_cruce,$opcGrillaContable,$cont,$id_tercero_general,$id_tercero,$cuenta,$evento,$debe,$haber,$link){
		...
		//SI ESA CUENTA ESTA RELACIONADA A UN DOCUMENTO FC 'FACTURA DE COMPRA'
		if ($documento_cruce=='FV') {
			$tablaCruce='ventas_facturas';
			$wherePrefijo= 'prefijo = \''.$prefijo_documento_cruce.'\'';
			$campoNumeroDocumentoCruce='numero_factura';
			$idTerceroCruce='id_cliente';
		}
		else if ($documento_cruce=='FC') {
			$tablaCruce='compras_facturas';
			$wherePrefijo= 'prefijo_factura = \''.$prefijo_documento_cruce.'\'';
			$campoNumeroDocumentoCruce='numero_factura';
			$idTerceroCruce='id_proveedor';
		}

		$whereDocumento=$wherePrefijo.' AND '.$campoNumeroDocumentoCruce.'='.$numero_documento_cruce.' AND ';
		//VALIDAR QUE TENGA UN NUMERO DE DOCUMENTO
		if ($numero_documento_cruce=='' || $numero_documento_cruce=='0') {
			echo'<script>
					alert("Si relaiona una '.$documento_cruce.', debe ingresar el numero!");
					document.getElementById("numeroDocumentoCruce'.$opcGrillaContable.'_'.$cont.'").focus();
				</script>';
			exit;
		}

		$id_tercero_documento=($id_tercero=='' || $id_tercero==0)? $id_tercero_general : $id_tercero;
		...
		if ($idAsiento=='') {
			...
			echo '<script>
					alert("El valor a cruzar es superior al registrado en el documento.");
					'.$script.'
					setTimeout(function(){ document.getElementById("cuenta'.$opcGrillaContable.'_'.$cont2.'").focus(); },100);
				</script>';
			exit;
	...
	//========================== FUNCION PARA CARGAR LA CUENTA NIIF ==============================================================//
 	function cargaConfiguracionCuenta($id_nota,$idInsertCuenta,$idCuenta,$id_empresa,$opcGrillaContable,$cont,$link,$block){
		$sql   = "SELECT N.cuenta_niif,
					...
					FROM nota_contable_general_cuentas AS N LEFT JOIN puc_niif AS P ON(
							P.activo = 1
							AND P.id_empresa = N.id_empresa
							AND P.cuenta = N.cuenta_niif
							AND P.id = N.id_niif
						)
					WHERE N.id = '$idInsertCuenta'
						AND N.id_niif = '$idCuenta'
		...
		if($tieneCcos == 'Si'){
 		...
 		echo'<div style="width:100%; padding-top:10px;">
 				'.$divCccos.'
 				<div style="color: #15428b;font-weight: bold;font-size: 13px;font-family: tahoma,arial,verdana,sans-serif;text-align:center;margin-top:20px;float: left;width:100%;">OBSERVACION</div>
 				<div style="float:left;width:90%;background-color:#FFF;margin-top:10px;margin-left:20px;border:1px solid #D4D4D4;">
	...
	function cancelarDocumento($id,$opcGrillaContable,$tablaPrincipal,$tablaCuentasNota,$idTablaPrincipal,$id_sucursal,$id_empresa,$link){
		...
		$sql   = "SELECT estado,consecutivo,sinc_nota FROM $tablaPrincipal WHERE id='$id' AND id_empresa='$id_empresa' LIMIT 0,1 ";
		$query = mysql_query($sql,$link);
		...
		else if ($estado=='0') { $sqlUpdate="UPDATE $tablaPrincipal SET activo=0 WHERE id='$id' AND id_sucursal='$id_sucursal' AND id_empresa='$id_empresa' ";	}
		else if ($estado=='1') {
			//VALIDAMOS QUE NO TENGA ARTICULOS RELACIONADOS, SE DEBE REVERSAR EL MOVIMIENTO QUE HICIERON LOS ARTICULOS
			$sqlValidaArticulos   ="SELECT COUNT(id) AS cont FROM inventario_movimiento_notas WHERE activo=1 AND consecutivo_nota='$consecutivo' AND id_empresa='$id_empresa' AND id_sucursal='$id_sucursal'";
			$queryValidaArticulos =mysql_query($sqlValidaArticulos,$link);
 	...
 	function validarCuenta($cuenta,$id_empresa,$link){
		$sqlCuenta   = "SELECT COUNT(id) AS contCuenta FROM puc_niif WHERE activo=1 AND id_empresa='$id_empresa' AND cuenta LIKE '$cuenta%'";
		$queryCuenta = mysql_query($sqlCuenta,$link);
