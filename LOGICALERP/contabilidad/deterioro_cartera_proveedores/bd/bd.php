<?php
	...
	function cambia_sucursal($id_sucursal,$id_deterioro_cliente,$id_empresa,$mysql){
		echo$sql="UPDATE deterioro_cartera_proveedores SET id_sucursal='$id_sucursal' WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente ";
	...
	function cambia_fecha($fecha,$id_deterioro_cliente,$id_empresa,$mysql){
		$sql="UPDATE deterioro_cartera_proveedores SET fecha='$fecha-12-31' WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente ";
		$query=$mysql->query($sql,$mysql->link);
		if (!$query) {
	...
	function guardar_valor_input($id_deterioro_cliente,$opcGrillaContable,$valor,$id_input,$id_empresa,$mysql){
		...
		echo$sql="UPDATE deterioro_cartera_proveedores SET $campoUpdate WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente ";
		$query=$mysql->query($sql,$mysql->link);
		if (!$query) {
	...
	function cargar_facturas($id_deterioro_cliente,$opcGrillaContable,$tasa_descuento,$rotacion,$fecha,$id_empresa,$mysql){
		...
		$sql="SELECT
					...
					deteriorable,
					fecha_factura
				FROM deterioro_cartera_proveedores_facturas
		...
		$sql="SELECT
				...
				FROM
					deterioro_cartera_proveedores_facturas AS DF
				INNER JOIN deterioro_cartera_proveedores AS D ON DF.id_deterioro_cliente=D.id
				WHERE
				D.estado=1
	...
	function guardar_factura($id_deterioro_cliente,$id_factura,$tiempo_estimado,$porcentaje_recaudo,$estado,$deterioro,$deteriorar,$id_empresa,$mysql){
		...
		$sql    = "SELECT id FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente=$id_deterioro_cliente AND id_factura=$id_factura";
		$query  = $mysql->query($sql,$mysql->link);
		$id_row = $mysql->result($query,0,'id');

		if ($id_row>0) {
			$sql="UPDATE deterioro_cartera_proveedores_facturas SET estado='$estado',tiempo_estimado_pago='$tiempo_estimado',porcentaje_recaudo='$porcentaje_recaudo',deterioro='$deterioro',deteriorable='$deteriorar' WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente=$id_deterioro_cliente AND id_factura=$id_factura";
			$query=$mysql->query($sql,$mysql->link);
		}
		else{
			$sql="INSERT INTO deterioro_cartera_proveedores_facturas
	...
	function eliminar_factura($id_deterioro_cliente,$id_factura,$id_empresa,$mysql){
		$sql="DELETE FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente=$id_deterioro_cliente AND id_factura=$id_factura";
		$query=$mysql->query($sql,$mysql->link);
		echo '<script>
	...
	function deteriorable($id_deterioro_cliente,$id_factura,$deteriorable,$id_empresa,$mysql){
		echo$sql="UPDATE deterioro_cartera_proveedores_facturas SET deteriorable='$deteriorable' WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente=$id_deterioro_cliente AND id_factura=$id_factura";
		$query=$mysql->query($sql,$mysql->link);
		if ($query){
	...
	function calcular_deterioro($opcGrillaContable,$id_deterioro_cliente,$id_factura,$periodos,$tasa_descuento,$rotacion,$valor_factura,$id_empresa,$mysql){
		...
		$sql="SELECT COUNT(id) AS deterioros_anteriores,tiempo_estimado_pago FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND id_factura=$id_factura AND deteriorable='true' ";
		$query=$mysql->query($sql,$mysql->link);
	...
	function generar_deterioro($id_deterioro_cliente,$id_empresa,$mysql){
		...
		$sql="SELECT COUNT(id) AS cont FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente=$id_deterioro_cliente";
		$query=$mysql->query($sql,$mysql->link);
		...
		echo '<script>
				...
				Ext.get(divLoad).load({
					url     : "deterioro_cartera_proveedores/deterioro_cartera_proveedores_bloqueado.php",
					scripts : true,
					nocache : true,
					params  :
					{
	...
	function contabilizar_documento($id_deterioro_cliente,$accion,$id_empresa,$mysql){
		if ($accion=='contabilizar') {
			...
			$sql="SELECT cuenta,naturaleza,estado,centro_costo FROM deterioro_cartera_proveedores_cuentas WHERE activo=1 AND id_empresa=$id_empresa";
			$query=$mysql->query($sql,$mysql->link);
			while ($row=$mysql->fetch_array($query)) {
				$cuenta     = $row['cuenta'];
			...
			$sql="UPDATE deterioro_cartera_proveedores SET estado=1 WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente";
			$query=$mysql->query($sql,$mysql->link);

			// CONSULTAR LOS DATOS DE LA CABECERA DEL DOCUMENTO
			$sql   = "SELECT fecha,tasa_descuento,rotacion,consecutivo,id_sucursal FROM deterioro_cartera_proveedores WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente";
			$query = $mysql->query($sql,$mysql->link);
			...
			$sql="SELECT
					...
					/*SELECT COUNT(id) AS cont,id_factura FROM deterioro_cartera_proveedores_facturas
			...
			if (!$query) {
				...
				$sql="UPDATE deterioro_cartera_proveedores SET estado=0 WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente";
				$query=$mysql->query($sql,$mysql->link);
				exit;
			}
	...
	function editar_documento($id_deterioro_cliente,$id_empresa,$mysql){
		...
		echo '<script>
				...
				var divLoad = document.getElementById("content_DeterioroCarteraClientes").parentNode.id;
				Ext.get(divLoad).load({
					url     : "deterioro_cartera_proveedores/deterioro_cartera_proveedores.php",
					scripts : true,
					nocache : true,
					params  :
					{
	...
	function cancelar_documento($id_deterioro_cliente,$id_empresa,$mysql){
		...
		// CAMBIAR EL ESTADO DEL DOCUMENTO
		$sql="UPDATE deterioro_cartera_proveedores SET estado=3 WHERE activo=1 AND id_empresa=$id_empresa AND id=$id_deterioro_cliente";
		$query=$mysql->query($sql,$mysql->link);
		if (!$query) {
			echo '<script>
					MyLoading2("off",{texto:"No se actualizo el documento, intentelo de nuevo",icono:"fail",duracion:2500});
