<?php
	...
	if(!isset($id_deterioro_cliente) || $id_deterioro_cliente=='' || $id_deterioro_cliente==0){
		...
		$sql="INSERT INTO deterioro_cartera_proveedores (random,fecha,id_usuario,id_sucursal,id_empresa)
				VALUES ('$random_documento','$fecha',$id_usuario,$id_sucursal,$id_empresa) ";
		$query=$mysql->query($sql,$mysql->link);

		// CONSULTAR EL ID DEL DOCUMENTO
		$sql   = "SELECT id FROM deterioro_cartera_proveedores WHERE activo=1 AND id_empresa=$id_empresa AND random='$random_documento' ";
		$query = $mysql->query($sql,$mysql->link);
	...
	// SI EXISTE LA NOTA DE DETERIORO
		...
		$sql="SELECT
				...
				id_sucursal,
				consecutivo
			FROM deterioro_cartera_proveedores
		...
		$sql="SELECT
					...
					deteriorable,
					fecha_factura
				FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND id_deterioro_cliente = $id_deterioro_cliente";
		$query=$mysql->query($sql,$mysql->link);
		while ($row=$mysql->fetch_array($query)) {
			$porcentaje_baja    = '';
			$porcentaje_recaudo = '';
		...
		$sql="SELECT COUNT(id) AS cont,id_factura FROM deterioro_cartera_proveedores_facturas WHERE activo=1 AND id_empresa=$id_empresa AND $whereIdFacturasPeriodos GROUP BY id_factura";
		$query=$mysql->query($sql,$mysql->link);
		while ($row=$mysql->fetch_array($query)) {
...
<div class="content" id="content_<?php echo $opcGrillaContable; ?>">
	<div class="separator">DETERIORO CARTERA PROVEEDORES <div class="close" onclick="Win_Ventana_nuevo_deterioro.close();"></div></div>
...
<script>
	...
	function cambia_sucursal_<?php echo $opcGrillaContable; ?>(id_sucursal) {
		...
		Ext.get('loadForm').load({
			url     : 'deterioro_cartera_proveedores/bd/bd.php',
			scripts : true,
			nocache : true,
			params  :
			{
