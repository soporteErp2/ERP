<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//QUERY
			...
			$grilla->MyWhere				= " activo = 1 AND id_empresa=$id_empresa AND (id in(SELECT id_extracto FROM extractos_detalle WHERE activo=1) OR consecutivo>0) ";		//WHERE DE LA CONSULTA A LA TABLA "$TableName"s"
...
if(!isset($opcion)){  ?>
	...
	<script>
		function Editar_<?php echo $opcGrillaContable; ?>(id){
			var direccionRender = 'conciliacion_bancaria/extracto/bd/grillaContableBloqueada.php'
			,	  estado          = document.getElementById('imgEstado<?php echo $opcGrillaContable; ?>_'+id).getAttribute('src');

      if(estado == 'img/estado_doc/0.png'){ direccionRender = 'conciliacion_bancaria/extracto/grillaContable.php'; }
	    ...
	    Ext.get("contenedor<?php echo $opcGrillaContable; ?>").load({
 			...
 			if(consecutivoDoc != ''){
				document.getElementById('titleDocumento<?php echo $opcGrillaContable; ?>').innerHTML = 'Extracto<br>N. ' + consecutivoDoc;
				if(estado == 'img/estado_doc/3.png'){
					document.getElementById('titleDocumento<?php echo $opcGrillaContable; ?>').style.color = "#f00";
				} else{
					document.getElementById('titleDocumento<?php echo $opcGrillaContable; ?>').style.color = "#333";
				}
			}
			else{
