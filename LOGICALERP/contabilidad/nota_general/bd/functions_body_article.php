<?php
	...
	function cargaArticulosSaveConTercero($id,$observacion,$estado,$opcGrillaContable,$tablaCuentasNota,$idTablaPrincipal,$link){
		...
		$idRemision              = '';
		$btn_elimina_cuentas =($estado<1)? '<img src="img/delete_all.png" style="cursor:pointer;" title="Eliminar Registros" onclick="dir_eliminar_cuentas()" id="button-delete-acounts">' : '' ;
		$sql   = "SELECT id,id_puc,cuenta_puc,descripcion_puc,debe,haber,id_tercero,nit_tercero,tercero,tipo_documento_cruce,id_documento_cruce,prefijo_documento_cruce,numero_documento_cruce FROM $tablaCuentasNota WHERE $idTablaPrincipal='$id' ORDER BY cuenta_puc ASC";
		...
		$body = '<div class="contenedorGrilla">
					...
					<div class="contenedorHeadArticulos">
						<div class="headArticulos" id="head'.$opcGrillaContable.'">
							...
							<div style="float:right; min-width:80px;">'.$btn_elimina_cuentas.'</div>
						</div>
					</div>
			...
			if ($estado < 1) {
				while($row = mysql_fetch_array($query)){
					...
					$body .='<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
								'.cargaDivsUnidadesSaveConTercero($cont,$opcGrillaContable, $row['id'], $row['id_puc'], $row['cuenta_puc'], $row['descripcion_puc'], $row['debe'], $row['haber'],$row['id_tercero'],$row['nit_tercero'],$row['tercero'], $row['tipo_documento_cruce'],$row['id_documento_cruce'], $row['prefijo_documento_cruce'], $row['numero_documento_cruce'],$link).'
							</div>';
				}
				$cont++;
				$body .='<div class="bodyDivArticulos'.$opcGrillaContable.'" id="bodyDivArticulos'.$opcGrillaContable.'_'.$cont.'">
	...
	function cargaDivsUnidadesSaveConTercero($cont,$opcGrillaContable,$id = 0,$id_puc = 0,$cuenta='',$descripcion='',$debe=0,$haber=0,$id_tercero,$nit,$tercero,$tipo_documento_cruce,$id_documento_cruce,$prefijo_documento_cruce,$numero_documento_cruce,$link){
		...
		$body ='<div class="campoNotaGeneral" style="width:40px !important; overflow:hidden;">
				...
				<div class="campoNotaGeneral"><input type="text" id="descripcion'.$opcGrillaContable.'_'.$cont.'" style="text-align:left;" readonly value="'.$descripcion.'" /></div>
	...
	function cargaDivsUnidadesBloqueadasConTercero($cont,$opcGrillaContable,$id = 0,$id_puc = 0,$cuenta='',$descripcion='',$debe=0,$haber=0,$id_tercero,$nit,$tercero,$tipo_documento_cruce,$prefijo_documento_cruce,$numero_documento_cruce){
		...
		$body ='<div class="campoNotaGeneral" style="width:40px !important; overflow:hidden;">
				...
				<div style="float:right; min-width:80px;">
					<div onclick="ventanaConfiguracion'.$opcGrillaContable.'('.$cont.')" id="configurarCuenta'.$opcGrillaContable.'_'.$cont.'" title="Configurar Cuenta Niif" style="width:20px; float:left; margin-top:3px;cursor:pointer;'.$displayBtns.'"><img src="img/config16.png" /></div>
				</div>
