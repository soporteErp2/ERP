<?php
	...
	//CONFIGURACION//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//QUERY
			...
			$grilla->MyWhere			= "activo = 1 AND id_sucursal=$id_sucursal AND id_empresa=$id_empresa AND (sinc_nota='colgaap_niif' OR sinc_nota='colgaap')
											AND id in(SELECT $idTablaPrincipal FROM $tablaCuentasNota WHERE activo=1)";						//WHERE DE LA CONSULTA A LA TABLA "$TableName"
		...
		//CONFIGURACION DE CAMPOS EN LA GRILLA
			$grilla->AddRowImage('Estado','<center><img src="img/estado_doc/[estado].png"  style="cursor:pointer" width="16" height="16" id="imgEstado'.$opcGrilla.'_[id]" /></center>','50');
...
if(!isset($opcion)){  ?>
	...
	<script>
		function Editar_<?php echo $opcGrilla; ?>(id){
			var direccionRender = '<?php echo $carpeta; ?>bd/grillaContableBloqueada.php'
			,	estado          = document.getElementById('imgEstado<?php echo $opcGrilla; ?>_'+id).getAttribute('src');

            if(estado == 'img/estado_doc/0.png' ){ direccionRender = '<?php echo $carpeta; ?>grilla/grillaContable.php'; }

	        Ext.get("contenedor_<?php echo $opcGrilla; ?>").load({
				url     : direccionRender,
				scripts : true,
				nocache : true,
				params  :
 			...
 			consecutivoDoc = document.getElementById('div_<?php echo $opcGrilla; ?>_consecutivo_'+id).innerHTML;
 			if(consecutivoDoc != ''){ document.getElementById('titleDocumento<?php echo $opcGrilla; ?>').innerHTML='Nota Contable<br>N. '+consecutivoDoc; }
			else{ document.getElementById('titleDocumento<?php echo $opcGrilla; ?>').innerHTML='' }

			Win_Ventana_buscar_<?php echo $opcGrilla; ?>.close();
		}
