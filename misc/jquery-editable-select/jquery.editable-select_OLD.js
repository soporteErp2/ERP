...
(function ($) {
	...
	$.fn.editableSelect = function (options) {
		...
		var select = this.clone(), input = $('<input type="text" placeholder="Seleccione...">'), list = $('<ul class="es-list">');
		...
		var EditableSelect = {
			init: function () {
				...
				select.find('option').each(function () {
					...
					if ($(this).attr('selected')) {
						input.val($(this).text());
						input.attr("data-id",$(this).val());
						input.attr("data-value",$(this).text());	
					}		   
			...
			show: function (e) {
				...
				list.find('li').each(function (i, element) {					
					var largo = $(element).html().length;
					if(largo > mayor) mayor = largo;					
				});				
				width = mayor*8;
				if(width < 200)	width = 198;				
				list.find('li').show();
				list.css({ top: input.offset().top + input.outerHeight() - 1, left: input.offset().left, width:  width});
				var hidden = (options.filter && evento != 'click') ? list.find('li:nic(' + input.val() + ')').hide().size() : 0;
